...
| **Materialised views** | Asynchronous, eventually-consistent filter / re-project views maintained off a source tree's write-ahead log. A predicate projection keeps the matching subset, retracts keys that update out of the predicate, and converges by source HLC. | [Materialised Views](docs/lattice/materialised-views.md) |
