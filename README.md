...
| **Atomic visibility (single-tree)** | Strict per-tree atomic visibility for `SetManyAtomicAsync`: a per-tree `ITxRegistryGrain` is the single tree-wide linearization point for the saga's commit/abort decision, and every read path consults it before exposing prepared values. No reader anywhere in the local cluster ever observes a partial-set state. The saga's all-or-nothing **outcome** is preserved across clusters — successful commits and abort-compensations are both durably replicated under causal+/LWW, so remote peers eventually converge to the same outcome the local cluster reached. **Cross-cluster atomic *visibility* is not currently exposed:** a remote reader may observe individual keys of an in-flight batch arriving per-key before the full set has converged. See [Consistency](docs/lattice/consistency.md#atomic-visibility-single-tree-foreground-and-cross-cluster). |
