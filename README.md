...
| `Orleans.Lattice.Api.TreeAdmin` | Transport-agnostic control facade for whole-tree administration, composing the existing single-responsibility facades (it wraps the schema control facade by delegation). This foundation ships the capability probe only; whole-tree lifecycle operations land in later releases. | [README](docs/lattice.api.treeadmin/README.md) |
