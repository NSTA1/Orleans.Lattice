...
| **Backup & restore** | Point-in-time backup and restore for a tree: causally consistent full and incremental capture, scheduling with chain retention, an optional cross-tree causal fence, and a fail-closed permission model over a pluggable sink (a durable Azure Blob sink is included). | [Backup & Restore](docs/lattice.backup/README.md) | [sample](samples/BackupAndRestore/README.md) |
