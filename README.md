...
| **Explorer plugins** | The console is composed of plugins rather than built-in tabs. A plugin ships as its own package carrying its own view, the single domain contract that is the whole of its reach, and its own access gate; a head surfaces an area by registering it and withholds it by not registering it. Adding a tab needs no change to the shell. | [Writing a plugin](docs/lattice.explorer/writing-a-plugin.md) | [sample](samples/Explorer/README.md) |
