...
- **Shard projection-digest entry counts stay exact across internal-node splits (FX-022).** When a B+ tree internal node split, the moved half of its children left stale per-child digest rows behind on the donor node, and a moved child could keep publishing its digest upward to its former parent - so the shard's `GetLeafProjectionDigestAsync` total double-counted (or, after a partial fix, under-counted) the moved subtree's entries until the next reactivation. The split path now prunes the moved children's digest rows and recomputes the donor's subtree aggregate before publishing upward, rejects digest snapshots from children a node no longer owns, and re-seeds a child's parent pointer only while the node still owns it. `LeafProjectionDigest.EntryCount` now stays exactly equal to the number of distinct entries under the shard across an arbitrary sequence of internal-node splits, with no transient over- or under-count visible to a quiescent digest read.
