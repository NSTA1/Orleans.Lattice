...
- **`docs/lattice/configuration.md` Options Reference table now lists every `LatticeOptions` property.** Twelve options that exist on `LatticeOptions` were missing from the alphabetical options-reference table - `ActivationReadyTimeout`, `CompactionTriggerCooldown`, `DirtyLeafFlushIntervalMs`, `LeafSnapshotMargin`, `LeafSnapshotReClassifyEveryNCheckpoints`, `MaxLeafEntriesBeforeForcedCompaction`, `MaxSnapshotReplayEntries`, `MinTombstoneRatioForCompaction`, `RetryPolicy`, `ShardForwardTimeout`, `SnapshotLeafIdleTtl`, and `WalStorageProvider` - so a reader scanning the table for a knob to tune could not find it. Each row now appears alphabetically with its current default and a link to wherever the option's behaviour is described (in-file for the four with prose sections in `configuration.md` itself; cross-doc to `tombstone-compaction.md` / `projection-rebuild.md` / `snapshot-cursors.md` / `retry-policy.md` / `wal-storage-providers.md` for the rest).
...
- **Storage-usage gauges no longer pin idle trees fully resident,
