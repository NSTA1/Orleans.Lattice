...
- **Writer-layer per-partition pending-append dispatch is now observable, so the upstream variant of the WAL wedge is attributable without source-walking (G-025).** The G-024 pack instruments `WalShardGrain.FlushAsync` and surfaces every in-flight slot's lifecycle stage, but a 2026-06-03 cohort showed 5 of 7 wedged silos with every shard's in-flight chain empty and hundreds of callers parked upstream inside `WalCommitLogWriter.AppendForPartitionAsync` instead. Two new instruments close the writer-layer attribution gap: `orleans.lattice.wal.writer.append.dispatched` (per-`(tree, partition)` counter, incremented at the `Enqueued` lifecycle stamp before the shard RPC is invoked) and `orleans.lattice.wal.writer.partition.pending_appends` (histogram of the per-partition pending-dispatch depth observed at every append entry). Each in-flight dispatch also carries a lightweight lifecycle stamp (`Stage`, `StageStartedTicks`) on a `PendingAppend` instance held by the writer's per-partition tracker; an out-of-process ClrMD walk reads the stamp and the azure-throughput silo's `StallWatchdog` is extended to emit one `[wal-append] tree=... partition=... entries=N stage=... stuck=Nms` line per pending dispatch at every snapshot, mirroring the shard-layer `[wal-slot]` line one layer up. No public-API surface change.
