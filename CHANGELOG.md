...
### Pending: v9.0.0 coordinated family major release

Every published package advances in lockstep to `9.0.0`. This major consolidates a whole-tree administration control plane, completes the convergent-type primitive catalogue, and re-gates the highest-blast-radius tree operations behind a dedicated capability. The full, per-entry detail of every change folded into this release is preserved in the v8.x archive's `## Unreleased` section ([`CHANGELOG.old.v8.md`](CHANGELOG.old.v8.md)); the headlines follow.
...
Published v9.x releases will appear here, newest first, once tagged. See [`docs/RELEASING.md`](docs/RELEASING.md) for the per-package tag-and-publish protocol.
