...
never double-counts. Set `StorageUsagePollInterval` to `TimeSpan.Zero` to opt out. The advisory policy uses a hysteresis band so a tree hovering near the ceiling is not trimmed on every garbage-collection pass: byte pressure arms when retained WAL crosses `WalMaxRetainedBytes` and disarms once a trim drives retained bytes at or below `WalMaxRetainedBytes * WalBytePressureReclaimTarget` (default `0.8`), and growth that stays inside that band does not re-trigger until the ceiling is crossed again.
