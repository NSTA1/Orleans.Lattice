...
- **Per-shard WAL flush lifecycle and reshard activity are now observable, so the residual phase-1/activation WAL wedge can be attributed without source-walking (G-024).** The G-023 deadlines correctly bound their own seams but never fire on the residual wedge, which leaves the in-flight chain pinned at `WalMaxPendingBatches` for 120+ seconds with no signal as to which line of `WalShardGrain.FlushAsync` the head slot is stuck at. Five new instruments close that gap: `orleans.lattice.wal.shard.start_flush.calls` (per-`(tree, shard)` counter that distinguishes "new flushes still kicking off" from "kick-off loop blocked" in one cohort), `orleans.lattice.wal.shard.pending_segments` (histogram of `_pendingSegments.Count` at every `StartFlush`), `orleans.lattice.shard_root.reshard.initiated` / `.rejected` / `.completed` (counters tagged by `tree` and, for rejections, `reason`), and `orleans.lattice.shard_root.reshard.in_flight` (per-tree 0/1 observation recorded at every `ReshardAsync` entry). Each in-flight flush slot also carries a lightweight lifecycle stamp (`Stage`, `StageStartedTicks`) that an out-of-process ClrMD walk can read to name the exact code region a wedged slot is stuck at; the azure-throughput silo's `StallWatchdog` is extended to emit one `[wal-slot] tree=... shard=... slot=[start,end) stage=... stuck=Nms` line per slot at every snapshot. No public-API surface change.
