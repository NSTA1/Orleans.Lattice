...
- **Storage-usage gauges no longer pin idle trees fully resident, and no longer compete with the producer hot path under load (FX-025).** The byte-accurate storage-usage feature shipped in v6.1.x ran its per-silo poller through `ILatticeAdmin.GetTotalStorageUsageAsync`, which fanned every poll out across every shard's leaf chain, activating every leaf, every leftmost-spine internal node, and every per-leaf snapshot storage grain on the poll cadence - so cold trees were never cold, and on the saturation rung the poll path contended with foreground writes on the same Azure Table connection pool, the `ILattice` activation, and the WAL shard's non-reentrant turn. The poller now drives a new cheap `ILatticeAdmin.PollWalUsageAsync` that fans out only to WAL partition grains and is bypassed by every leaf and snapshot grain entirely. The leaf-state and snapshot-bytes surfaces on `ILattice.GetStorageUsageAsync` are now served in O(1) per shard from incrementally-maintained running totals (leaves publish their per-leaf byte footprint to the owning shard root on every commit boundary; `LeafSnapshotBlob.SnapshotBytes` is precomputed at capture time), so the deep API stays accurate without re-walking the tree. A new `ILatticeAdmin.RefreshStorageUsageAsync` is the explicit operator-driven re-anchor seam for the rare case where a fresh deep walk is wanted (post-migration validation, reconciliation). The cluster-wide `Get`/`Poll`/`Refresh` admin methods are now `[AlwaysInterleave]` so a slow tree never stalls sibling administrative calls.
