...
- Multi-partition WAL replay on leaf activation. Hosts that opt into `LatticeOptions.WalPartitions > 1` now have every leaf's activation-time materialiser fan out across `[0, WalPartitions)` partitions, each with its own fall-off-log classification, slice read loop, projection-checkpoint advance, and saga-prepare clamp. New wire-additive `LeafNodeState.ProjectionCheckpointOffsetsByPartition` slot persists per-partition checkpoints; per-partition cursor consumer ids (`_lattice_materialiser_{treeId}_{leafGrainId}_{partition}`) let the per-shard WAL GC trim each partition under its own slowest consumer. New `LatticeOptions.WalPartitions >= 1` option validator. Silo-wide default remains `1` so existing single-partition deployments are unchanged.
