...
Last updated: 2026-07-06 (W3 landing #1125 scheduling+retention e929fc15: public ILatticeBackupScheduler [TriggerFullBackupAsync/TriggerIncrementalBackupAsync/EnsureScheduleAsync/PruneAsync] forwards to one per-scope scheduler grain keyed by BackupScopeKey.For(scope) so triggers/scheduled captures/retention for a scope never overlap. Overlap guard is an activation-local check-and-set with no await between [atomic within a turn], methods AlwaysInterleave so a concurrent trigger observes the guard. Recurring full/incremental schedules ride Orleans reminders, both operator opt-in and disabled by default, intervals clamp up to a 1-min minimum. Retention: keep-last-count OR max-age union, then ExpandBaseClosure computes the transitive BaseBackupId closure to a fixpoint so a retained increment's whole base chain survives; content-addressed artifacts shared with a pruned manifest are protected from deletion; disabled = keep-all no-op. Incremental exposed via ILatticeBackupIncrementalCaptureService; default BaselineIncrementalBackupCaptureService is a chain-shape-correct TryAddSingleton stand-in [full snapshot stamped as increment on a real base, guarded so it never deletes its own base/artifact] that #1124's differential engine will override. LatticeBackupScheduleOptions per-scope named options; ConfigureLatticeBackupSchedule global+per-scope. Aliases olb.gs/ss/rr. Cross-feature fix: implemented CaptureSetAsync on the GatedBackupCaptureService test double to satisfy the interface member #1133 added. Backup non-chaos 116/116. W3 remaining: #1124 incremental [NO COMMIT on its branch yet; #1125 already ships the incremental SEAM + stand-in, so #1124 becomes the differential ENGINE that overrides the stand-in registration - reconcile at land time, must not re-declare ILatticeBackupIncrementalCaptureService / LatticeBackupIncrementalCaptureRequest]. Prior W3: #1133 cross-tree fence 761966d0; #1126 restore a0bdc5b8.
