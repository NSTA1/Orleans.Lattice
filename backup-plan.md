...
Last updated: 2026-07-06 (W3 landing #1126 restore a0bdc5b8: public ILatticeBackupRestoreService [RestoreAsync + RevertRestoreAsync]; reads a manifest chain base-first with cycle detection; validates every artifact against its recorded SHA-256 digest before applying [trust boundary]; verbatim replay through the core HLC-preserving shard seams [BulkLoadRawAsync empty-tree fast path; MergeManyAsync LWW convergence] preserving HLC/VV/origin/expiry/tombstone; in-place + atomic shadow-cutover [alias swap + revert] modes; prefix/key granularity validated within the captured scope; fail-closed Restore auth; deterministic idempotency. 5 deviations ratified [null-origin-safe MergeManyAsync/BulkLoadRawAsync over ApplyMergeManyAsync; fast-path on !ExistsAsync; single-silo proactive routing refresh; dropped a capture-side tombstone test; the CRDT-label test]. 11 restore tests; backup project 78/78 non-chaos. IMPORTANT FIX 6c93b8bd: the #1123 capture merge-mode refinement was accidentally left UNSTAGED when #1123 was squashed [committed the test but not the production edit], so a clean checkout of feat/backup had the CRDT-label test failing; the #1126 sub-agent correctly flagged it; staged the production refinement and re-verified backup 67/67 from a clean tree. W3 remaining in flight: #1133, #1124, #1125.)
