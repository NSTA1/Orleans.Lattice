...
| W3 | #1124 | Incremental | LANDED | 17eeffc6 |
| W3 | #1125 | Scheduling + retention | LANDED | e929fc15 |
| W3 | #1126 | Restore | LANDED | a0bdc5b8 |
| W4 | #1128 | Control facade | LANDED | 04431383 |
| W5 | #1129 | gRPC binding + client | LANDED | 96bba177 |
| W6 | #1130 | Observability + samples + e2e + docs | LANDED | 647c0ed2 |

Last updated: 2026-07-06 (W6 LANDED 647c0ed2 - all six waves in). W6 shipped backup observability (the full orleans.lattice.backup.* instrument family: capture/restore/retention counters + duration histograms + incremental-lag + failure counters tagged phase/reason + inventory observable gauges over an in-memory registry), the admin surface (control-facade GetInventoryAsync/GetScopeStatusAsync + additive scheduler-state last-run/last-success slots), a BackupAndRestore console sample, and end-to-end + observability + admin test suites. Coordinator review fix folded into the landing commit: W6's instruments were consolidated onto the package's dedicated orleans.lattice.backup meter (BackupMetrics.Meter) rather than the core meter, matching the auth/membership one-meter-per-add-on convention and unifying them with the existing cross-tree-fence instruments. Verified: backup 130/130, api.backup 28/28 (non-chaos). Separately landed a hygiene slice-coverage completeness guard (d0bd9a7a) that asserts CoreHygieneScope.AllPackageSliceRoots agrees exactly with the per-package fixtures' ForSlice(...) declarations, so a package can never be registered-but-unscanned (gap) or scanned-twice (drift). DEFERRED to epic close (coordinator/dashboards-owned): a bundled Backup Grafana dashboard + LatticeDashboardKind.Backup + BackupMeterDashboardCoverageTests (the shared MeterDashboardCoverageTestsBase wiring for the backup meter), and an optional RequestSizeContractTests for the backup API surfaces. NEXT: epic-close docs (docs/lattice.backup + docs/lattice.api.backup + docs/lattice.api.backup.grpc), per-package features.md, CHANGELOG, dashboard + coverage test, post this plan's detail as a comment on epic #1119, full non-chaos suite, then raise the feat/backup -> main PR.
Last updated: 2026-07-06 (W4 + W5 LANDED; W6 in progress). W4 #1128 api.backup control facade landed on feat/backup as 04431383 (gate-first control ops; exclusive-ordinal catalog cursor; bounded-memory streaming export; base-first chain describe; safe delete that never removes a shared artifact) - reviewed against source, 22/22 targeted tests. W5 #1129 api.backup.grpc code-first binding + typed client landed as 96bba177 (mirrors api.state.grpc: marshallers/methods/service, default-deny auth interceptor exempting only the auth-scheme discovery call, oibg. alias registry) - reviewed, 26/26 targeted tests. feat/backup then rebased onto origin/main d21a928f (picks up #1140 history-view drain de-flake) and force-pushed with lease; the pre-rebase per-issue source shas in the table above are retained as the authoritative landing record. W6 #1130 (observability metrics + admin surface + BackupAndRestore sample + e2e) dispatched to a background Feature Dev agent; docs / CHANGELOG / features.md / this plan's retirement remain coordinator-owned at epic close. NEXT: land W6, then epic-close docs + full non-chaos suite + feat/backup -> main PR.
Last updated: 2026-07-07 (W3 COMPLETE - landing #1124 incremental 17eeffc6: replaced the #1125 full-snapshot incremental stand-in with a true forward-WAL differential engine, adapted onto #1125's ILatticeBackupIncrementalCaptureService seam - the capture service now implements that seam directly and #1124's parallel main-interface CaptureIncrementalAsync + LatticeBackupIncrementalRequest were dropped. Engine resumes from the base backup's per-partition WAL offsets [carried on BackupConsistencyCut.WalPartitionOffsets], tails every partition through the shared commit-log subscription [all origins, maintenance-filtered, fall-off aware], scope-filters, and streams the delta as a uniform LwwEntry[] artifact - byte-identical in shape to a full capture, so the restore chain [#1126] decodes base+increments through one path with ZERO restore changes. Set->live entry, Delete->tombstone entry, each carrying its full LWW envelope. WAL fall-off past the base resume point AND a range delete in the delta window [no faithful point-keyed representation] both fall back to a fresh full backup. DI: the concrete capture service is registered once and forwarded to both ILatticeBackupCaptureService and ILatticeBackupIncrementalCaptureService [one instance, shared WAL pin]; the BaselineIncrementalBackupCaptureService stand-in was removed. Cross-feature BUG FOUND + FIXED: the increment content descriptor's ContentHash must be the pure artifact-bytes hash so #1126 restore's integrity gate reproduces it; the chained backup id folds in the base id separately [so two empty increments off different bases never collide]. Added an end-to-end base+increment chain-restore test to LatticeBackupRestoreIntegrationTests plus a range-delete->full fallback test. Deviations ratified: (1) surface adapted to #1125 seam; (2) artifact LwwEntry[] not LatticeMutation[] [per-key Mode preserved in manifest KeyDescriptors, matching full-capture convention]; (3) range-delete->full fallback; (4) scope inherited from base manifest [request.Scope advisory, matches scheduler call site]. Backup non-chaos 123/123 [incl. the previously-flagged merge-mode-label test, now green]. Solution Release build clean. Prior W3: #1125 e929fc15; #1133 cross-tree fence 761966d0; #1126 restore a0bdc5b8. NEXT: W3-boundary full non-chaos suite, then W4 #1128 control facade.
