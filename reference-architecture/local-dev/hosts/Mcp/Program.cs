...
// Development-only per-request identity for the local-dev dual-cluster harness.
// MCP tool discovery is fail-closed on the caller's resolved credential, so a head
// running without an identity provider (Entra off) advertises ZERO tools to an
// anonymous caller. When explicitly opted in here, each request is authenticated as
// the subject named in its OWN `Authorization: Bearer <id>` header and that id is
// forwarded to the silo, so an agent can act as any identity by changing the bearer
// token and the silo's deny-by-default model enforces on it. Forced OFF whenever
// Entra is enabled, so it can never weaken a real deployment even if left set.
var devPerRequestIdentity = !entraEnabled && config.GetValue("Mcp:DevPerRequestIdentity", false);
...
// The tenant self-awareness + administration tool groups (lattice_tenant_*).
// Wired only when the estate runs with multi-tenancy enabled (the silo maps the
// tenant-admin gRPC facade under the matching Tenancy:Enabled flag); when off, the
// TenantAdmin endpoint is left null so neither the self-awareness nor the admin
// tools are contributed and the head is unchanged. Co-hosted on the silo gRPC
...
foreach (var regionSection in config.GetSection("Mcp:Regions").GetChildren())
{
    ...
    peerRegions.Add(new LatticeApiMcpRemoteRegionOptions
    {
        ...
        TenantAdmin = tenancyEnabled ? RegionEndpoint(regionSection["TenantAdminEndpoint"] ?? peerState) : null,
    });
}
...
builder.Services.AddLatticeMcpRemote(options =>
{
    ...
    // Tenant self-awareness + administration, wired only when tenancy is enabled;
    // left null otherwise so neither tool group is contributed. Co-hosted on the
    // silo gRPC endpoint, so it dials the same address as State.
    if (tenancyEnabled)
    {
        options.TenantAdmin = new LatticeApiMcpRemoteEndpoint { Endpoint = tenantAdminEndpoint, CallInvoker = OriginLockInvoker(tenantAdminEndpoint) };
        options.EnableTenantControl = enableTenantControl;
    }
...
else if (devPerRequestIdentity)
{
    // Local dev per-request identity: authenticate each request as the subject
    // named in its own bearer token so fail-closed tool discovery scopes to that
    // caller. See the devPerRequestIdentity comment above; unreachable when Entra
    // is on.
    builder.Services.AddLocalDevPerRequestIdentity();
    builder.Services.AddAuthorization();
}
...
if (entraEnabled || devPerRequestIdentity)
{
    app.UseAuthentication();
    app.UseAuthorization();
}

if (devPerRequestIdentity)
{
    app.Logger.LogWarning(
        "DEVELOPMENT PER-REQUEST IDENTITY ACTIVE: every MCP request is authenticated as the subject named in its " +
        "own Authorization: Bearer <id> header. This is only permitted because Entra is disabled and " +
        "Mcp:DevPerRequestIdentity=true. Never enable this in a real deployment.");
}
