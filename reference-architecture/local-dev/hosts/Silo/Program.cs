...
// Multi-tenancy. Off by default: the whole tenant stack (isolation gate, tenant
// registry, self-service + administration API, and its gRPC binding) is wired
// only when Tenancy:Enabled=true, so a default harness run is byte-for-byte the
// single-tenant cluster. When enabled, the silo registers tenancy AFTER auth /
// membership (a hard ordering the tenancy registrar fail-fasts on), co-hosts the
// tenant-admin control API on the shared silo gRPC endpoint, and the TenantSeeder
...
builder.Host.UseOrleans(silo =>
{
    ...
    // Registered only when Tenancy:Enabled=true, and strictly AFTER membership
    // and auth above: AddLatticeTenancy fail-fasts if the membership directory or
    // the auth decision engine is absent, and AddLatticeTenantAdminApi fail-fasts
    // if the tenant registry is absent. Adding tenancy layers the active tenant
    // isolation gate on top of the auth gate and stands up the tenant registry;
    // the tenant-admin control API exposes the self-service (current / list / get)
    // and administration facades the MCP head's lattice_tenant_* tools call. The
    // tenant-admin gRPC binding and its endpoint mapping are added with the other
    // facades below, under the same tenancyEnabled guard.
    if (tenancyEnabled)
    {
    ...
    else if (config.GetValue("Auth:DevAuthenticateForwardedSubject", false))
    {
        ...
        // named subject verbatim and attach the groups the mounted identity model
        // declares for it, so an agent can act as any identity by setting
        // `Authorization: Bearer <id>` (through the MCP head, a gRPC facade, or the
        // Explorer sign-in). Enforcement stays real - a subject has authority only
        // through the groups the model grants it under deny-by-default - so an
        // unlisted id authenticates as itself but is denied everything. Registered
        // only in the no-Entra harness; see DevIdentityCredentialAuthenticator.
        var identityModel = LocalDevIdentityModel.Load(
            config["Auth:IdentitiesFile"] ?? LocalDevIdentityModel.DefaultPath);
        silo.Services.AddSingleton(identityModel);
        silo.Services.AddSingleton<ILatticeCredentialAuthenticator, DevIdentityCredentialAuthenticator>();
        ...
        // subjects) declared in the same mounted identity model into the tenant
        // registry, so lattice_tenant_list / _get resolve differentiated results
        // per identity. The seeder needs the LocalDevIdentityModel singleton
        // registered just above, so it is registered here in the same dev-auth
        // branch rather than beside AddLatticeTenancy.
        if (tenancyEnabled)
        {
            silo.Services.AddHostedService<TenantSeeder>();
        }
    }
});
...
// The tenant-administration control-API gRPC binding, co-hosted on the shared
// silo gRPC endpoint (same address as State), wired only when tenancy is enabled.
// The coarse transport gate mirrors every other facade: RequireAuthorization is
// off in the local harness, so the AllowAll authorizer is registered only when a
// deployment turns the transport gate on. Real authority is enforced beneath it -
// the self-service reads resolve the caller's own tenants from their credential,
// and the mutating admin RPCs are gated by the tenant policy cascade.
if (tenancyEnabled)
{
    builder.Services.AddLatticeTenantAdminApiGrpc(options => options.RequireAuthorization = requireApiAuthorization);
    if (requireApiAuthorization)
    {
        builder.Services.AddSingleton<ILatticeTenantAdminApiAuthorizer, AllowAllTenantAdminApiAuthorizer>();
    }
}
