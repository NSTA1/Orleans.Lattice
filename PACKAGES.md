...
| `Orleans.Lattice.Api.Backup` | [![NuGet](https://img.shields.io/nuget/v/Orleans.Lattice.Api.Backup)](https://www.nuget.org/packages/Orleans.Lattice.Api.Backup) | Transport-agnostic control facade for driving backup capture (full, incremental, and backup sets), recurring schedules, restore and revert, cold restore and catalog rebuild from the sink, catalog listing, chain describe, deletion, and backup health monitoring. | [README](docs/lattice.api.backup/README.md) |
...
| Package | NuGet | Description | Docs |
|---|---|---|---|
| `Orleans.Lattice.Explorer.Web` | [![NuGet](https://img.shields.io/nuget/v/Orleans.Lattice.Explorer.Web)](https://www.nuget.org/packages/Orleans.Lattice.Explorer.Web) | Opt-in, auth-aware web console for a running cluster - a tree browser plus capability-gated Backups, Access, Tenant administration, My tenant, and Telemetry areas over the gRPC APIs (and a Schema area withheld by default) - embeddable via `AddLatticeExplorerWeb` / `MapLatticeExplorer` or run standalone. Composes the shared explorer libraries (`Explorer.Core`, `.UI`, `.Backup`, `.Access`, `.Schema`) and the Tenants, My Tenant, and Telemetry plugins into an ASP.NET Core head. | [README](docs/lattice.explorer/README.md) |
...
| `Orleans.Lattice.Explorer.UI` | [![NuGet](https://img.shields.io/nuget/v/Orleans.Lattice.Explorer.UI)](https://www.nuget.org/packages/Orleans.Lattice.Explorer.UI) | Shared Razor component class library for the Explorer: the routable pages, layout, navigation, detail, and authentication components (plus packaged static web assets) rendered identically by every explorer head. | [README](docs/lattice.explorer/running-the-explorer.md) |
...
| `Orleans.Lattice.Explorer.Plugins.Data` | Unreleased | The Data browser plugin: cursor-paged key/value browsing for a selected tree or view, with a starts-with key search, a tag-index filter, live-following entry detail, and decoded CRDT current state. | [Docs](docs/lattice.explorer/writing-a-plugin.md) |
...
| `Orleans.Lattice.Explorer.Plugins.DeadLetter` | Unreleased | The dead-letter plugin: a read-only, cursor-paged view of the strict-mode dead letters a selected tree rejected, with a per-entry detail pane. | [Docs](docs/lattice.explorer/writing-a-plugin.md) |
...
| `Orleans.Lattice.Explorer.Plugins.Tenants` | Unreleased | The Tenants plugin, for a platform operator: tenant lifecycle, quota, region authorization, admin subjects, and cross-tenant grants. | [Docs](docs/lattice.explorer/writing-a-plugin.md) |
