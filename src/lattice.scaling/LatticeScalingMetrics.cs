...
public static class LatticeScalingMetrics
{
    ...
    /// </summary>
    /// <remarks>
    /// This class declares no instrument fields of its own; the autoscaling instruments
    /// are created elsewhere from this meter. Note that this field sits at the end of
    /// the file: if an instrument field is ever added here it must be declared
    /// <b>below</b> this line and constructed from this field, never above it. Static
    /// field initialisers execute in declaration order, so a listener matching
    /// <c>ReferenceEquals(instrument.Meter, LatticeScalingMetrics.Meter)</c> that is the
