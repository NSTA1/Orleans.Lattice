...
public static partial class LatticeReplicationServiceCollectionExtensions
{
    ...
    /// <summary>
    /// Registers a typed OR-Map replication shape for the tree identified by
    /// <paramref name="treeName"/>. Required whenever a tree is configured
    /// for <see cref="BPlusTree.Grains.LatticeMergeMode.OrMap"/>: the
    /// receiver-side applier reads the matching descriptor at apply time so
    /// it can deserialise the generic
    /// <see cref="OrMapDelta{TKey, TValue}"/> wire payload and fold it into
    /// the loaded <see cref="Primitives.OrMap{TKey, TValue}"/> via
    /// <see cref="Primitives.OrMap{TKey, TValue}.MergeDelta(OrMapDelta{TKey, TValue})"/>.
    /// Registering a different <c>(TKey, TValue)</c> pair for the same tree
    /// is a configuration error and throws at registration time.
    /// </summary>
    public static ISiloBuilder AddOrMapReplicationShape<TKey, TValue>(
        this ISiloBuilder builder,
        string treeName)
        where TKey : notnull
        where TValue : Primitives.ICrdt<TValue>, new()
    {
        ...
        builder.Services.TryAddSingleton<OrMapReplicationShapeRegistry>();
        builder.Services.AddSingleton<IConfigureOptions<OrMapReplicationShapeRegistryStartupMarker>>(
            new ConfigureOrMapShape(treeName, OrMapReplicationShape.For<TKey, TValue>()));
        // Eager registration via a hosted-startup hook so the descriptor is
        // installed before the first WAL apply runs.
        builder.Services.AddSingleton<IHostedService, OrMapReplicationShapeStartup>();
        return builder;
    }

    /// <summary>Internal hosted-service marker; registered once per silo.</summary>
    internal sealed class OrMapReplicationShapeRegistryStartupMarker { }

    /// <summary>Internal <see cref="IConfigureOptions{TOptions}"/> carrying one shape registration.</summary>
    internal sealed class ConfigureOrMapShape(string treeId, OrMapReplicationShape shape)
        : IConfigureOptions<OrMapReplicationShapeRegistryStartupMarker>
    {
        /// <summary>The tree id this shape is bound to.</summary>
        public string TreeId { get; } = treeId;

        /// <summary>The apply descriptor.</summary>
        public OrMapReplicationShape Shape { get; } = shape;

        /// <inheritdoc />
        public void Configure(OrMapReplicationShapeRegistryStartupMarker options) { }
    }

    /// <summary>
    /// Hosted service that drains every registered
    /// <see cref="ConfigureOrMapShape"/> into the singleton
    /// <see cref="OrMapReplicationShapeRegistry"/> at silo start, before
    /// the replication applier services accept any inbound entry.
    /// </summary>
    internal sealed class OrMapReplicationShapeStartup(
        OrMapReplicationShapeRegistry registry,
        IEnumerable<IConfigureOptions<OrMapReplicationShapeRegistryStartupMarker>> registrations) : IHostedService
    {
        /// <inheritdoc />
