...
internal abstract class LatticeTreeAdminGrpcServiceBase
{
    ...
    public abstract Task<TreeOrphanedLeafReport> AuditOrphanedLeaves(TreeAdminTreeRequest request, ServerCallContext context);

    /// <summary>Repairs the tree's orphaned leaves on the wrapped facade.</summary>
    public abstract Task<TreeOrphanedLeafReport> RepairOrphanedLeaves(TreeAdminTreeRequest request, ServerCallContext context);
    ...
    public static void BindService(ServiceBinderBase binder, LatticeTreeAdminGrpcServiceBase? serviceImpl)
    {
        ...
        if (serviceImpl is null)
        {
            ...
            binder.AddMethod(methods.AuditOrphanedLeaves, (UnaryServerMethod<TreeAdminTreeRequest, TreeOrphanedLeafReport>?)null);
            binder.AddMethod(methods.RepairOrphanedLeaves, (UnaryServerMethod<TreeAdminTreeRequest, TreeOrphanedLeafReport>?)null);
        ...
        binder.AddMethod(methods.AuditOrphanedLeaves, new UnaryServerMethod<TreeAdminTreeRequest, TreeOrphanedLeafReport>(serviceImpl.AuditOrphanedLeaves));
        binder.AddMethod(methods.RepairOrphanedLeaves, new UnaryServerMethod<TreeAdminTreeRequest, TreeOrphanedLeafReport>(serviceImpl.RepairOrphanedLeaves));
...
internal sealed class LatticeTreeAdminGrpcService : LatticeTreeAdminGrpcServiceBase
{
    ...
    /// <inheritdoc />
    public override Task<TreeOrphanedLeafReport> AuditOrphanedLeaves(TreeAdminTreeRequest request, ServerCallContext context)
        => InvokeAsync(request, context, static (control, req, ct) => control.AuditOrphanedLeavesAsync(req.TreeId, ct));

    /// <inheritdoc />
    public override Task<TreeOrphanedLeafReport> RepairOrphanedLeaves(TreeAdminTreeRequest request, ServerCallContext context)
        => InvokeAsync(request, context, static (control, req, ct) => control.RepairOrphanedLeavesAsync(req.TreeId, ct));
