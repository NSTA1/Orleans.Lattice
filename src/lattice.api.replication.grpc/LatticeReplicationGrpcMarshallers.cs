...
/// Orleans serializer, so every replication control-API wire message rides the
/// same versioned, additive-only serialization contract the facade DTOs already
/// use. The buffer-writer hand-off writes the encoded payload straight into the
/// gRPC stream's <see cref="IBufferWriter{T}"/> without an intermediate array.
/// </summary>
