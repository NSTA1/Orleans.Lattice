...
/// an ambient <see cref="LatticeCredential"/>, which the schema control path
/// stamps on the Lattice credential context so the schema access gate can
/// resolve the caller's subject and authorize each operation.
/// </summary>
/// <remarks>
/// <para>
/// This is the identity seam for auth-backed schema control. A host that needs a
...
/// <c>AddLatticeSchemaApiGrpc</c> runs; the built-in default reads a single
/// configurable bearer-style header
/// (<see cref="LatticeSchemaApiGrpcOptions.CredentialHeaderName"/> /
/// <see cref="LatticeSchemaApiGrpcOptions.CredentialScheme"/>).
/// </para>
/// <para>
/// <b>Precedence.</b> This runs independently of, and after, the transport-level
/// <see cref="ILatticeSchemaApiAuthorizer"/> (the coarse allow / deny gate keyed
...
/// schema engine's own fail-closed access gate applied to a call that was
/// allowed to run.
/// </para>
/// <para>
/// <b>Fail-closed.</b> Returning <see langword="null"/> (no resolvable
/// credential) leaves the caller anonymous. When auth-backed schema control is
/// active an anonymous caller is denied, so a missing or malformed credential
/// header can never drive a mutation.
/// </para>
/// </remarks>
public interface ILatticeSchemaApiCredentialBridge
{
    /// <summary>
