...
/// <see cref="ServiceClient"/> must be configured. The cache reads the populated
...
/// </para>
/// </summary>
public sealed class LatticeAzureBlobCacheOptions
{
    /// <summary>
    ...
    /// </summary>
    public const string DefaultContainerName = "orleans-lattice-cache";

    /// <summary>
    /// Storage account connection string. When set, the cache builds the
    ...
    /// When set, the cache uses this instance verbatim instead of constructing
    /// its own from <see cref="ConnectionString"/> / <see cref="ServiceUri"/> +
    ...
    /// <summary>
    /// The blob container that backs the cache. Defaults to
    ...
    /// non-default name or a <see cref="KeyPrefix"/> to share an account across
    /// multiple caches without collisions.
    /// </summary>
    ...
    /// <summary>
    /// Optional callback invoked when the cache constructs the
    ...
    internal void Validate()
    {
        if (string.IsNullOrWhiteSpace(ContainerName))
        {
            throw new InvalidOperationException(
                $"{nameof(LatticeAzureBlobCacheOptions)}.{nameof(ContainerName)} must not be null or empty.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"{nameof(LatticeAzureBlobCacheOptions)} requires one authentication mode: set "
        ...
        {
            throw new InvalidOperationException(
                $"{nameof(LatticeAzureBlobCacheOptions)} authentication modes are mutually exclusive: set only one of "
    ...
    /// </summary>
    /// <returns>The container client the cache reads from and writes to.</returns>
    /// <exception cref="InvalidOperationException">The options are not a valid, single authentication mode.</exception>
