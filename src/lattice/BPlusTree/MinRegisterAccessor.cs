...
/// <summary>
/// Typed value-surface accessor for a monotone <em>min</em> register stored
/// under a single key in an <see cref="ILattice"/> - the low-water-mark
/// primitive that keeps the smallest totally-ordered value ever written. The
...
/// value under the total order carried on the wire (a strictly lesser order
/// key); backwards writes and duplicate deliveries are no-ops. Concurrent
/// active-active writes from different clusters converge on the single smallest
/// value because the fold is a directional min over a total order - commutative,
...
public readonly record struct MinRegisterAccessor<T>
{
    ...
    internal MinRegisterAccessor(ILattice lattice, string key, Func<T, byte[]> orderKeySelector, ILatticeSerializer<T> serializer)
    {
        _lattice = lattice;
        _key = key;
    ...
    /// blind candidate delta: the receiver folds it as a directional min and
    ...
    public Task SetAsync(T value, CancellationToken cancellationToken = default, int maxAttempts = DefaultMaxAttempts)
    {
        ...
        return _lattice.ApplyCrdtDeltaAsync(_key, LatticeMergeMode.MinRegister, deltaBytes, cancellationToken);
    }
    ...
    public Task SetAsync(T value, TimeSpan ttl, CancellationToken cancellationToken = default, int maxAttempts = DefaultMaxAttempts)
    {
        ...
        EnsureInitialised();
        var deltaBytes = BoundedRegisterAccessorHelper.EncodeDelta(_serializer, _orderKeySelector, value);
        return _lattice.ApplyCrdtDeltaAsync(_key, LatticeMergeMode.MinRegister, deltaBytes, ttl, cancellationToken);
    }
    ...
    /// <summary>
    /// Reads the current register state. Returns an empty (never-written) min
    ...
    /// min direction. Useful for replication consumers that have computed a
    ...
    private void EnsureInitialised()
    {
        if (_lattice is null || _serializer is null || _orderKeySelector is null)
        {
            throw new InvalidOperationException(
                "MinRegisterAccessor is uninitialised; obtain it via ILattice.MinRegister<T>(key, orderKeySelector) instead of `default`.");
        }
    }
}
