...
internal sealed partial class BPlusLeafGrain
{
    ...
    private async Task PersistAsync([System.Runtime.CompilerServices.CallerMemberName] string caller = "")
    {
        ...
        try
        {
            // #1557 / #1566: converge a benign cold-start first-create write
            // race (both etags empty on the brand-new leaf row) by adopting the
            // concurrently-committed topology seed. The shared helper's
            // empty/empty guard restricts the adopt to a genuine insert-vs-insert
            // race; a stale-state conflict on an existing row still surfaces,
            // preserving the #1560 fail-loud contract.
