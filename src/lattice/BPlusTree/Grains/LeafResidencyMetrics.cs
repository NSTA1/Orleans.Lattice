...
/// </para>
/// <para>
/// The gauges carry the <b>platform</b> tenant sentinel and no other tag. The
/// working set is per-silo and spans every tree, so it has no tenant to derive
/// and a <c>tree</c> tag would be a category error; the silo dimension arrives
/// from the exporter's resource attributes.
/// <see cref="LatticeTenantLabel.PlatformMeasurement{T}"/> is what states that
/// positively. A bare <c>Measurement&lt;long&gt;</c> would leave the series with
/// no tenant dimension at all, which an operator cannot distinguish from a site
/// that simply forgot to attribute one - the same absent-versus-zero ambiguity
/// these gauges exist to remove, displaced from the value onto the label.
/// <c>TenantMetricDimensionHygieneTests</c> enforces this across <c>src/</c>.
/// </para>
/// </summary>
internal static class LeafResidencyMetrics
{
    ...
    internal static Measurement<long> MeasureBudgetBytes(LeafResidentWorkingSet workingSet)
    {
        ...
        return LatticeTenantLabel.PlatformMeasurement(workingSet.BudgetBytes);
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(workingSet);
        return LatticeTenantLabel.PlatformMeasurement(workingSet.ResidentBytes);
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(workingSet);
        return LatticeTenantLabel.PlatformMeasurement((long)workingSet.RegisteredCount);
    }
}
