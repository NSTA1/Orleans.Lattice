...
/// (issue 1955). A handful cannot be: for them, the fact that no other message
/// runs on the shard between the first and last leaf is precisely the invariant
/// the surrounding protocol depends on. Releasing the non-reentrant shard
/// mid-walk would open exactly the window those protocols exist to close - see
/// the per-site notes at each call site, and issue 1956 for the analysis.
/// </para>
/// <para>
/// Since they cannot be bounded, they are instead made <em>attributable</em>.
/// When one of these walks holds the shard longer than
/// <see cref="WarnAfter"/>, it logs a warning naming the operation and the
/// number of leaves it visited. Without that, a stalled shard surfaces only as
...
internal struct AtomicLeafWalk
{
    ...
    internal readonly void ReportIfSlow(ILogger logger, GrainId grainId)
    {
        ...
        logger.LogWarning(
            "ShardRootGrain {GrainId} held the shard for {ElapsedMs} ms in {Operation}, visiting {LeavesVisited} leaves. " +
            "This walk is intentionally atomic and is not work-bounded, so the shard is unavailable to every other " +
            "caller for its duration (issue 1956). A large leaf count here explains a concurrent burst of Orleans " +
            "long-request warnings on this shard.",
            grainId,
