...
- **Navigate the code graph** - `repocontext_outline` returns a file's declared-symbol skeleton without its body, `repocontext_related` resolves the types it references, the files that depend on it, and the tests that cover it, and `repocontext_changed` reports how the workspace has drifted from the index and the blast radius of those edits (it walks a caller-supplied path, so like the onboarding tools it is offered only when a `workspaceRoot` is configured).
- **Pack budgeted context** - `repocontext_context` returns a ranked, explained bundle of source for a task under a hard token ceiling and never re-charges content the caller already holds; `repocontext_stats` reports the aggregate token savings.
- **Coordinate several agents** - `repocontext_claim`, `_renew_claim`, `_release_claim`, and `_claim_status` take a leased, fenced claim over a memory record, so concurrent agents can drain one shared work queue without colliding.
...
`AddRepoContextTools()` with writes off offers only the read-only tools; pass `enableWrites: true` to also contribute the memory-writing and claim tools. Bootstrap takes the directory to walk from the caller, so it is contributed only when a `workspaceRoot` is passed as well - the snippet above, which passes none, offers no onboarding tool and no `repocontext_changed`. Bind an `IEmbeddingProvider` to turn on semantic search; without one, search still works in keyword mode.
