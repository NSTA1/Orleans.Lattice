        ...
        => McpServerTool.Create(
            ...
            new McpServerToolCreateOptions
            {
                ...
                Description =
                    ...
                    + "vectors are available it runs a semantic (nearest-neighbour) search, APPROXIMATE BY "
                    + "DEFAULT: it is answered from a persisted approximate index whose recall is bounded, not "
                    + "complete, with published floors of recall@10 >= 0.95 on a clustered corpus and >= 0.55 on "
                    + "an adversarially unclustered one, in exchange for a query cost that is sub-linear in the "
                    + "corpus and survives a restart. A host that needs complete recall configures the exact "
                    ...
                    + "reports which path answered ('semantic', 'keyword', or 'empty'), and the additive "
                    + "'retrievalPath' says exactly which retrieval path served it and, for a keyword answer, WHY: "
                    + "'semantic.exact' (complete-recall vector search), 'semantic.approximate' (bounded-recall "
        ...
        => McpServerTool.Create(
            ...
            new McpServerToolCreateOptions
            {
                ...
                Description =
                    ...
                    + "(semantic when an embedder and vectors are available - APPROXIMATE BY DEFAULT, with bounded "
                    + "rather than complete recall, or exact when the host configures it; otherwise a degraded "
                    + "keyword scan - a "
        ...
        => McpServerTool.Create(
            ...
            new McpServerToolCreateOptions
            {
                ...
                Description =
                    ...
                    + "cross-reference, session, and every vector tree - and leaves the durable agent-memory "
                    + "records for that repository intact. Use it to repair a "
                    + "wedged, stale, or corrupt code index without discarding notes, decisions, and gotchas: "
                    + "the repository stays registered AND stays listed by 'repocontext_list_repos', now "
                    + "reporting no ingest, no file count, and no indexed commit, which is exactly the state "
                    + "it is in. That matters because 'repocontext_list_repos' is how a caller resolves a "
                    + "repository id, so a reset repository whose memory was deliberately preserved stays "
                    + "discoverable rather than reachable only by an id you already knew. A subsequent "
                    + "'repocontext_add_repo' rebuilds the "
                    + "index and its vectors from the working files and repopulates those fields, and the "
                    + "surviving memory records are "
                    + "re-embedded on the next indexing pass. The working tree on disk is never touched. "
                    + "Resetting an unknown repository is a no-op that reports zero deletions and does not "
