...
/// <summary>
/// Default <see cref="ILatticeAuthApiCredentialBridge"/> that lifts a single
/// configurable request header into a <see cref="LatticeCredential"/>. Reads the
/// header named by <see cref="LatticeAuthApiGrpcOptions.CredentialHeaderName"/>
/// (default <c>authorization</c>), strips a leading
/// <see cref="LatticeAuthApiGrpcOptions.CredentialScheme"/> prefix (default
...
/// the facade's administrator check fails closed on every operation.
/// </remarks>
internal sealed class HeaderLatticeAuthApiCredentialBridge : ILatticeAuthApiCredentialBridge
{
    private readonly IOptions<LatticeAuthApiGrpcOptions> _options;

    /// <summary>
    /// Initialises the bridge with the resolved gRPC binding options.
    /// </summary>
    public HeaderLatticeAuthApiCredentialBridge(IOptions<LatticeAuthApiGrpcOptions> options)
    {
