...
internal static class LatticeApiMcpGroupCapabilityMap
{
    ...
    public static LatticeOperation RequiredOperations(LatticeApiMcpGroup group) => group switch
    {
        ...
        // (TreeLifecycle), the streamed tree-creation capability (BulkLoad), or the
        // restore-into-tree capability (Restore). A caller holding any of these may
        // use the group; the per-verb gate at call time still separates routine from
