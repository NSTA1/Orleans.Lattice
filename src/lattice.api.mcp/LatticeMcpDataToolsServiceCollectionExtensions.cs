...
public static class LatticeMcpDataToolsServiceCollectionExtensions
{
    ...
    /// plus the thirteen typed-CRDT reads; contributes the mutating tools (the point /
    /// batch writes <c>data_set</c>, <c>data_delete</c>, <c>data_delete_range</c>,
    /// <c>data_set_many</c>, <c>data_set_many_atomic</c>, <c>data_set_many_atomic_cross_tree</c>
    /// plus the thirteen typed-CRDT writes) only when <paramref name="enableWrites"/> is
