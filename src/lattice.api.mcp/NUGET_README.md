...
- **Permission-aware discovery** - a `lattice_capabilities` meta-tool and a per-session tool list computed from the caller's effective permissions: a facade group's tools are listed only to a caller granted an operation the group covers, so an ungranted group is never listed, not listed-then-denied. Each call is still authorized per tree and per verb by the facade's access gate.
