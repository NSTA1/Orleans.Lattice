...
- An **access gate** on the four-state model that distinguishes allowed, a
  genuine denial, and an unauthenticated connection (so the shell prompts a
  sign-in instead of greying the area out). It never reports the area
  unavailable: any other refusal or transport fault, including a cluster that
  does not expose the auth control facade at all, is treated as a denial, so the
  entry is demoted rather than removed. It also publishes the cluster's
  directory-availability sub-capability under the plugin's own scoped access key.
