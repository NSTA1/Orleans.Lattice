...
/// <summary>
/// Where the open sub-surface lives: in the address, so a link to the quota
/// surface reopens on the quota surface, and in the shell's preference contract,
/// so the caller returns to the surface they left.
/// </summary>
/// <remarks>
/// <para>
...
public sealed partial class TenantsWorkspace
{
    ...
    private void AddressSurface(string surfaceId, bool replaceHistoryEntry)
    {
        ...
        var next = route.WithParameter(TenantsPluginKeys.SurfaceQueryKey, surfaceId);
    ...
    private async Task PersistSurfaceAsync(string surfaceId)
    {
        ...
        {
            await _preferences.EnsureLoadedAsync().ConfigureAwait(false);
            await _preferences.SetAsync(TenantsPluginKeys.SurfacePreference, surfaceId)
    ...
    private string? AddressedSurface()
    {
        ...
        var surfaceId = route.Parameters.GetValueOrEmpty(TenantsPluginKeys.SurfaceQueryKey);
        return TenantsSurfaces.IsKnown(surfaceId) ? surfaceId : null;
    }
    ...
    private void OnRouteChanged(ExplorerRoute route)
    {
        ...
        var surfaceId = route.Parameters.GetValueOrEmpty(TenantsPluginKeys.SurfaceQueryKey);
        if (!TenantsSurfaces.IsKnown(surfaceId)
            || string.Equals(surfaceId, ActiveSurfaceId, StringComparison.Ordinal))
        {
            return;
        }

        // Fire and forget, exactly as a gate change that reopens the list does:
    ...
    private static bool IsThisArea(ExplorerRoute route) =>
        string.Equals(route.Area, TenantsPluginKeys.AreaSlug, StringComparison.Ordinal);
