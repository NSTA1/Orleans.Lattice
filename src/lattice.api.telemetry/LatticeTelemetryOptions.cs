...
/// <summary>
/// Options for the transport-neutral telemetry proxy: the read-only Prometheus /
/// PromQL-compatible backend a binding queries, the backend credential presented
/// on the far side of the dual-credential trust boundary, the range-query
/// guardrails, and the metric-access allow-list. Bound by
/// <see cref="LatticeApiTelemetryServiceCollectionExtensions.AddLatticeTelemetryBackend"/>
/// and resolvable via <c>IOptions&lt;LatticeTelemetryOptions&gt;</c>.
/// </summary>
/// <remarks>
...
/// caller's Lattice credential to the backend: the binding-side authorization (a
/// <c>LatticeOperation.Telemetry</c> grant) and the backend-side credential are
/// two independent halves of the trust boundary. A transport binding may derive
/// from this type to add its own binding-specific options while reusing the
/// backend, guardrail, and metric-access surface unchanged.
/// </remarks>
public class LatticeTelemetryOptions
{
    /// <summary>
