...
/// <summary>
/// Options controlling the <c>Orleans.Lattice.Api.Telemetry.Grpc</c> server-side
/// binding.
/// </summary>
public sealed class LatticeTelemetryApiGrpcOptions
{
    /// <summary>
    /// Whether the authorization interceptor enforces
    /// <see cref="ILatticeTelemetryApiAuthorizer"/> on every inbound telemetry
    /// call. Defaults to <see langword="true"/> (default-deny): the binding fails
    /// closed unless a host either registers a permissive authorizer or explicitly
    /// turns enforcement off. Set to <see langword="false"/> only when an outer
    /// authentication boundary already guards the endpoint.
    /// </summary>
    ...
    /// facade's own fail-closed gate can resolve the caller's subject and derive
    /// the effective tenant. Defaults to <c>authorization</c>. Only consulted when
    /// auth-backed control is active (the <c>Orleans.Lattice.Auth</c> add-on is
    /// registered); when it is not, no header is read.
    /// </summary>
    ...
    /// matching what the API clients send.
    /// </summary>
    /// <remarks>
    /// The assertion is only <em>carried</em> here. It is re-validated against the
    /// caller's own membership downstream, and the telemetry facade derives the
    /// effective tenant server-side regardless, so this header can never widen a
    /// caller's scope. Without it a multi-tenant caller on a split-head deployment
    /// would resolve to the reserved default tenant instead of the tenant it is
    /// actually acting as.
    /// </remarks>
