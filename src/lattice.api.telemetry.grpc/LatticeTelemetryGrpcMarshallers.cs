...
/// <summary>
/// Builds gRPC <see cref="Marshaller{T}"/> instances that delegate to the Orleans
/// serializer, so every telemetry wire message rides the same versioned,
/// additive-only serialization contract the facade DTOs already use. The
/// buffer-writer hand-off writes the encoded payload straight into the gRPC
/// stream's <see cref="IBufferWriter{T}"/> without an intermediate array, and the
/// read path decodes in place from a single-segment payload, renting from
/// <see cref="ArrayPool{T}"/> only for the rarer multi-segment case.
/// </summary>
internal static class LatticeTelemetryGrpcMarshallers
{
    /// <summary>
    /// Builds a contextual <see cref="Marshaller{T}"/> for <typeparamref name="T"/>
    /// bound to the supplied Orleans <paramref name="serializer"/>.
    /// </summary>
    /// <typeparam name="T">The wire message type.</typeparam>
    /// <param name="serializer">The Orleans serializer for <typeparamref name="T"/>.</param>
    /// <returns>A marshaller that encodes and decodes <typeparamref name="T"/>.</returns>
