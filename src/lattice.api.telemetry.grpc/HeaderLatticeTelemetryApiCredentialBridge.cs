...
/// <summary>
/// Default <see cref="ILatticeTelemetryApiCredentialBridge"/> that lifts a single
/// configurable request header into a <see cref="LatticeCredential"/>. Reads the
/// header named by
/// <see cref="LatticeTelemetryApiGrpcOptions.CredentialHeaderName"/> (default
/// <c>authorization</c>), strips a leading
/// <see cref="LatticeTelemetryApiGrpcOptions.CredentialScheme"/> prefix (default
/// <c>Bearer</c>) when present, and stamps the remaining token as the credential
/// with that scheme so a registered <c>ILatticeCredentialAuthenticator</c> can
/// resolve the caller's subject.
/// </summary>
/// <remarks>
/// A registered <c>ILatticeCredentialAuthenticator</c> is responsible for
/// validating the token; this bridge performs no validation and only shuttles the
/// opaque token onto the ambient credential context. An absent, empty, or
/// whitespace header yields <see langword="null"/> (an anonymous caller), which
/// the facade scopes fail-closed.
/// </remarks>
internal sealed class HeaderLatticeTelemetryApiCredentialBridge : ILatticeTelemetryApiCredentialBridge
{
    private readonly IOptions<LatticeTelemetryApiGrpcOptions> _options;

    /// <summary>Initialises the bridge with the resolved gRPC binding options.</summary>
    /// <param name="options">The binding options.</param>
    /// <exception cref="ArgumentNullException"><paramref name="options"/> is <see langword="null"/>.</exception>
    public HeaderLatticeTelemetryApiCredentialBridge(IOptions<LatticeTelemetryApiGrpcOptions> options)
    {
