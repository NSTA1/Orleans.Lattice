...
- **If a coordinator dispatched you**, its workstream topic is your
  **coordination bus**: `scan` it before you start, post your handoffs back to it
  with no TTL (silent expiry starves whoever reads next), and set `author` to your session
...
- **Internal visibility**: Non-public grain interfaces (everything other than the public entry points `ILattice`, `ILatticeAdmin`, `ILatticeLockGrain`, and `IAtomicActionGrain`) must be declared `internal`. The C# type system enforces the boundary at compile time - do not add runtime guard filters.
...
3. **Raise a chore release PR.** This PR contains the changelog stamp from step 2 **and** the package-version bumps to `X.Y.Z` across every `.csproj` (each carries its own `<Version>`) / version-stamping file that participates in the release. Workflow:
