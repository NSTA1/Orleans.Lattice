...
- **Drift is bidirectional - audit omissions, not just inaccuracies.** Verifying that every claim the docs *make* is accurate is only half the sweep; it is blind to what the docs *fail to mention*. A missing package row, an undocumented option, an unlisted metric, or a source type with no docs entry is drift of equal severity to a wrong value, and the accuracy pass cannot surface it because there is no false claim to catch. For every doc structure that enumerates a source set - the `PACKAGES.md` package inventory, feature tables, metric catalogs, option tables, per-project `api.md`/`configuration.md` - you must run the diff in *both* directions: doc -> source (is every listed item still real and accurate?) and source -> doc (does every source item appear?). Report a source item with no doc entry as an omission finding, with the source citation, exactly as you would a stale claim.
...
- **Edit deterministically, not by similarity.** Every markdown edit goes through verbatim `replace_string_in_file` anchors per the byte-level rule in the markdown-editing skill (`.github/skills/markdown-editing/SKILL.md`), which `.github/copilot-instructions.md` points to. No `// ...existing code...` placeholders on long markdown files; long markdown is fragile and silently collapses neighbouring near-identical bullets.
...
- **Each project's docs mirror the same layout.** Every project under `docs/<project>/` is anchored by a `README.md` in the standard project layout (What is it? / Core Properties / Features table / Quick Start / Reference links to the sub-feature docs; the root `README.md` is now the platform entry point and no longer follows that layout), plus an `api.md`, a `configuration.md`, an `architecture.md`, and a `chaos-tests.md` wherever that project ships a chaos suite. There is one `docs/<package>/` folder per `src/<package>/`, plus the docs-only `docs/crdt/` conceptual topic and the `docs/videos/` companion pages for the video series - do not treat any short list of them as exhaustive. The core library is the one exception to the anchoring rule: `docs/lattice/` carries no index of its own, because the repository root `README.md` is its index, which is why every sibling package links back to it as `../../README.md`. Project-specific content lives in that project's docs; a sibling project's docs link across rather than duplicate. When content in a core (or replication) doc is specific only to an add-on project, move it into that project's docs (or, if already there, truncate the host doc to a one-line pointer) - never maintain two copies.

## Scope contract

When the user requests a documentation review, settle these three parameters before starting:
...
| **Per-project layout** | `docs/<project>/README.md` | confirm each project README follows the standard project layout and links its sub-feature docs (including `api.md`, `configuration.md`, `architecture.md`, `chaos-tests.md` where they exist) |
...
3. For each doc-side claim, classify as one of:
   ...
   For every axis that enumerates a source set, additionally run the source -> doc reverse pass and classify each source item as **Documented** or **Omitted**; add every omission (the missing item plus its source citation and the doc structure it belongs in) to the fix queue alongside the stale claims.
...
2. For long markdown (> ~200 lines) or files with many adjacent near-identical bullets, follow the byte-level edit protocol in the markdown-editing skill (`.github/skills/markdown-editing/SKILL.md`) - assert exactly-one-match before replacing, then `git diff` to confirm only the intended lines changed.
...
- **Auditing only doc -> source (accuracy) and never source -> doc (completeness).** A table that enumerates a set - packages, metrics, options, public members - is wrong if it is missing a row, even when every row it does contain is accurate. Verifying the present rows and stopping is the omission blind spot the user has been burned by. For every enumerating inventory, run the reverse set-diff and list the absentees. The `PACKAGES.md` inventory specifically must have one row per released/publishable child package; a table where every row is accurate but five packages are missing is a failed sweep, not a passed one.
...
- The root README child-package table listed only 24 of the 33 published child packages. Nine real, tag-released packages had no row: the three MCP packages (`Api.Mcp`, `Api.Mcp.Telemetry`, `Api.Mcp.Telemetry.Azure`), the Explorer Entra login provider, and the five shared Explorer libraries (`Explorer.Core`, `.UI`, `.Access`, `.Backup`, `.Schema`). Every row that *was* present was accurate, so a doc-claim-only accuracy pass reported the table as clean; a reverse set-diff of `git tag` package prefixes and packable `<PackageId>`s against the table rows surfaced all nine in one command. This is the canonical omission-class finding: completeness, not correctness.
