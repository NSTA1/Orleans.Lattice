...
- **Each project's docs mirror the same layout.** Every project under `docs/<project>/` (`docs/lattice/`, `docs/lattice.replication/`, ...) is anchored by a `README.md` that mirrors the root `README.md` structure (What is it? / Core Properties / Features table / Quick Start / Reference links to the sub-feature docs). Project-specific content lives in that project's docs; a sibling project's docs link across rather than duplicate. When content in a core doc is specific only to an add-on project, move it into that project's docs (or, if already there, truncate the core doc to a one-line pointer) - never maintain two copies.
