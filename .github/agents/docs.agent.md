...
| **Enumerated-inventory completeness** (any doc table or list that claims to enumerate a source set - the root `README.md` child-package table foremost, plus feature tables, metric catalogs, and option tables) | the full source set: for the package table, every packable `src/<project>/` (a `.csproj` whose `<PackageId>` is set and `<IsPackable>` is not `false`) cross-checked against released `git tag` prefixes; for other inventories, the relevant source registry | **reverse set-diff, source -> doc.** Enumerate the source set, extract the doc set, and report every source item with no row/entry as an omission. For the package table: `git tag` prefixes and packable-csproj `<PackageId>`s must each have a table row (the core `Orleans.Lattice` is documented above the table, not as a child-package row); the row count should equal the distinct child-package count. Nested projects (e.g. the shared `Explorer.*` libraries under `src/lattice.explorer/`) are separate packages - do not assume one `src/<project>/` maps to one package |
...
- **Auditing only doc -> source (accuracy) and never source -> doc (completeness).** A table that enumerates a set - packages, metrics, options, public members - is wrong if it is missing a row, even when every row it does contain is accurate. Verifying the present rows and stopping is the omission blind spot the user has been burned by. For every enumerating inventory, run the reverse set-diff and list the absentees. The root README child-package table specifically must have one row per released/publishable child package; a table where every row is accurate but five packages are missing is a failed sweep, not a passed one.
