...
| HLC monotonicity violation across deactivation | A `HybridLogicalClock.Tick(previous)` call whose `previous` is not reloaded from the persisted high-water mark on `OnActivateAsync`. | Integration test that mutates a grain, deactivates it (from the fixture side, with the `ForceDeactivateAsync()` hook the leaf and shard-root grain interfaces expose, or `grain.AsReference<IGrainManagementExtension>().DeactivateOnIdle()` for any other grain - both only schedule the deactivation for after the current turn, so let it apply before calling again), reactivates, and asserts the new HLC is strictly greater than the pre-deactivation one. |
...
| Sub-class | Detection signal | Proof shape |
|---|---|---|
| Read-modify-write across `await` | A grain method that reads `state.State.X`, awaits a non-trivial call, then writes back `state.State.X = newX` without re-reading or without using a CRDT-style merge. A second call can interleave at the `await` only where reentrancy is enabled: a `[Reentrant]` grain class (rare here - only the distributed-lock grain behind `ILatticeLockGrain`) or an `[AlwaysInterleave]` interface method, which is common on the core grain interfaces (the leaf, internal-node, shard-root and WAL-shard grains among them). `[StatelessWorker]` does not enable it: the grain behind `ILattice` is a stateless worker, and Orleans keeps each of its activations non-reentrant like any other grain's. | Integration test issuing two concurrent grain calls that both pass the read-then-await window, then asserting the second write doesn't clobber the first. |
...
| Persisted/in-memory divergence on write failure (idempotency-guarded, **high priority**) | A pattern like `state.State.X = newX; await state.WriteStateAsync();` where the in-memory mutation was applied *before* the persist call **and the same method (or another method on the same activation) short-circuits on the post-mutation value**. If `WriteStateAsync` throws, the in-memory state is dirty, the disk state is stale, and the guard short-circuits every retry from the same activation - turning a transient storage failure into a permanent split-brain until something else recycles the activation. Examples: `if (state.State.TreeId is not null) return;`, `if (state.State.IsDeleted) return;`, `if (state.State.PurgeComplete) throw...`. | Unit test using `FakePersistentState<T>` configured to throw on `WriteStateAsync`, calling the mutating method, catching the exception, then asserting `state.State.X` is the *pre-mutation* value. |
| Persisted/in-memory divergence on write failure (unguarded, **low priority**) | Same shape as the row above, but the method has no idempotency guard or guarded precondition - on retry it simply re-runs the mutation and the next `WriteStateAsync` reconciles in-memory with disk. The divergence window is bounded by "until the next successful call from the same caller (which typically retries) or until the activation is recycled". Fix for hygiene and to close the read-while-dirty window for non-reentrant siblings, but do **not** bundle into a high-priority cycle unless it sits in the same grain as a guarded site (where bundling avoids two near-identical reviews). | Same as the row above. |
...
| Timer used where reminder was needed | A grain timer (`this.RegisterGrainTimer(...)`, the Orleans timer API every timer in `src/` uses; the obsolete `RegisterTimer` has no call sites) for a job that must survive deactivation. Timers don't. | Integration test that schedules the timer, deactivates the grain, and asserts the job did not fire after reactivation - paired with a fix that uses `IGrainReminder` instead. |
...
Common fix patterns: persist *before* mutate (`var prev = state.State.X; state.State.X = newX; try { await state.WriteStateAsync(); } catch { state.State.X = prev; throw; }`), or use a snapshot-and-swap pattern; replace the grain timer (`RegisterGrainTimer`) with `this.RegisterOrUpdateReminder`; add `ArgumentException` guards in static key-parsing helpers and unit-test them directly.

**Orleans lifecycle note on the "divergence on write failure" sub-classes.** Orleans does **not** automatically deactivate a grain when a regular method (or its awaited `WriteStateAsync`) throws - with one exception: when an `InconsistentStateException` (an ETag conflict on `WriteStateAsync`) escapes a grain method and originated in that activation, the runtime deactivates the activation (Orleans 10.2.2), so the next call reloads state. Any other exception propagates to the caller through the grain reference proxy and the activation continues to serve subsequent calls with whatever in-memory state the throwing method assigned before the throw. Apart from that case, the only failure that auto-destroys an activation is one inside `OnActivateAsync`. The practical impact of these sub-classes is therefore bounded by "from the failed write until something else recycles the activation (idle timeout, explicit `DeactivateOnIdle`, silo shutdown, directory rebalance)". That window is non-trivial when (a) the method has an idempotency guard or guarded precondition that short-circuits retries on the dirty in-memory value (this is the high-priority shape above), (b) a concurrent in-flight reader on a non-reentrant grain observes `state.State.X` during the window, or (c) the caller retries promptly on the same proxy and the retry's outcome depends on the dirtied field. Pure unguarded paths self-heal on the next successful write, so prioritise the guarded shape first and bundle unguarded siblings only when they share a grain with a guarded site.
...
| Dead-letter not surfaced | An apply failure that's silently caught without a `DeadLetterEntry` write (look for `try { Apply } catch { return; }` that swallows the failure before it reaches the dead-letter-tracking decorator around `IReplicationApplier`, which parks the entry on the per-tree dead-letter queue after `LatticeReplicationOptions.MaxApplyRetries` consecutive failures; `ILatticeReplicationDeadLetters` is only the list / count / discard / replay seam over that queue and has no write method). | Integration test corrupting an inbound delta (e.g. invalid HLC) and asserting a dead-letter row is materialised. |
...
Common fix patterns: compute the WAL truncation watermark as `min(cursorPosition for cursor in registry)` and never advance past it; add a `HashSet<(OriginClusterId, Hlc)>` recently-applied cache or, better, persist the high-water-mark per origin and reject deltas at or below it; never construct a value on a write path without stamping the ambient `LatticeVectorClockContext.Current` frontier onto it (the context has no bump method: a replication apply scopes the source frontier with `LatticeVectorClockContext.With(...)`, and a purely local write leaves it `null` by design).
