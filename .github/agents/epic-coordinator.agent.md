...
5. **Sweep prior memory before planning anything.** Follow the memory retrieval protocol in `.github/instructions/repocontext.instructions.md` (Moment 1), covering the epic's own bus topic `epic-<number>` (a resumed epic may already have one) as well as the durable topics. Surface: decisions that already constrain this epic, gotchas a sub-agent will otherwise re-hit, and conventions the epic must honour. Fold what you find into the kickoff prompts in Phase 3 - that is cheaper than every sub-agent rediscovering it in its own worktree.
...
2. Create the epic integration branch off the current `main`: `<type>/epic/<epic-slug>` (the type is the epic's own - `feat/epic/...` for a feature epic, `docs/epic/...` for a documentation one - and the slug is kebab-case, derived from the epic title; never a username). All sub-work branches from here. The name **groups under an `epic` segment** rather than adding an `epic` prefix: the CI branch-name guard's regex already permits further path segments, so `<type>/epic/<epic-slug>` and the per-item `<type>/epic/<epic-slug>-<item-slug>` both pass unchanged, whereas a bare `epic/<epic-slug>` fails. **The per-item separator is a hyphen, not a slash, and git forces that.** A nested `<type>/epic/<epic-slug>/<item-slug>` is unimplementable beside an epic branch parked on the bare slug: git stores a branch as a file at `refs/heads/<name>`, so `refs/heads/X` and `refs/heads/X/anything` cannot coexist, and the conflict is symmetric - whichever is created first, the other is refused with `cannot lock ref ... exists`. Do not re-derive the nested form from the fact that the regex permits it; the regex is not what rejects it. **Do not put branch protection on the epic branch** - a required check with `strict` is exactly what serialises pull requests, and avoiding that is the whole point of the branch. CI still runs on epic-targeted pull requests because `.github/workflows/ci.yml` triggers on `*/epic/**`, which matches the epic branch as a pull-request **base** regardless of how the head branch is named. The full rules, including the drift-maintenance duty, are in `.github/copilot-instructions.md`.
...
4. **Seed the coordination bus** (principle 11). `repocontext_remember` one orientation entry under topic `epic-<number>`: the epic goal, the branch name, the DAG/integration order, and any constraint from the Phase 1 memory sweep that every sub-agent must honour. `kind: Decision`, `author: epic-coordinator`, no TTL. This is the entry a sub-agent reads to understand the epic without you re-explaining it in every kickoff prompt.
...
2. **Dispatch each ready sub-issue in parallel** as an **inspectable `feature-dev` child session** (principle 10), one per sub-issue, each in its own worktree branched off the current integration branch. Use `create_session`, **not** the background `task`/sub-agent mechanism and **not** a manual `git worktree add` - the session creates and owns its own worktree and branch:
   ...
   - "rename your branch once, at the start, with the `rename_branch` tool, to the epic's per-item form `<type>/epic/<epic-slug>-<item-slug>` - the app's generated session branch name can contain the operator's username, which the CI branch-name guard rejects. Note the final separator is a **hyphen**: a nested `<type>/epic/<epic-slug>/<item-slug>` cannot be created beside the epic branch at all, because git stores refs as files and `refs/heads/X` and `refs/heads/X/anything` are mutually exclusive. If a local rename is unavailable or already wrong, the local name does not have to block you - push an explicit refspec instead, `git push <remote> HEAD:refs/heads/<type>/epic/<epic-slug>-<item-slug>`, which is what the guard actually inspects";
   ...
   - "before you start, `repocontext_scan` scope `MemoryTopic` topic `epic-<number>` - that is the epic's coordination bus and carries the integrated seams, sibling gotchas, and constraints you must build on; post your own handoffs back to it (`author: <your session id>`, no TTL) as you land work";
...
5. **Add a row to the feature catalogue.** The feature table lives in `FEATURES.md`, not `README.md` (which no longer carries one): in the `FEATURES.md` category table the epic's capability belongs to (columns Feature | What it gives you | Docs | Sample), add one row for the epic's capability: a one-line "what it gives you" summary, a **Docs** link to the epic's primary doc under `docs/<package>/`, and a **Sample** link to the sample created in step 3. Use the byte-level markdown-editing technique.
6. **Add exactly one `CHANGELOG.md` entry for the epic.** Under `## Unreleased`, add a **single** user-facing entry (in the right subsection - `### Added`/`### Changed`/etc.) written to the **entry style** in [`docs/RELEASING.md`](../../docs/RELEASING.md) ("Updating `CHANGELOG.md`" -> "Entry style"), which is the single source of truth: a `**<Prefix> - <Short title>.**` heading plus prose, **300 characters or fewer in total**, describing the epic **at a high level** - the capability the whole epic delivers, phrased from the user's perspective - closing with the package list it ships in. Link **the epic issue only** (`#<epic>`). Do **not** add a line per sub-issue and do **not** link the sub-issues; the epic is the one changelog-visible unit of work, and its issue is where a reader goes for the detail. No version stamp, and no paragraph - a paragraph-length entry moves the epic issue's body into a file nobody maintains.
...
9. **Close the coordination bus: promote, then retire the rest deliberately** (principle 11). `repocontext_scan` scope `MemoryTopic` topic `epic-<number>` and read every entry the epic accumulated - yours and the sub-agents'. For each one that matters **beyond** this epic, re-`remember` it under its durable topic (`gotchas` / `conventions` / `decisions`) with **no TTL**, keeping the rationale and the `author`; link it to the code it describes (`addLinks`) so a later `recall` flags it stale when that file drifts. Then `forget` the purely operational handoffs ("T12 integrated at 2dbeecba") explicitly. They carry no TTL, so closing the bus is an act somebody performs rather than something that happens a week later to nobody's knowledge - which means the topic is empty because you emptied it, not because it quietly drained. Report which entries you promoted and which you retired. Skipping this step is how an epic's hard-won knowledge evaporates while the coordination chatter outlives it.
...
- **Does not let sub-agents run the non-chaos suite, integration-category tests, write docs, or open PRs.** Running the non-chaos suite and every integration-category test is reserved to the coordinator (at stages it deems appropriate); docs, changelog, the sample, the `FEATURES.md` feature-table entry, and the release plumbing (`.github/workflows/publish.yml` + `docs/RELEASING.md`) for any new package are coordinator-only; child-session PRs are forbidden.
