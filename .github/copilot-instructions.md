...
- **Commits carry no trailers.** Do not add author attribution, or
  ...
  any PR commit whose message carries one, and - see the next bullet - on any
  pull request whose commits carry more than one author identity. That step's
  `trailers` alternation mirrors this rule; change the two together.
- **`gh pr merge --squash` manufactures one of those trailers. No pre-merge
  check can catch the trailer, so the guard gates its cause instead.** When no
  explicit body is passed, GitHub
  composes the squash commit message itself and appends a `Co-authored-by:`
  line for every distinct author identity on the pull request, so a branch
  whose every commit is trailer-free still lands a trailer on the base. The
  guard's message arm is powerless here: the offending artefact does not exist
  until
  the merge happens, and the guard runs before it. The general form is worth
  holding onto, because it recurs well beyond trailers - **any artefact
  generated by an action cannot be validated by a check that runs before that
  action**, so the fix has to be at the action, never at the gate. Always pass
  `--subject` and `--body` explicitly:

  ```powershell
  gh pr merge <n> --squash --subject "fix(scope): summary (#<n>)" --body "..."
  ```
  ...
  - **What IS decidable before the merge is the cause, and it is now gated.**
    ...
    author identity**, so a pull request whose commits all carry one gives it
    nothing to attribute and no trailer is composed at all. A second arm of the
    same guard fails a pull request on that count, deduping on the author
    **email** (`%ae`) because that is the identity GitHub attributes on and the
    display name need not differ. Normalise the authorship rather than relying
    on remembering to strip the line by hand:
    `git rebase <base> --exec "git commit --amend --no-edit --reset-author"`,
    then force-push. Note this does **not** retire the explicit
    `--subject`/`--body` rule above, which is still the only thing standing
    between a single-identity pull request and whatever else GitHub decides to
    compose.
  - **The guard is an inline bash block, so it has no importable surface and
    its healthy outcome is silence.** `.github/workflows/trailer-guard-selftest.py`
    extracts it from `ci.yml` unmodified and drives all three arms against real
    temporary git repositories, with a mutation suite in which each mutation
    names the defect it reintroduces and the case that must change verdict.
    `CiTrailerGuardTests` holds both the guard's shape and that self-test's
    wiring in place, because a self-test nothing invokes reports the same
    nothing as a clean guard.
  - Twice now the explicit-body form was skipped and two author identities
    produced one manufactured trailer: on #2638, where repairing it cost a full
    branch rewrite, and again at the merge dialog for #3388, caught by eye
    seconds before confirming. The second occurrence is what moved this from
    advice to a gate - a rule whose only enforcement is a human noticing a
    prefilled textarea is not enforced.
