...
- **`gh pr merge --squash` manufactures one of those trailers, and no
  pre-merge check can catch it.** When no explicit body is passed, GitHub
  ...
  guard above is powerless here: the offending artefact does not exist until
  ...
  stays clean. The explicit-body form has held on every squash merge since it
  was adopted; the one time it was skipped, on #2638, two author identities
  produced one manufactured trailer and repairing it cost a full branch
  rewrite.
