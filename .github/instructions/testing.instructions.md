...
| `InstrumentLivenessCensusTests` | `test/lattice/` | a **report-only** census of which declared instruments have a test that witnesses them actually emitting. It never fails on a finding, so its enrolment value is the arm that asserts its declared-but-never-emitted partition still agrees with `InstrumentEmissionCoverageTests`: a divergence there means one of the two has silently stopped seeing an instrument |
