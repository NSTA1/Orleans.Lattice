...
| `LwwValue<T>` | Last-writer-wins register resolved by HLC. Carries optional `ExpiresAtTicks` (TTL) - `0` means no expiry; reads filter entries where `IsExpired(nowUtcTicks)` returns `true`. Carries optional `OriginClusterId` (cluster-of-origin stamp, authored from the ambient `LatticeOriginContext`) and optional `VectorClock` (sparse `{originClusterId -> HLC}` commit-time frontier, authored from the ambient `LatticeVectorClockContext`); both default to `null` for legacy persisted state. Also carries a migration marker (`[Id(6)]`), set when a cross-leaf migration import wins the merge and cleared by the next non-migration write; legacy persisted state decodes it to `false`. |
...
| `ICrdt<TSelf>` | Internal-feeling but `public` interface declaring `MergeFrom(TSelf)`, `IsBottom`, and `Clone()`. Implemented by every CRDT primitive in this folder - `BoundedRegister`, `GCounter`, `GSet`, `MvRegister`, `OrFlag`, `OrMap`, `OrSet`, `PnCounter`, `Rga`, `RwFlag`, `RwSet`, and `VersionVector` - and, outside it, by `LatticeReplicationConfigEntry` in `Orleans.Lattice.Replication`; the constraint that lets `OrMap<TKey, TValue>` recurse through nested CRDT values without reflection. Keep this the table's only `ICrdt<TSelf>` row. |
...
| `OrSetDelta` | Public typed replication delta DTO for `OrSet`: the element-tagged add dots and the observed remove dots since the receiver's cursor (both `OrSetDeltaDot`). Folded by `OrSet.MergeDelta(OrSetDelta)`; dispatched under `LatticeMergeMode.OrSet`. Lives under `Crdt/`. |
| `OrSetDeltaDot` | A single `(element, replicaId, counter)` dot inside an `OrSetDelta` or a `RwSetDelta`. |
| `PnCounterDelta` | Public typed replication delta DTO for `PnCounter`: the per-replica increment and decrement components `{replicaId -> long}` that advanced since the receiver's cursor, applied by pointwise-max. Folded by `PnCounter.MergeDelta(PnCounterDelta)`; dispatched under `LatticeMergeMode.PnCounter`. Lives under `Crdt/`. |
| `VersionVectorDelta` | Public typed replication delta DTO for `VersionVector`: the per-replica `HybridLogicalClock` entries that advanced since the receiver's cursor, applied by pointwise-max. Folded by `VersionVector.MergeDelta(VersionVectorDelta)`; dispatched under `LatticeMergeMode.VersionVector`. Lives under `Crdt/`. |
| `MvRegisterDelta` | Public typed replication delta DTO for `MvRegister`: the dot-tagged entries the producing write added (`MvRegisterEntry`) plus the producer's post-write dot context `{replicaId -> counter}`. Folded by `MvRegister.MergeDelta(MvRegisterDelta)`; dispatched under `LatticeMergeMode.MvRegister`. Lives under `Crdt/`. |
| `OrMapDelta<TKey, TValue>` | Public typed replication delta DTO for `OrMap<TKey, TValue>`: the dot-tagged value snapshots added (`OrMapDeltaEntry<TKey, TValue>`) and the observed-removed dots (`OrMapDeltaTombstone<TKey>`). Folded by `OrMap.MergeDelta(OrMapDelta<TKey, TValue>)`; dispatched under `LatticeMergeMode.OrMap`. Lives under `Crdt/`. |
| `OrMapDeltaEntry<TKey, TValue>` | A single `(key, replicaId, counter, value)` add inside an `OrMapDelta`, carrying the value-CRDT snapshot attached to that dot. |
| `OrMapDeltaTombstone<TKey>` | A single `(key, replicaId, counter)` observed-removed dot inside an `OrMapDelta`. |
| `LwwRegisterDelta` | Public typed replication delta DTO for a last-writer-wins value: the committed bytes, the commit-time `HybridLogicalClock`, the tombstone flag, the TTL expiry, and the origin cluster id; also the opaque-bytes fallback shape for a value that is not a recognised CRDT primitive. No primitive folds it, and the typed-CRDT delta seam refuses `LatticeMergeMode.LwwRegister`. Lives under `Crdt/`. |
| `StateDelta` | Captures entries changed since a given version vector |
| `SplitState` | Enum tracking leaf/internal split lifecycle |
| `LeafDeliveryCursor` | Activation-scoped `(Epoch, Sequence)` delivery cursor that a leaf's read-through cache uses to pull only the writes it has not yet observed, independent of the LWW HLC ordering. Not persisted: an epoch mismatch (or the `Empty` cursor) forces a full-projection delivery. |
