...
| Var | Default | Effect |
|-----|---------|--------|
| `BENCH_SATURATION_SAMPLE_MS` | `LatticeOptions.DefaultWalSaturationSampleInterval` (200) | WAL saturation sampler tick (ms). `0` disables the sampler (signal pins to Healthy; TCP-read gating becomes a no-op; the silo maps `0` to `Timeout.InfiniteTimeSpan`, the library's "disabled" value). On ACA pass it with `run-cohort-aca.ps1 -ExtraSiloEnv 'BENCH_SATURATION_SAMPLE_MS=0'`. |
