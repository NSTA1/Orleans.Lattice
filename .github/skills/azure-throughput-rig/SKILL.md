...
| `BENCH_WAL_SATURATION_ACUTE_ONLY` | `1` (rig opts in; library default `LatticeOptions.DefaultWalSaturationAcuteOnly` is off) | Sets `WalSaturationAcuteOnly` (#3348): an admission semaphore at its cap classifies Throttled instead of Saturated, and a gate-parked append resumes once its partition leaves Saturated. `0` measures the library default. `run-cohort-aca.ps1 -WalSaturationAcuteOnly 0|1`. |
