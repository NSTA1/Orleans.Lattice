...
#### U9g-pre falsification (2026-05-25T10:18Z-T10:35Z) - ramping `BENCH_FLUSH_CONCURRENCY` ∈ {8, 16, 24} on the U9f baseline. Higher flush concurrency makes throughput *worse*, not better. Producer-side fan-out is **not** the binding constraint; the binding constraint is now identified as serialised `LatticeGrain.SetManyAsync` (FALSIFIED that more flush concurrency helps; CONFIRMED the per-tree `LatticeGrain` activation is the real ceiling)
...
**Failure mode (smoking gun).** The FC=16 silo log contains the diagnostic message that pins the bottleneck to the per-tree `LatticeGrain` activation, not `ShardRootGrain`. A single `Orleans.Lattice.ILattice.SetManyAsync` request on activation `lattice/azure-throughput-20260525-102907@6417c29d…` was reported as `IsExecuting: True`, `NumRunning=1`, `NonReentrancyQueueSize=0`, `CurrentlyExecuting=…SetManyAsync…`, with `was enqueued 00:00:25.388 ago and has now been executing for 00:00:25.388`. The producer-side `TcpIngestService.FlushAsync` cancellation token then fires (`Silo/Program.cs` line 666), the batch is logged as failed, and the next FC slot tries to enqueue another `SetManyAsync` against the same single activation. At FC=16 this means *two* in-flight + queued `SetManyAsync` calls per activation; at FC=24 *three*. The activation is a `StatelessWorkerPlacement` `LatticeGrain` (not a partitioned shard-root), so the producer's per-batch routing collapses every flush down to the same logical activation: more flush concurrency does not give us more parallelism, it just lengthens the queue ahead of an already-saturated turn.

**Re-attributed binding constraint.** The U9f working baseline's serial turn is *not* on `ShardRootGrain.SetManyAsync` (per-shard, 16 of them) - it is **upstream** of the shard fan-out, on `LatticeGrain.SetManyAsync` itself. The U9f working point at FC=8 is the largest flush concurrency that keeps one in-flight `LatticeGrain.SetManyAsync` call near its turn-cost floor; FC>8 over-pressures that one turn and starts timing out batches at the producer.

**Re-ranked probe order (sixth revision; U9g's premise needs an upstream pivot).**

- **U9 / U9b / U9c / U9d / U9e / U9f** - rolled up above; U9f remains the current shipping baseline.
- **U9g-pre - FALSIFIES the "raise flush concurrency to unblock per-shard parallelism" hypothesis.** The producer-side fan-out is already at the ceiling of the upstream serial turn on the per-tree `LatticeGrain` activation, so `ShardRootGrain` parallelism is not yet exercised. Any U9g code change that targets `ShardRootGrain.SetManyAsync` re-entrancy will buy *zero* throughput until the `LatticeGrain.SetManyAsync` turn is widened, because the producer cannot get more than one `SetManyAsync` request into the lattice grain at a time.
- **U9g (revised, the next throughput probe) - make `LatticeGrain.SetManyAsync` reentrant or partition the upstream ingress.** Two viable shapes: (a) annotate `ILattice.SetManyAsync` with `[AlwaysInterleave]` if the implementation only mutates per-shard state that is already protected by `ShardRootGrain`'s single-activation invariant (cheapest fix, but must audit the lattice grain's own state for read/write hazards); or (b) introduce a per-shard `IShardLatticeGrain` projection so the producer can fan out across `shardCount` activations of the *lattice* type the same way it already does for shard-root. Shape (a) is the smaller change and is the right first probe; shape (b) is the fallback if (a) finds unsafe shared state on `LatticeGrain`.
- **U9h (still deferred; promoted only after U9g revised lands) - reentrant `ShardRootGrain.SetManyAsync`.** This is the *original* U9g probe, now correctly ordered as a *follow-on* to widening the upstream lattice-grain ingress. Until `LatticeGrain.SetManyAsync` itself can interleave, `ShardRootGrain` reentrancy is unreachable from the producer.
- **U9i (deferred) - investigate the benign cold-start reshard rejections** (the former U9h hygiene item; renumbered).
