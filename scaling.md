...
- **NEW NEXT (c2-viii unblocked Phase D) - Phase D parallel saga fan-out (`AtomicWriteGrain.D1`).** The c2-viii tuned atomic probe (`vehicles=500, batchSize=256, atomicBatchSize=64`) ran 101 s of productive window with `FinalFailed=0` and pinned per-key atomic-saga cost at ~57 ms (derived from `lattice.op.duration_ms` p50 = 3,600 ms / 64 keys/saga). Phase D's deferral condition (Anomaly 3: atomic-write bench variance at short runs) is now met. Phase D's D1 step (parallel per-key fan-out inside `AtomicWriteGrain` under a `Task.WhenAll` bounded by `WalPartitions`) directly attacks that 57 ms/key figure; structural upper bound on the lift is ~4-8x given the `FlushConcurrency=8` cap on the saga's WAL append + commit-mark path.
