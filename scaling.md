...
- **U9j - the next probe.** Rung ladder at the proven-safe FC=8 through `5000:5` and `10000:5` to break the "4 active shards out of 16" ceiling that bounds the `1000:5` rung at ~2 369/s (4 active shards * 1 000 ms / 25 ms phase-1 RT * ~16 entries-per-batch). FC=8 is the new working set after U9h-D; raising FC further requires resolving the ingest-side Orleans response-timeout fail mode that U9h-D surfaced, which is a separate probe (provisionally U9k, scoped after U9j gives a clean rung-scaling curve).
