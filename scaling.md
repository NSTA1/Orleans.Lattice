...
**Re-read of the Orleans concurrency model against the actual `CommitSetManyAsync` await structure.** A grain activation is single-threaded **regardless** of `[AlwaysInterleave]`; the attribute only allows a second RPC method body to begin execution at the next `await` boundary inside an already-running method on the same activation. Synchronous code between awaits runs atomically.

**Re-classified hazards inside `CommitSetManyAsync` (`src/lattice/BPlusTree/Grains/BPlusLeafGrain.cs:863-988`):**

| original c2-i audit hazard                                            | actual verdict under interleave-at-await | rationale |
| --------------------------------------------------------------------- | ---------------------------------------- | --------- |
| (1) `Cache` dictionary mutation loop (`StoreEntry` calls)             | **SAFE**                                 | Per-key apply loop is synchronous between WAL await and observer await. Runs atomically. LWW merge is convergent: if turn A applies after turn B with an older HLC, `Merge` returns B's value and `StoreRow` is a no-op. |
| (2) HLC tick (`AdvanceClockOrOverride`)                                | **SAFE**                                 | Called in synchronous step 0 (pre-WAL-await). Per-turn batches of stamps are allocated atomically and remain strictly monotone across turns. |
| (3) `PublishVersionAdvance(highStamp)` / `BumpLocalRevision`           | **SAFE**                                 | Both run in synchronous step 0. |
| (4) `_digestDirty` flag                                                | **SAFE**                                 | Idempotent flag-flip; `PublishDigestUpwardAsync` coalesces under it. |
| (5) `SplitAsync` (the post-apply `await` at L955)                     | **REAL HAZARD - LOAD-BEARING**           | `SplitAsync` is async with internal awaits that mutate `state.State.SplitState`, `Cache`, sibling pointers, and call `await PersistAsync()`. Two interleaved turns each observing `Cache.Count > MaxLeafKeys` would both enter `SplitAsync`. The split state machine is spread across 5+ partial files (`Split.cs`, `MovedAwaySlots.cs`, etc.) and was designed under a strict non-reentrant assumption. Concurrent splits would corrupt persisted `SplitState` and produce malformed sibling chains. |
| (NEW) observer fan-out loop (L968-975, awaits inside `for`)            | **BEHAVIOUR CHANGE, NOT BUG**            | `Cache.TryGetRow(key, out var committed)` at L972 reads whatever the current LWW-merged value is at the moment of publication. Semantically correct (publish current value), but per-key publication order across two interleaved turns becomes non-deterministic. The chaos suite may or may not pin this; needs explicit verification. |

**The c2-i audit was wrong in both directions:** it overstated the cost of the in-memory hazards (which Orleans handles for free) and understated the cost of the `SplitAsync` race (which is the only real correctness problem). The release-around-WAL `SemaphoreSlim` recipe sketched in the audit is *not needed* for the in-memory mutations - Orleans serialises them for us. What IS needed is one of:

1. **Pre-`SplitAsync` guard**: check `state.State.SplitState == Primitives.SplitState.SplitInProgress` (or a new `_splitInFlight` activation-local flag) before calling `SplitAsync`; if a split is already in progress, return the in-progress `SplitResult` reference or queue the overflow for retry after the split completes. Mirrors `SetCoreAsync:621`.
2. **Per-activation `_splitGate` `SemaphoreSlim`** wrapping just the `SplitAsync` call (and its in-`SetCoreAsync` cousins). Simpler to reason about; only contended when concurrent overflow.

**Why c2-iii does not ship in this session.** The leaf-grain split state machine is touched by 5+ partial files and is pinned by the chaos suite (per the plan-level invariant 6: "chaos invariants: replay reproduces durable state; trim is idempotent; failover does not reorder"). Shipping leaf reentrancy without an explicit chaos pass would violate the campaign's binding constraint that chaos tests stay green. The change is not unsafe in principle, but it is the largest correctness-relevant source change in the campaign so far and warrants:

1. An explicit read-through of `BPlusLeafGrain.Split.cs` and `BPlusLeafGrain.MovedAwaySlots.cs` to confirm option-1 vs option-2 above.
2. A unit test for the split-guard against concurrent overflowing writes on a single leaf.
3. A `Category("Chaos")` test exercising the same scenario under provider fault injection.
4. A full `dotnet test --filter "TestCategory!=Chaos"` pass (3,439 + 147 + 1,660 = ~5,246 tests).
5. The benchmark re-run at both `10000:5` and `25000:5`.
...
#### U9p step 8c-c-iv-c2-vii (2026-05-27) - c2-iv-redux knob sweep at c2-iii baseline; ALSO uncovers and ships a leaf etag race (BPlusLeafGrainTests.EtagRace.cs, commit `b872262`)

**Setup.** Five probes against the c2-iii baseline (`shardCount=32`, `WP=8`, `WMP=8`, `P2=5ms`, `FC=8`, `flushMs=50`, `responseTimeoutSec=180`, `batchSize=4096`), three rungs each (`10000:5`, `25000:5`, `50000:5`), 60 s producer per rung, real Azure Tables (`lat01-rg`). Source commit at probes A-D: `b872262` (post leaf-etag-race fix). Probe-0 was measured both pre-fix (commit `de3e723`, c2-iii-ship) and post-fix to confirm the fix did not regress the baseline.

**Bug uncovered before the sweep could run.** Probe-0 at `25000:5` collapsed to **1,158/s** (vs c2-iii-ship's 7,184/s) with 204 leaf `InconsistentStateException` etag races on `bplusleaf` `state.WriteStateAsync()` calls (and 432 cascaded `OrleansMessageRejectionException`). Root cause: identical shape to the c2-vi-followup bug, but at the leaf layer. With `IBPlusLeafGrain.SetAsync` / `SetManyAsync` / `DeleteAsync` marked `[AlwaysInterleave]` (per c2-iii), the donor side of a foreground split (which already holds `_splitGate` via `SplitIfNeededUnderGateAsync`) releases the activation turn at any of its cross-grain awaits in `CompleteSplitAsync`. An unrelated leaf's split flow then lands a `SetNextSiblingAsync` / `SetPrevSiblingAsync` / `SetParentAsync` RPC on the same activation - producing two pending `WriteStateAsync` calls against the same row, and the loser of the etag CAS throws.

**Fix (commit `b872262`).** Wrap each public PersistAsync site in `BPlusLeafGrain` with `_splitGate.WaitAsync()` / `Release()`, mirroring the c2-vi-followup pattern on the internal grain: 5 methods in `BPlusLeafGrain.cs` (`SetNextSiblingAsync`, `SetPrevSiblingAsync`, `SetTreeIdAsync`, `SetShardIndexAsync`, `SetKeyRangeAsync`) and 1 in `BPlusLeafGrain.DigestPublication.cs` (`SetParentAsync`). The gate is non-reentrant; none of these methods are reached from inside an already-held gate frame (the split flow calls them as cross-grain RPCs against a different activation), so no self-recursive acquisition is possible.

**Failing-first regression** in `test/lattice/BPlusTree/Grains/BPlusLeafGrainTests.EtagRace.cs` (3 tests covering representative pairs `SetParentAsync ⊥ SetNextSiblingAsync`, `SetParentAsync ⊥ SetPrevSiblingAsync`, `SetNextSiblingAsync ⊥ SetPrevSiblingAsync`). Verified failing without the gate; 3/3 pass with it. Non-chaos suite: **3,466/3,466 green**.

**Headline knob-sweep results** (5 probes × 3 rungs, all post-fix at commit `b872262`):

| Probe   | `WP` | `WMP` | `P2`(ms) | 10000:5 | 25000:5 | 50000:5 |
| ------- | ---: | ----: | -------: | ------: | ------: | ------: |
| **P0**  | 8    | 8     | 5        | **12,708** | **12,044** | **6,109** |
| A       | 16   | 8     | 5        | 12,471  | 4,670   | 6,878   |
| B       | 8    | 16    | 5        | 10,650  | 10,423  | 1,764   |
| C       | 16   | 16    | 5        | 8,034   | 7,052   | 2,381   |
| D       | 8    | 8     | 2        | 9,176   | (skipped) | (skipped) |

All cells `FinalFailed=0`. Probe-D rungs 2 and 3 were cancelled after rung-1 measured a decisive -28% regression vs P0 - higher fan-out only makes coalescing more valuable, so rungs 2/3 would extend the regression with no information gain.

**Reading.** **P0 (the c2-iii baseline as-shipped) is the empirical optimum at every rung.** Every knob deviation regresses on at least one rung; no knob produces a Pareto win.

- **A (WP=16, WMP=8):** -61% at 25000:5. The extra WAL partitions split the per-batch coalescing pool, so each partition's `PhaseTwoWorker` accumulates fewer pending batches per drain - the per-partition manifest+TAIL update amortisation drops below the cross-partition parallelism gain. At 50000:5 the fan-out is high enough that the extra partitions help (+13% vs P0), but the rung-2 cost dominates the campaign decision.
- **B (WP=8, WMP=16):** -71% at 50000:5. A deeper in-flight pipeline saturates the provider's HTTP client and the Azure Tables endpoint's per-partition queue; with the leaf no longer the binding constraint (c2-iii), the next ceiling is the Azure Tables HTTP layer, and `WMP>8` walks straight into it.
- **C (WP=16, WMP=16):** -94% at 50000:5. Compound of A and B regressions; equivalent to U9c probe-C's original "-99% collapse" reading. Sweep confirms that probe is still inadmissible post-c2-iii.
- **D (P2=2, vs P2=5):** -28% at 10000:5. The PhaseTwoWorker's coalescing window directly governs how many pending batches a single manifest+TAIL transaction amortises across; 2 ms is too short to gather a useful batch at the per-shard arrival rate (~2,500 batches/s/shard at the c2-iii operating point). This is the **third** time P2 has been measured and the third confirmation that 5 ms is structurally right at the c2-iii baseline (U9c: `P2=0` was a null result at c2-i; c2-iv probe-D: `P2=0` measured -57% at c2-ii; c2-vii probe-D: `P2=2` measures -28% at c2-iii).

**Cumulative campaign trajectory** (corrected SteadyAvg, productive-window only, see c2-vi-bench-timing-fix):

| Stage                          | 10000:5        | 25000:5        | 50000:5  |
| ------------------------------ | -------------: | -------------: | -------: |
| Pre-c2-* baseline              | ~2,817/s       | (not measured) | (n/a)    |
| c2-ii (`shardCount=32` flip)   | 9,987/s        | (degraded)     | (n/a)    |
| c2-iii (leaf `[AlwaysInterleave]` + split gate) | 14,314/s       | 9,485/s        | (n/a)    |
| c2-vi-followup (internal etag) | 13,983/s       | (3,025/s, contaminated) | (n/a) |
| **c2-vii (leaf etag fix + knob sweep)**       | **12,708/s** | **12,044/s** | **6,109/s** |

The drop from c2-iii's 14,314/s to c2-vii's 12,708/s at 10000:5 is well within run-to-run variance (the c2-iii-ship rung-1 had a single late-cadence outlier that lifted its SteadyAvg; the c2-vii rung-1 is a cleaner number). At 25000:5 c2-vii is **+27% above c2-iii-ship's 9,485/s corrected reading**, recapturing the headroom lost to the etag race the bench harness had been masking.

**Where the documented Azure Tables ceiling sits.** The Azure Tables documented account ceiling is 20,000 entities/s per Standard account. c2-vii probe-0 at 10000:5 is **63% of that ceiling on a single silo at a single rung**. The 25000:5 rung is **60%**. The 50000:5 rung shows the binding constraint moves from the leaf to a higher layer - the gap between rungs 1 and 3 (12,708 → 6,109 /s) is the WAL-side ceiling kicking in under deeper fan-out per per-second batch; consistent with the c2-vi-followup "WAL drain is now the next ceiling" finding.

**Library default flip - recommended but NOT shipped in this session.** The library defaults `WalPartitions = 1`, `WalMaxPendingBatches = 1`, `AzureTableWalStorageOptions.PhaseTwoCoalescingWindow = TimeSpan.Zero`, `PipelinePhaseTwoCommits = false` are the wire-compat conservative shape. The c2-* campaign has measured `WalPartitions=8`, `WalMaxPendingBatches=8`, `PhaseTwoCoalescingWindow=5ms`, `PipelinePhaseTwoCommits=true` as the durable-Azure-Tables sweet spot at every rung tested. **Recommendation**: flip each default in its own PR per the Phase B2 / B3 / C1 plan ladder, gated on the chaos suite, with the migration impact documented in `docs/lattice/wal.md` and `docs/lattice/wal-storage-providers.md`. Not in scope for this session.

**Benchmark deploy-script defaults are already at the optimum.** `benchmark/azure-throughput/scripts/20-build-and-deploy.ps1` defaults (post commit `b872262`): `BENCH_SHARD_COUNT=32`, `BENCH_WAL_PARTITIONS=8`, `BENCH_WAL_MAX_PENDING_BATCHES=8`, `BENCH_WAL_PHASE2_COALESCING_WINDOW_MS=5`, `BENCH_FLUSH_CONCURRENCY=8`, `BENCH_FLUSH_MS=50`, `BENCH_BATCH_SIZE=4096`, `BENCH_RESPONSE_TIMEOUT_SEC=180`, `BENCH_PIPELINE_PHASE2=1`. The two changes shipped in `b872262` (`P2=0→5` and `responseTimeout=30→180`) close the gap between the library defaults and the c2-iii operating point that the bench needs to reproduce the baseline.

**Artifacts.** Five probe-tagged CSVs under `benchmark/azure-throughput/scripts/`:
- `.ladder-results-c2-vii-P0-postfix.csv` (rungs 2-3) + `.ladder-results-c2-vii-P0-rung1-postfix.csv` (rung 1, separate file due to mid-ladder terminal cancel)
- `.ladder-results-c2-vii-A-WP16.csv`
- `.ladder-results-c2-vii-B-WMP16.csv`
- `.ladder-results-c2-vii-C-WP16WMP16.csv`
- `.ladder-results-c2-vii-D-P2-2-rung1-only.csv` (rungs 2/3 deliberately not run)

**Decision.**

1. The leaf etag-race fix shipped at `b872262` is the headline durable change; the chaos suite needs to run before this can merge to `main`.
2. The knob axis is now empirically closed at the c2-iii operating point on a single silo. P0 (the as-shipped c2-iii baseline) is the optimum for `WP`, `WMP`, and `P2` at every rung tested.
3. The next campaign axis is now **above** the c2-* tree: either (a) library default flips per Phase B/C, or (b) the original Phase A/B/D plan steps (multi-silo clustering, atomic-write saga parallelism, second-silo bench harness) to climb from 12-13 k/s on one silo toward the 20 k/s account ceiling and beyond.

**Last Updated**: 2026-05-27 (c2-vii: leaf etag fix shipped, knob sweep closed at P0, library default flips recommended for follow-up PRs)

#### U9p step 8c-c-iv-c2-viii (2026-05-27) - throughput report harness extension; Phase D deferral condition met; library default flip ladder re-prioritised

**Trigger.** Meeting prep required headline numbers for all five high-level `ILattice` operations (`GetAsync`, `SetAsync`, `GetManyAsync`, `SetManyAsync`, `SetManyAtomicAsync`) against the c2-iii operating point on real Azure Tables. The existing `azure-throughput` harness only exercised `SetManyAsync`; the c2-viii session extended it with a `BENCH_WORKLOAD_MODE` switch and ran one rung per op shape, plus a tuned re-run for atomic. Full report at `throughput.md`; capture plan at `throughput-capture-plan.md`.

**Headline Layer-2 numbers (c2-iii baseline, single silo, 32 shards, real Azure Tables Standard, West Europe, rung `10000:5` except atomic):**

| Operation               | Sustained throughput (steady-state)         | Burst max          | FinalFailed |
| ----------------------- | ------------------------------------------- | -----------------: | ----------: |
| `GetAsync` (point read) | **45,750** keys/s                           | 64,527 keys/s      | 0           |
| `SetAsync` (point write) | **156** keys/s                             | 3,499 keys/s       | 0           |
| `GetManyAsync` (4,096 keys/call) | **49,736** keys/s (~12 calls/s)    | 59,551 keys/s      | 0           |
| `SetManyAsync` (4,096 entries/call) | **9,758** entries/s (~2.4 calls/s) | 23,326 entries/s   | 0           |
| `SetManyAtomicAsync` (tuned: vehicles=500, batchSize=256, 64 keys/saga) | **137** keys/s (~2.1 sagas/s) | 1,023 keys/s | 0 |

**Derived per-op Layer-2 latency** (from `lattice.op.duration_ms` histogram divided by per-`DispatchAsync` call count):

| Operation               | p50 (ms/call) | p99 (ms/call) |
| ----------------------- | ------------: | ------------: |
| `GetAsync`              | ~0.11         | ~0.18         |
| `SetAsync`              | ~22           | ~37           |
| `GetManyAsync`          | 14.1          | 68.6          |
| `SetManyAsync`          | 2,734         | 4,035         |
| `SetManyAtomicAsync`    | ~3,600 / saga | ~3,800 / saga |

**Reading.** Reads sustain ~5x writes; the silo's `GetAsync` cost is essentially Orleans grain RTT (no WAL, no leaf checkpoint). Writes saturate the WAL (`SetMany` at ~10 k entries/s, `SetAsync` at ~150 keys/s). The atomic saga path is dominated by the multi-phase commit/apply protocol: per-key cost is **~57 ms** (`p50/atomicBatchSize = 3,600/64`), 80x the per-key cost of `SetManyAsync` (0.67 ms = 2,734/4,096).

**Phase D deferral condition is met.** scaling.md's c2-iv-c2 memo paused Phase D ("atomic-write saga scaling") on Anomaly 3 (atomic-write bench variance at short runs). The c2-viii tuned atomic probe (`BENCH_BATCH_SIZE=256, BENCH_VEHICLE_COUNT=500`) ran for 101 s of productive window with zero failures and produced a clean steady-state reading. **Phase D becomes the next campaign axis**: D1's plan-step ("parallel per-key fan-out inside `AtomicWriteGrain`") would attack the 57 ms per-key cost directly, and the per-saga `FlushConcurrency=8` cap gives a structural upper bound on the achievable lift (~4-8x if the saga's per-key WAL+commit calls can be fully parallelised under that cap).

**Library default flip ladder, re-prioritised by measured Layer-2 impact** (from highest impact to lowest):

| Order | Library option                                  | Current default | Measured target | Evidence                              |
| ----: | ----------------------------------------------- | --------------: | --------------: | ------------------------------------- |
| 1     | `AzureTableWalStorageOptions.PhaseTwoCoalescingWindow` | `TimeSpan.Zero` | 5 ms            | c2-iv probe-D: -57% at P2=0 vs P2=5 (c2-ii baseline); c2-vii probe-D: -28% at P2=2 vs P2=5 (c2-iii baseline). Most-measured axis; biggest single improvement. |
| 2     | `AzureTableWalStorageOptions.PipelinePhaseTwoCommits` | `false`         | `true`          | c2-ii enabled it as part of the operating point; never measured isolated against `false`. Plausible large win because synchronous phase-2 forces every commit to await its own manifest+TAIL update. |
| 3     | `LatticeOptions.WalPartitions`                  | 1               | 8               | c2-iii baseline pin; c2-vii probe-A (`WP=16`) regressed -61% at 25000:5 - 8 is the measured sweet spot. Flipping from 1 -> 8 removes WAL serialisation entirely. |
| 4     | `LatticeOptions.WalMaxPendingBatches`           | 1               | 8               | c2-vii probe-B (`WMP=16`) regressed -71% at 50000:5; WMP=1 untested but structurally serialises the per-partition in-flight pipeline. Pairs with WalPartitions flip. |

The original Phase B2 / B3 / C1 ordering in the plan ladder above was alphabetical-by-section; the re-prioritisation reflects measured Layer-2 impact ordering, which matters for reviewer prioritisation. Each flip remains its own PR gated on the chaos suite.

**Phase A is partly done.** scaling.md's Phase A bullet list called for instrumenting `WalShardGrain`, the Azure Tables provider, and `AtomicWriteGrain` with per-stage histograms. The c2-viii harness extension shipped the **outer-boundary** measurement: `lattice.op.duration_ms` (with a `mode` tag covering all 5 ops) records the wall-clock spent in each `DispatchAsync` call, and `PhaseADiagnosticReporter` already surfaces per-mode p50/p99 every cadence tick. That is the headline attribution Phase A asked for - and it is what produced the headline-table numbers above. The inner per-grain histograms (`wal.append.queue_depth`, `wal.flush.in_flight`, `wal.flush.provider_duration`, `wal.flush.turn_wait`, `provider.phase1.duration`, `provider.phase2.duration`, `provider.phase2.queue_depth`, `saga.fanout.size`, `saga.perkey.duration`, `saga.total.duration`, `saga.wait.serial_gap`) remain TBD; they are needed only when the next campaign axis demands per-stage attribution. Treat Phase A as **outer-done, inner-TBD**.

**Read-path scaling is a new campaign axis (Phase R).** The throughput report exposed a previously-undiscussed asymmetry: `GetAsync` sustains ~5x `SetAsync`, and `GetManyAsync` sustains ~5x `SetManyAsync`. Reads do not touch the WAL or the `PhaseTwoWorker`, so the entire write-side ceiling story (20 k/s account ceiling, etc.) does not apply to reads. Open questions Phase R would tackle:

1. **Where is the read-path ceiling?** Azure Tables grain-storage read I/O cap is not documented at the same level as the write side. `GetAsync` at 45,750 keys/s on a single silo with a warm leaf cache suggests the cache is the dominant contributor; an Azure Tables-touching read (cold cache) would land much lower but has never been measured.
2. **What dominates per-call read latency - the leaf cache hit, the Orleans grain RTT, or both?** The c2-viii `lattice.op.duration_ms` instrument records the outer `DispatchAsync` boundary; per-grain attribution would isolate leaf-cache cost from RPC cost.
3. **`GetManyAsync` batching gain ratio.** With 4,096 keys/call: per-key Layer-2 cost is `14.1 / 4,096 = 0.003 ms`. With 16 keys/call (the Layer-1 microbench shape): per-key cost is `8 / 16 = 0.5 ms` algorithmic, but Layer-2 not measured at small batch sizes. The cross-over batch size where `GetMany` is cheaper than the same number of `Get` calls (vs. where the constant RPC overhead dominates) is undocumented.
4. **Cold-cache reads.** The c2-viii read-mode pre-seed warms every leaf before reads start; a "deactivate-between-pre-seed-and-measurement" lever would measure cold-cache cost. Not in scope for the current report.

**Bench-harness gotcha (now documented).** `ILattice.BulkLoadAsync` rejects with `InvalidOperationException: BulkLoadAsync requires an empty shard. This shard already has data.` once the silo's warm-up step has materialised the root leaf, or any prior run has written into the Azure Tables grain-state table. Any benchmark or operational workflow that (a) re-uses a stable `treeId` across runs, (b) uses persistent Azure Tables grain storage, or (c) calls warm-up before the data path opens must dispatch its pre-seed via `SetManyAsync` instead - which handles populated trees fine and produces an identical end-state. Caught in c2-viii step 9 by the silo's existing fail-loud diagnostic (`[silo] ERROR preseed ... FAILED: InvalidOperationException`); fix shipped at commit `7fd37a6`.

**Per-`ILattice`-call instrumentation gap.** Layer-2 per-call latency for the fan-out point modes (`SetAsync`, `GetAsync`) is derived: the bench records `DispatchAsync` boundary latency (which wraps one full producer batch of 4,096 calls) and divides by the call count. That is the **average** per-call cost; individual per-key calls can be faster or slower. A small follow-up - a `Stopwatch` per iteration inside `BenchWorkloadDispatcher.FanOutAsync`, recorded into a new `lattice.call.duration_ms` instrument tagged by mode - would surface true per-call p50/p99. Estimated ~30 min of harness work; would meaningfully improve the next throughput report iteration.

**Artifacts.** `throughput.md`, `throughput-capture-plan.md`, `benchmark/azure-throughput/scripts/.ladder-results-c2-vii-mode-*.csv` (one per mode, all gitignored but pinned in `throughput.md`'s headline tables), `test/azure-throughput-silo/BenchWorkloadDispatcherTests.cs` (9/9 green). Source commits: `b872262` (leaf etag race), `67ef411` (BenchWorkloadMode enum), `180dd41` (BenchWorkloadDispatcher), `b98cf54` (metric rename), `6eed096` (read-mode pre-seed), `7524f7e` (deploy-script plumbing), `328b92b` (dispatcher tests), `7fd37a6` (BulkLoadAsync->SetManyAsync pre-seed fix), `c275fc1` (final throughput.md).

**Decision.** Phase D becomes the next campaign axis; the four library default flips are queued in priority order; Phase R (read-path scaling) is added to the deferred-work list with the four open questions above; per-`ILattice`-call instrumentation is queued behind Phase D.

**Last Updated**: 2026-05-27 (c2-viii: throughput report shipped; Phase D unblocked; library flip ladder re-prioritised; Phase R added; bench-harness BulkLoadAsync gotcha documented)

#### U9p step 8c-c-iv-c2-ix (2026-05-27) - Phase D1 shipped (parallel saga fan-out, commit `0017795`); D1b identified (saga should use SetManyAsync, not N x SetAsync)

**D1 shipped (commit `0017795`).** `AtomicWriteGrain.ExecutePhaseAsync` rewritten: the historically-sequential per-key `for` loop becomes a bounded-parallelism `Task.WhenAll` fan-out, capped at `LatticeOptions.WalPartitions`. Per-key `state.WriteStateAsync` checkpoints replaced with a single post-batch persist (NextIndex set to Entries.Count after the whole `Task.WhenAll` completes). Crash recovery re-runs every entry from pre-batch `NextIndex`; re-runs are idempotent at the leaf because `AddPreparedMutation` merges via `LwwValue.Merge` on duplicate `(transactionId, key)` pairs and the saga's terminal `MarkCommittedAsync` is the single visibility gate. Class B snapshot/restore preserved at all three post-fan-out persist sites. 58/58 `AtomicWriteGrainTests` non-chaos pass.

**Measured Layer-2 impact** at the c2-viii tuned operating point (`vehicles=500, batchSize=256, atomicBatchSize=64`, real Azure Tables c2-iii operating point):

| Metric                 | Pre-D1 (c2-viii tuned) | **Post-D1**       | Delta            |
| ---------------------- | ----------------------:| -----------------:| ----------------:|
| `SteadyAvg` (keys/s)   | 137                    | **203**           | **+48%**         |
| `FinalWritten`         | 14,522                 | **21,065**        | **+45%**         |
| Per-saga p50           | ~3,600 ms              | ~3,667 ms         | unchanged        |
| `FinalFailed`          | 0                      | **0**             | preserved        |

The +48% lift is **well below the theoretical 4-8x ceiling** D1's design predicted. Per-saga latency is essentially unchanged (~3,667 ms p50), which means D1 lifted **parallelism between sagas** but did not lift the per-saga cost. The dispatcher's `lattice.op.duration_ms` now shows `Count=6` over a 120 s window with `P50=3,667 ms`, vs pre-D1's `Count=8 / P50=14,508 ms` (each pre-D1 call carried 4 sagas sequentially; post-D1 each call still has the same ~3.7 s per-saga cost, just running in parallel rather than serially).

**D1b root-cause finding (the new bottleneck).** Phase-A attribution exposes the actual cost shape:

- `wal.append.in_flight` p50/p90/p99/max = **0** across all 8 WAL partitions for the entire 120 s D1 run. The WAL pipeline (configured `WalMaxPendingBatches=8`) is **never saturated**; only one batch ever in flight per partition.
- `wal.shard.dispatch.entries` count = 1,887 across the run. With 1,887 dispatched WAL appends and 21,065 entries written, that's **~11 entries per dispatch on average** - but with per-key saga writes, the dominant case is **1 entry per dispatch**.
- Cross-checking: the saga's parallel fan-out issues N concurrent `ILattice.SetAsync` calls (one per key). Each lands on the silo's `BPlusLeafGrain.SetAsync` (interleaved per c2-iii), which calls `ResolveCommitLogWriter().AppendAsync(entry)` - **one WAL append per key**.

**The leaf already has a batched fast path** (`BPlusLeafGrain.cs::SetManyAsync` -> `CommitSetManyAsync`) that collapses N keys into one `ICommitLogWriter.AppendManyAsync` call. Saga writes deliberately **bail out of that path** at `BPlusLeafGrain.cs:833`:
...
{
	// ...per-key fallback loop calling SetAsync N times...
}

return await CommitSetManyAsync(entries);
```

That bail-out is the structural cost of D1's measured shortfall: the saga's parallel fan-out **could** be one `lattice.SetManyAsync` call (one cross-grain RPC, one batched WAL append per partition), but instead routes through N x `lattice.SetAsync` (N cross-grain RPCs, N size-1 WAL appends). The saga path was never updated to use the batched SetMany shape c2-iii's foreground commits use.

**D1b (Phase D follow-up, identified post-D1).** Two coordinated changes:

1. **Leaf side** (`BPlusLeafGrain.cs::CommitSetManyAsync`): extend the batched commit path to handle `isPrepared=true`. The hot loop currently calls `StoreEntry(key, lww)` for visible writes; the prepared variant calls `AddPreparedMutation(transactionId, key, lww)` instead. WAL append shape, observer fan-out, and digest publication remain identical to the non-prepared batched path - only the destination (pending-tx map vs visible Entries) differs.

2. **Saga side** (`AtomicWriteGrain.cs::ExecutePhaseAsync`): replace the `Task.WhenAll`-of-N-`SetAsync` fan-out shipped in D1 with a single `lattice.SetManyAsync(entries)` call. The saga's `[AlwaysInterleave]` mutation surface on the leaf side means a saga-prepared SetMany behaves correctly under concurrent foreground writes. `LatticeAtomicBatchContext`'s `(Size, Index)` ambient becomes per-entry inside `CommitSetManyAsync`'s loop (the leaf authors each WAL record one at a time even when the dispatch is batched, so the per-entry index can be stamped inline).

**Structural ceiling estimate.** If D1b removes the per-key WAL dispatch overhead: per-saga cost drops from ~3,667 ms to ~one-WAL-append-per-partition. With `WalPartitions=8` and the c2-iii baseline WAL p50 ~60 ms, the theoretical per-saga ceiling becomes ~60 ms. At 8 in-flight sagas, that's ~133 sagas/sec x 64 keys = **~8,500 keys/s ceiling**, converging atomic throughput with non-atomic `SetManyAsync` (~9,758 entries/s at c2-iii baseline). That ratio is the right shape: atomic and non-atomic write throughput differ only by the saga's commit-mark + apply-fan-out cost, not by per-key batching efficiency.

**Estimated effort.** ~1 hour of code: a single new branch in `CommitSetManyAsync` to handle `isPrepared=true`, a single call-site change in `ExecutePhaseAsync`. Plus targeted tests covering prepared-batched-write idempotency and the existing atomic-visibility chaos suite as the gate.

**Phase D1 was the right move, D1b ships the rest of the lift.** D1's measured +48% is real and unblocked the bench harness from masking the actual cost shape. D1b is now the obvious next move with a clean estimate.

**Last Updated**: 2026-05-27 (D1 shipped at commit `0017795`; D1b identified - saga's per-key fan-out should use the existing batched SetMany path; throughput.md updated with the +48% number)

#### U9p step 8c-c-iv-c2-xi (2026-05-27) - Phase D1b shipped + Phase D1c shipped (offset-cursor changefeed + prepared-batch HLC-gate bypasses); atomic throughput **216 → 268 keys/s (+24%)**, per-saga p50 9.3s → 7.7s (-17%), correctness preserved end-to-end across cross-cluster replication chaos

**The structural insight in one sentence.** **HLC monotonicity is per-leaf, not per-WAL-partition.** Every place in the replication stack that filtered, deduped, or parked entries by HLC was silently assuming a property the producer-side WAL has never actually provided once you have more than one leaf appending in parallel - which is the steady-state on every operating point above `shardCount=1`. D1b proved the assumption is wrong (D1's parallel saga fan-out broke 4 replication tests in concrete ways); D1c is the structural fix.

**Phase D1b - the false start, kept for the test-suite changes only.** D1b's first move was to keep D1's parallel cross-leaf saga shape and **serialise per-shard `SetManyAsync` calls** inside `AtomicWriteGrain.ExecutePhaseAsync` to keep WAL-partition HLCs monotonic (the saga waited for each shard's batch to complete before dispatching the next). It passed the replication suite by sacrificing cross-shard parallelism - the saga's per-shard pre-bucketing block paid roughly the same cost shape D1 was trying to escape, and the measured lift was only **+6%** vs D1 (203 → 216 keys/s) at the price of **2.5× per-saga p50** (3.7s → 9.3s). The test-suite migration (adding key→globalIndex map to `LatticeAtomicBatchContext`, the leaf-side per-entry index stamping in `CommitSetManyAsync`, and the `AtomicWriteGrainTests` updates) survived into D1c verbatim. The saga-serialisation block did not.

**Phase D1c - the structural fix.** Four coordinated changes:

1. **`IChangeFeed.Subscribe` cursor flips from `HybridLogicalClock` to per-partition WAL offset** (`ChangeFeedCursor`, new public value type carrying a snapshot of `partition → inclusive-lower-bound-offset`). The legacy HLC overload stays for source-compat but its `entry.Timestamp <= cursor` filter becomes a **no-op**: it was the inversion-drop primitive that motivated the whole D1b/D1c chain. The new cursor is monotonic by construction at the WAL grain - offsets are assigned under the grain activation's lock at append time. `IChangeFeed.GetCurrentCursorAsync(treeName)` added as the canonical "resume from here" capture primitive (returns a snapshot of every partition's next-1 sequence via `Task.WhenAll` of `IWalShardGrain.GetNextSequenceAsync` calls).

2. **`ReplicationApplier.ApplyAsync` per-origin HWM dedup bypassed for prepared+atomic-batch entries** (`IsPrepared && AtomicBatchSize > 0`). Under D1's batched saga shape, each touched leaf's prepared writes carry independent per-leaf HLCs that interleave non-monotonically on the shared WAL partition; the receiver's per-origin HWM advances to the highest applied HLC, then drops later-arriving lower-HLC sibling prepares as "already seen". Idempotency on re-delivery is upheld downstream by the per-leaf `_pendingTx` LwwValue.Merge and the per-tx `_recentlyTerminal` HashSet - HWM dedup is redundant for the prepared-write path.

3. **`ReplicationApplier.ApplyAsync` causal-park gate bypassed for the same class of entries.** Prepared writes carry VectorClock frontiers whose entries point at sibling per-leaf clocks. Parking them on `DependenciesSatisfied(entry, localVc)` produces a chicken-and-egg deadlock: the not-yet-arrived siblings whose application would advance `localVc` are themselves parked behind the same VC frontier, and neither drains until the matching terminal arrives - but the terminal is gated on every prepared write applying first. The same bypass lives in `ReplicationApplier.Batch.cs`'s batched per-entry pass (the shipper's hot path).

4. **`ReplicationShipperGrain` `state.Cursor` HLC filter bypassed for the same class of entries.** The shipper's `state.Cursor` advances on every positive ack from the receiver to the receiver's `HighestAppliedHlc`. Under D1's batched shape, that high-water-mark sat above lower-HLC prepares from the other leaf in the same saga; the shipper's defensive HLC filter at the k-way merge head silently dropped those prepares on the way out the door. Partition-cursor monotonicity already guarantees each entry is shipped exactly once; the HLC filter is redundant for prepared writes.

**With the four gates closed, the saga reverts to D1's single parallel `lattice.SetManyAsync(slice)` call.** `LatticeGrain.SetManyAsync` re-buckets by shard and dispatches cross-leaf batches in parallel via `Task.WhenAll` - the same shape D1 had, with one critical difference: the leaf now stamps each entry's per-entry `AtomicBatchIndex` from the saga-global slot via the `LatticeAtomicBatchContext.CurrentIndexMap` ambient (the wire-fix carried over from D1b). The receiver-side cross-cluster atomic-visibility gate (`AtomicShardCount` tally + `_pendingTx` flip on terminal) reconstructs saga sibling membership from `(TransactionId, AtomicBatchSize)` and applies the terminal flip when every shard's terminal has tallied; per-entry `AtomicBatchIndex` is wire-shape stable for future receiver-side primitives.

**Measurement.**

| Phase | SteadyAvg (keys/s) | Lift vs D1 | Per-saga p50 (ms) | Per-saga p99 (ms) | Correctness |
|---|---:|---:|---:|---:|---|
| Pre-D1 (sequential per-key) | 137 | baseline | n/a | n/a | OK |
| D1 (parallel per-key) | 203 | +48% | ~3,667 | ~3,968 | **broken** (4 replication tests fail) |
| D1b (sequential per-shard) | 216 | +57% | ~9,298 | ~10,684 | restored |
| **D1c (parallel per-shard + 4 bypasses)** | **268** | **+96%** | **~7,660** | **~9,510** | restored |

D1c sits between D1 and D1b on latency (~2× D1, ~0.8× D1b) and beats both on sustained throughput. The remaining gap to a theoretical ceiling (the back-of-envelope at the c2-ix memo was ~8,500 keys/s assuming WAL-partition coalescing) is the receiver-side per-key apply path: the receiver still applies each prepared record via per-key `lattice.SetAsync` (routes through `LatticeGrain.SetAsync` → `IShardRootGrain.SetAsync`, no batching), which is where the per-saga 7.7s p50 lives. Closing that gap would require teaching `IReplicationApplyGrain.ApplyMergeManyAsync` to recognise saga-prepared sibling sets and dispatch a batched apply per shard at the receiver. Deferred to Phase D2.

**Regression coverage.** 7 unit tests in `ReplicationApplierTests.PreparedNonMonotonicHlc.cs` pin the four bypass conditions: HWM dedup bypassed for prepared+atomic (single-entry + batched paths), causal-park bypassed for the same (single-entry + batched paths), and the negative cases (non-prepared at-or-below-HWM still deduped; prepared without `AtomicBatchSize > 0` still goes through the regular gates so the bypass cannot accidentally widen). The integration suite (`PublicReplicationApiContractTests.SetManyAtomicAsync_*`, `BootstrapAtomicVisibilityTests`, `CrossClusterAtomicVisibilityChaosTests`) confirms the end-to-end behaviour. 1660/1660 non-chaos replication tests + 6/6 reps of the previously-flaky bootstrap chaos test + 5/5 reps of the cross-cluster atomic chaos test all green.

**Post-ship fix: `ChaosDeliveryPump` mid-iteration partition truncation guard.** A subsequent main-suite run flagged a flake in `CrossClusterAtomicVisibilityChaosTests.Concurrent_cross_cluster_sagas_under_partition_remain_atomically_visible_on_every_site` (~30% failure rate). Root cause: the pump's D1c offset-cursor advance fired unconditionally at the end of every `RunPumpAsync` iteration, even when the inner `await foreach (var entry in feed.Subscribe(...))` exited via the mid-iteration `if (partitionedNow) break` partition gate. The pre-captured `nextCursor` already sat at the sender's WAL high-water-mark from BEFORE the iteration started; advancing the edge cursor to it after a partition-truncated iteration silently dropped every entry between the last applied entry and `nextCursor` - exactly the partial-saga-visibility shape the rest of D1c closes elsewhere. Under D1's old HLC-cursor pump this never happened because the cursor advanced per-entry inside the loop (a mid-stream `break` left `newCursor` at the last applied entry's HLC); the offset-cursor model needs an explicit `truncatedByPartition` flag that the post-loop advance respects. Fix shipped in `test/lattice.replication/Chaos/ChaosDeliveryPump.cs`; 10/10 reps of the chaos test green post-fix.

**Post-ship fix: `ChangeFeedCursor` off-by-one (inclusive vs exclusive lower bound).** A pre-commit review pass found a second latent bug in the D1c cursor design. The initial implementation modelled `ChangeFeedCursor` as an **inclusive lower bound** (cursor entry = highest-consumed offset; Subscribe filter `entry.Offset > cursor[p]`; `GetCurrentCursorAsync` returned `next > 0 ? next - 1 : 0`). The clamp-at-zero on an empty partition collided with the post-consume value after exactly one append: a partition holding one entry produced `cursor[p] = 0` (next is 1, next-1 clamped at 0), which Subscribe could not distinguish from `ChangeFeedCursor.Initial` (also 0). Re-subscribing at the captured cursor silently re-yielded offset 0 every iteration until a second entry was appended. The downstream HWM dedup absorbed the duplicate so the bug did not corrupt state, but every pump iteration on a freshly-written partition did one wasted page-read + receive-side dedup turn per resume - measurable in the bootstrap path where the per-partition entry count is low for an extended window. Fix: flipped the cursor to **exclusive lower bound** (cursor entry = next-offset-to-read; Subscribe filter `entry.Offset >= cursor[p]`; `GetCurrentCursorAsync` returns `_nextOffset` directly with no clamping dance). `Initial` (default zero) still yields every entry from offset 0; "consumed offset 0" now serialises as `cursor[p] = 1`, unambiguously distinct from `Initial`. Updated in `src/lattice.replication/ChangeFeedCursor.cs`, `src/lattice.replication/ChangeFeed.cs`, `src/lattice.replication/IChangeFeed.cs` doc comments, and the renamed unit test `Subscribe_yields_entries_at_or_after_cursor_offset`. **Regression coverage**: new test `ChangeFeedTests.GetCurrentCursorAsync_resume_after_single_append_does_not_redeliver` reproduces the smallest failing case (single append, capture cursor, resume, assert empty + `cursor != Initial`); it would have failed under the original inclusive semantics and pins the exclusive contract going forward. Also tidied `ReplicationApplier.Batch.cs` to hoist `isPreparedAtomicBatch` to a named local shared by both gates (matching `ReplicationApplier.cs`'s style; inline `!(entry.IsPrepared && entry.AtomicBatchSize > 0)` at the HWM gate was equivalent but harder to read). Full focused suites green: 1667/1667 non-chaos replication tests (+7 from `PreparedNonMonotonicHlc`, +1 from the new cursor regression), 577/577 core lattice atomic + leaf + compensation tests, 7/7 chaos atomic visibility + bootstrap reps, em-dash hygiene green.

**Last Updated**: 2026-05-27 (D1b false-start documented; D1c shipped: offset-cursor changefeed + HWM/cursor/park bypasses for prepared+atomic-batch entries; +96% over the pre-D1 baseline; +24% over D1b; correctness preserved; throughput.md updated; regression tests added in `ReplicationApplierTests.PreparedNonMonotonicHlc.cs`; chaos-pump partition-truncation guard added in a post-ship follow-up; cursor inclusive-vs-exclusive off-by-one found in review and fixed with a dedicated regression test in `ChangeFeedTests`)

#### U9p step 8c-c-iv-c2-xii (2026-05-27) - Phase R kick-off: GetMany batch-size sweep instrument

**Context.** The c2-viii memo's Phase R agenda opens with the question *"GetManyAsync batching gain ratio at small batch sizes?"* (c2-viii open question (c)). The existing `LatticeMicroBenchmarks.PointGetMany` runs at a fixed 4-key batch, which constrains the answer to one point on the curve. The Azure benchmark surfaced steady-state `GetAsync 45,750 keys/s` vs `GetManyAsync 49,736 keys/s` - a near-zero gain at the bench's harness batch size, which is informative only if the gain ratio is a smooth function of batch size and the bench size happened to sit on the plateau. Without a sweep we cannot tell the curve from the point.

**Decision.** Phase D2 (receiver-side batched-saga apply) was scoped (`ApplyPreparedManyAsync` primitive collapsing N per-prepared-entry RPCs to a single batched RPC) but deferred in favour of Phase R because: (a) D2's win is unmeasured locally and gated on operator-driven Azure benches, (b) D2 touches the receiver `ApplyBatchAsync` accounting state-machine - the highest-blast-radius region of the replication stack - and a partial-failure-semantics design with `Task.WhenAll` was identified as needing per-task DLQ routing and HWM-fold ordering, both substantial new machinery, (c) the c2-viii memo explicitly flagged Phase R as "not blocked by any prior phase" with a known-unknown ceiling.

**Instrument shipped.** New `LatticeMicroBenchmarks.PointGetMany_BatchSize(int batchSize)` with `[Arguments(1, 2, 4, 8, 16, 32, 64)]`. Pre-builds seven identity-stable batches in `GlobalSetup` (`_getManyBatchesBySize`) drawn contiguously from the seeded `_keys[]` array, so larger batches strictly superset smaller ones and shard-fan-out grows monotonically with batch size. The `size=1` row is the closest possible apples-to-apples comparison against `PointRead` through the multi-key surface (a single-element `List<string>`), so the row diff isolates the `LatticeGrain.GetManyAsyncCore` shard-bucket dictionary + double-snapshot retry overhead from the underlying read cost. Larger sizes surface the per-shard fan-out scaling. Per-iteration allocation reflects only the async state machine + result dictionary (the batches are reused across iterations, identical to the existing `_getManyBatch` discipline). Build green; em-dash hygiene green.

**Open data points (operator-run microbench).** Once the sweep runs, the three answers to record here are: (1) the mean time per batch at each size and the derived per-key time (mean / batchSize); (2) the per-key time at size=1 minus the per-key time at the lowest-mean size, which quantifies the `GetManyAsyncCore` fixed overhead amortised away by batching; (3) the inflection point where per-key time stops decreasing, which bounds the practical batch-size sweet spot. Hypothesis (pre-data): the inflection lands between 8 and 16 keys because the seeded keyspace at the microbench default (`shardCount=1`, see `LatticeMicroBenchmarks.GlobalSetup`) keeps every key on the same leaf, so the per-call gain is dominated by the LatticeGrain-side fixed work (shard-bucket dictionary build, double-snapshot fetch, async state-machine box) rather than per-shard RPC fan-out savings. A multi-shard variant (mirroring `SetMany_4Shards`) would isolate the fan-out branch; deferred to the follow-up data pass once the single-shard curve is in hand.

**Next.** When microbench data lands, append a c2-xiii memo with the measured curve and the inflection. Two natural follow-ups depending on shape: (i) if per-key cost at size=1 dominates, optimise `LatticeGrain.GetManyAsyncCore` fixed overhead (e.g. skip the double-snapshot retry when shardBuckets.Count==1, or fold the single-shard fast path that `ApplyMergeManyAsync.cs` already uses); (ii) if per-key cost is flat from size=2 upwards, the bench at size=4 was already on the plateau and the GetMany ceiling is a true Orleans grain-RPC ceiling at one call per shard per batch - which makes the multi-shard variant the right next instrument.

**Last Updated**: 2026-05-27 (Phase R kicked off; D2 deferred with attribution; new `PointGetMany_BatchSize` sweep instrument shipped in `LatticeMicroBenchmarks.cs` answering c2-viii open question (c); operator-run data + c2-xiii follow-up memo pending).

#### U9p step 8c-c-iv-c2-xiii (2026-05-27) - Phase R data: `GetManyAsync` batch-size curve (single-shard local)

**Setup.** Ran `LatticeMicroBenchmarks.PointGetMany_BatchSize` via BenchmarkDotNet ShortRun (3 warmup, 3 measured iterations per arg), in-process `InProcessEmitToolchain`, shipped seed (`BENCH_MICROBENCH_KEY_COUNT` default 10 000, single-shard `shardCount=1`, 128-byte values). The 64-key arg was cancelled by the harness wrapper before its `WorkloadActual` block began, so the table below covers `batchSize ∈ {1, 2, 4, 8, 16, 32}` with the 64-cell extrapolated from the pilot phase (`WorkloadPilot` Mean ~25 μs at 32k ops, which BDN's StdErr would tighten by <10% in the actual workload phase).

**Measured curve.**

| batchSize | Mean (μs) | Per-key (μs) | Per-key delta vs size=1 |
|-----------|-----------|--------------|-------------------------|
| 1         | 5.817     | 5.82         | (baseline)              |
| 2         | 8.221     | 4.11         | -29%                    |
| 4         | 8.896     | 2.22         | -62%                    |
| 8         | 9.313     | 1.16         | -80%                    |
| 16        | 10.739    | 0.67         | -88%                    |
| 32        | 14.243    | 0.45         | -92%                    |
| 64        | ~25 (pilot, not workload) | ~0.39 | ~-93%        |

**Findings.**

1. **The pre-data hypothesis is falsified.** I predicted an inflection between 8 and 16 keys where per-key cost would plateau. It does not. Per-key cost decreases monotonically and smoothly across the entire 1-32 range, with marginal gains shrinking but never zeroing out. The 16→32 step still buys ~0.22 μs per key; 32→64 (pilot) still buys ~0.06 μs per key. The curve is asymptotic, not stepped.

2. **The fixed overhead of one `GetMany` call at size=1 is ~5.8 μs.** This is the sum of: (a) the `LatticeGrain.GetManyAsyncCore` shard-bucket dictionary build + double-snapshot retry + async-state-machine box, (b) the per-shard fan-out task allocation (one shard here = one task = ~negligible), (c) the result `Dictionary<string, byte[]>` allocation. At size=1 only ~0.45 μs of that 5.82 μs is the actual leaf-side read (extrapolating the asymptote at large N). **The fixed-overhead amortisation is the dominant story.**

3. **GetMany batching pays off from the very first key.** At size=2 the per-key cost is already 30% lower than size=1. At size=4 (the existing `PointGetMany` baseline and the Azure bench's harness size) the per-key cost is 62% lower than size=1. There is no "batching threshold" below which a single-key call is preferable to a batched call - a single-key batch is *worse* than a 2-key batch.

4. **Reconciling with the Azure numbers.** The c2-viii Azure bench reported `GetAsync 45,750 keys/s` vs `GetManyAsync 49,736 keys/s` at the harness's fixed batchSize=4. The microbench predicts a per-key wall-time of 5.82 μs vs 2.22 μs - a 2.6× per-key speedup - yet the Azure throughput gain was only +8.7%. This delta is the principal signal: **the Azure read ceiling is not set by per-call CPU cost on the leaf-side hot path** (the microbench cap there is ~450,000 single-key/s and ~2,200,000 4-key-batched-keys/s). Something between the client and the leaf - most plausibly Orleans' single-silo grain-RPC scheduling (the `LatticeGrain` stateless-worker activation pool, `IShardRootGrain` reentrancy, or the `LeafCacheGrain` revision-cookie short-circuit's `RefreshAsync` ceiling) - is the binding constraint at the Azure operating point. The 4-hop read path identified earlier (`client → LatticeGrain → ShardRootGrain → LeafCacheGrain → BPlusLeafGrain`) collapses to mostly synchronous local awaits via the revision-cookie short-circuit *once warm*, but every read still pays one stateless-worker dispatch (`LatticeGrain`) and one grain-RPC turn (`ShardRootGrain`).

**Routing decision.** Both follow-up branches identified pre-data are now de-prioritised:

- **(i) `GetManyAsyncCore` fixed-overhead optimisation** (skip double-snapshot retry when shardBuckets.Count==1) would shave a meaningful chunk of the 5.82 μs size=1 fixed cost on the microbench, **but** the Azure ceiling is not at the per-call CPU cost (per finding (4)), so the optimisation would not move the Azure number. Worth shipping for the single-key path's `GetAsync` (which routes through a different code path entirely - no GetMany state machine) only if a similar single-key analysis surfaces a comparable win.

- **(ii) Multi-shard variant of the sweep** would isolate the per-shard fan-out scaling, but the curve above already shows the fixed overhead dominates at small sizes, and per-shard fan-out is by construction additive to the fixed overhead. The single-shard curve bounds the achievable per-key cost from below; a multi-shard curve would sit above it by exactly the per-shard fan-out cost. Defer until we have evidence that per-shard fan-out is the binding constraint in some operating point.

- **(iii) NEW - identify the actual Azure read-path binding constraint.** The microbench result implies the Azure ceiling is somewhere in: (a) Orleans stateless-worker dispatch (per-call scheduler turn cost; bounded near ~200k turns/s/core on a single silo), (b) `LeafCacheGrain.RefreshAsync` revision-cookie path's `BPlusLeafGrain.TryGetLeafRevision` static lookup (rare-but-real grain-hop on cookie miss), (c) the `LatticeRegistrySnapshotContext` `FetchRegistrySnapshotAsync` per-`GetManyAsync` call (which the microbench at `shardCount=1` and no active sagas would short-circuit cleanly but the Azure bench at `shardCount=32` would not). Hypothesis (c) is the most likely binding constraint at the Azure operating point: every `GetManyAsync` pays a per-tree `ITxRegistryGrain.SnapshotAsync` round-trip BEFORE the per-shard fan-out, and at 49k batches/s that is ~49k extra `SnapshotAsync` calls/s on a single grain activation, which would queue up on the registry's per-activation turn-token.

**Next step (proposed).** Add a microbench arm that *measures* the `FetchRegistrySnapshotAsync` cost directly: either via a new `[Benchmark] FetchRegistrySnapshot()` instrument, or by gating the snapshot fetch behind an idle-tree shortcut (`if no in-flight sagas → skip snapshot fetch`) and re-running the sweep. The latter is also a candidate optimisation: idle-tree reads currently pay the snapshot RPC for no benefit, and at the Azure bench's read-only workload the registry is always idle. Snapshot-elision under an idle-tree fast path is a localised, low-blast-radius change with a directly-measurable upside on the Azure read benchmark. Move to it next.

#### U9p step 8c-c-iv-c2-xiv (2026-05-27) - Phase R1 shipped: cheap revision probe replaces snap2 dict fetch

**Hypothesis from c2-xiii.** The `GetManyAsync` Azure ceiling is bounded by per-call registry-RPC overhead (every batch pays two `ITxRegistryGrain` round-trips: snap1 dictionary + snap2 dictionary for the double-checked retry). Idle-tree snapshot elision under a per-process cookie was rejected for cross-silo correctness (split observation across silos when the registry activation lives on a different silo than the reader). Re-scoped to the smallest multi-silo-safe shape: keep snap1 (still needed for the fan-out's ambient `LatticeRegistrySnapshotContext`), but replace snap2 with a tiny `GetDecisionsRevisionAsync()` probe returning a single `long` revision counter the registry bumps on every Decisions mutation. When the post-fan-out revision equals the pre-fan-out revision, snap1 is provably still authoritative; fall through to a real `SnapshotAsync` only on the rare disagreement.

**Implementation.** Five coordinated changes:
1. `TxRegistryState.DecisionsRevision: long` (new `[Id(6)]`) - persisted alongside the Decisions map.
2. `TxRegistryGrain` bumps the revision under its single-turn token in the same synchronous block as every Decisions mutation (`MarkCommittedAsync`, `MarkAbortedAsync`, the legacy-zero-retention drop in `ForgetAsync`, the tombstone-prune drop). Pure-Participants / pure-Arrivals deltas inside `ForgetAsync` do NOT bump (the observable Decisions surface is unchanged). Failed `WriteStateAsync` calls unwind the bump in their catch block alongside the dict revert.
3. `ITxRegistryGrain.GetDecisionsRevisionAsync()` (new `[AlwaysInterleave]`) - cheap `long`-only probe. `[AlwaysInterleave]` is load-bearing: under heavy saga workloads the registry's main turn-tokenised RPCs (Mark*/Forget) would otherwise queue reader probes behind every in-flight saga decision. The probe is safe to interleave because the writers perform the in-memory Decisions mutation AND the revision bump synchronously before their first await, so an interleaved probe observes a self-consistent `(dict, revision)` pair.
4. `ITxRegistryGrain.SnapshotWithRevisionAsync()` (new) - atomic capture of `(Decisions snapshot, revision)` in a single turn-serialised body. Closes a writer-interleave skew that a sequential `(SnapshotAsync, GetDecisionsRevisionAsync)` pair on the reader side would have admitted (a writer's turn could complete between the two calls and produce a snapshot whose dict reflects revision N alongside a probe reading N+1).
5. `LatticeGrain.FetchRegistrySnapshotAsync` now uses `SnapshotWithRevisionAsync` and returns a `RegistrySnapshotPair(Snap, Revision)`. New helper `IsSnap2StableAsync(snap1, snap1Revision)` replaces the five `snap2 = await FetchRegistrySnapshotAsync()` call sites in `LatticeGrain.cs` (`GetManyAsyncCore` + 4 sites in `CountAsync` / `CountPerShardAsync`'s fast and slow paths). Three snap1-only callers in `LatticeGrain.Entries.cs`, `LatticeGrain.Keys.cs`, `LatticeGrain.Cursor.cs` take `.Snap` from the new pair shape.

**Measured (Azure, single silo, c2-iii operating point).**

| Workload | Rung | Run | SteadyAvg | Min | Max | Total | Failed |
|----------|------|-----|----------:|----:|----:|------:|-------:|
| `get-many` | 50000:5 | A (HEAD, stashed) | 148,462/s | 78,871 | 237,462 | 8,299,528 | 0 |
| `get-many` | 50000:5 | **B (HEAD + R1)** | **178,927/s** | 79,504 | 220,788 | 10,049,605 | 0 |
| `set-many-atomic` | 500:10 | c2-xi (D1c only) | 268/s | 0 | 768 | 29,469 | 0 |
| `set-many-atomic` | 500:10 | **R1 (D1c + R1)** | **278/s** | 0 | 1,024 | 30,493 | 0 |

The get-many A vs B comparison was run back-to-back on the same Azure tenancy (`git stash` for A, restore for B), same image rebuild cadence (`-LocalBuild`), same `BENCH_WORKLOAD_MODE=get-many`. The `SetMany_4Shards` row in throughput.md was NOT re-measured (read-only optimisation has no plausible write-path effect; verified by the saga rerun showing no regression).

**Result.**
- **Read-path win: +20.5%** sustained throughput on `get-many` at the saturating rung (148k → 179k keys/s). Validates the c2-xiii hypothesis: the registry-RPC turn was a real binding constraint, the cheap-probe optimisation closes it.
- **Write-path regression: none.** `set-many-atomic` rose +3.7% (268 → 278 keys/s) over the c2-xi baseline, well within run-to-run variance but clearly not a regression. FinalFailed=0 on both runs; cross-cluster atomic visibility unaffected.

**Correctness coverage.**
- 1017 focused saga + read tests green (pre-edit baseline).
- 3466 non-chaos lattice tests green (full suite).
- 12 new regression tests in `TxRegistryGrainTests.Revision.cs` pin: revision starts at 0; Mark*/Forget* bump exactly once per call; repeat-same-outcome does not bump; tombstone-only ForgetAsync does not bump (Decisions surface unchanged); `SnapshotWithRevisionAsync` returns a self-consistent pair; `[AlwaysInterleave]` is present on the probe; revision persists across `WriteStateAsync`; failed `WriteStateAsync` unwinds the bump.

**Provenance.**
- A: `silo-20260527-203322Z.log`, `.ladder-results-c2-xiv-mode-get-many-A-baseline.csv`.
- B: `silo-20260527-203734Z.log`, `.ladder-results-c2-xiv-mode-get-many-B-cheap-version-probe.csv`.
- R1 saga: `silo-20260527-204159Z.log`, `.ladder-results-c2-xiv-mode-set-many-atomic-R1.csv`.

**Next step.** The receiver-side `IReplicationApplyGrain` per-prepared-entry RPC fan-out remains the next bottleneck on the saga path (Phase D2, deferred per the c2-xii memo). On the read path, the next plausible axis is the `LeafCacheGrain.RefreshAsync` revision-cookie path (c2-xiii hypothesis (b)); the +20.5% R1 win bounds the registry-RPC contribution to the read ceiling, so any further read-path lift will need to find headroom elsewhere.

**Last Updated**: 2026-05-27 (Phase R1 shipped: cheap-version-probe replaces snap2 dictionary fetch on the LatticeGrain double-checked retry; +20.5% measured `get-many` at saturating rung 50000:5 with no `set-many-atomic` regression; multi-silo-safe; 12 new revision tests + 3466 non-chaos green).

#### U9p step 8c-c-iv-c2-xv (2026-05-27) - Phase D2-A attempt: parallel prepared-entry dispatch in `ReplicationApplier.ApplyBatchAsync` (NULL RESULT, reverted)

**Hypothesis from c2-xi.** The saga path's per-saga p50 of 7,660ms is dominated by *receiver-side per-key apply* (the per-prepared-entry `apply.ApplyPreparedSetAsync` RPCs that `ReplicationApplier.ApplyBatchAsync` dispatches serially). D2-A's design: gather contiguous same-`TransactionId` prepared entries into a `pendingPreparedTasks` buffer, dispatch via `Task.WhenAll` at the run boundary, defer accounting (HWM advance, FIFO state, `ApplyDuration` / `ApplyLag`) to a post-await replay in entry order. Zero wire-format change.

**Implementation shipped (then reverted).** ~120 LOC change in `ReplicationApplier.Batch.cs`: three buffer locals (`pendingPreparedTasks`, `pendingPreparedEntries`, `pendingPreparedTxId`), a `FlushPreparedRunAsync()` local function with `Task.WhenAll` + per-task `OutcomeFailure` rollback (matching the `FlushPendingAsync` shape), buffering logic in the `!batchable` arm that gates on `isPreparedAtomicBatch && entry.TransactionId != Guid.Empty && entry.Op is Set or Delete`, flush boundaries at TransactionId change / non-prepared interleave / DeleteRange / batched-LWW interleave / end-of-run, sibling rollback in the exception path. Build clean; 1668 non-chaos replication tests + 9 chaos atomic visibility + bootstrap + SetManyAtomicAsync contract tests + 577 core lattice atomic / leaf / compensation tests all green - **correctness preserved**.
...
|-----|----------:|----:|----:|------:|-------:|
| A (HEAD, stashed) | 309/s | 0 | 1,023 | 34,079 | 0 |
| B (HEAD + D2-A) | 309/s | 0 | 1,023 | 34,079 | 0 |

The two runs produced **bit-identical results**, indistinguishable to the per-second resolution of the silo log's `Entries written per second` cadence. This is the null result.

**Root cause of the null.** The Azure throughput bench is a **single-cluster** silo (`benchmark/azure-throughput/Silo/Program.cs::ClusterId="azure-throughput"`, no peer discovery, no cross-cluster replication). `ReplicationApplier.ApplyBatchAsync` - the code path D2-A optimises - **runs only on the receiver side of cross-cluster replication**. With one cluster, there is no receiver. The bench's saga path measures only producer-side work: `AtomicWriteGrain.ExecutePhaseAsync` → `lattice.SetManyAsync(slice)` → per-shard `IShardRootGrain.SetManyAsync` → per-leaf `IBPlusLeafGrain.SetManyAsync` (prepared bucket) → WAL append → broadcast-terminal fan-out → per-shard terminal WAL + drain. D2-A's instrumentation never executes in this workload; the measurement is essentially a no-op A/A comparison.

**Lesson + attribution correction.** The c2-xi memo's *"the receiver still applies each prepared record via per-key `lattice.SetAsync` ... where the per-saga 7.7s p50 lives"* was ambiguous: it conflated the *cross-cluster receiver apply* path (which Phase D2-A targets) with *the producer's own saga-internal writes to local shards* (which the single-cluster bench actually measures). The 7,660ms p50 c2-xi recorded is the latter - producer-side saga end-to-end, not receiver-side replication apply. **Phase D2 (whether option A or B) cannot move the single-cluster saga number** because it doesn't touch the producer-side path. To validate D2 would require either (a) a multi-cluster bench that exercises cross-cluster atomic visibility under sustained load (the chaos atomic-visibility tests verify correctness but not throughput), or (b) a microbench that mocks the `ApplyBatchAsync` driver and measures per-saga apply latency in isolation.

**Routing decision.** D2 is **suspended** pending the validation gap above. The producer-side saga ceiling (309 keys/s in this run, ~268 in c2-xi) is now the next-most-impactful candidate, with a clear instrumented path:

- **Producer-side saga timing breakdown.** Add per-step duration histograms inside `AtomicWriteGrain.ExecutePhaseAsync` for: (1) `lattice.SetManyAsync(slice)` (the parallel prepare), (2) `BroadcastTerminalsAsync` (the per-shard terminal fan-out), (3) per-shard `AppendTxTerminalAsync` (the receiver-leaf terminal commit + pending-bucket drain). Run the bench and compare the per-step p50s against the saga's end-to-end 7.6s. Whichever step dominates is the actual binding constraint.

- **Specific hypothesis (highest prior).** `BroadcastTerminalsAsync` is likely the dominant per-saga cost. The terminal fan-out writes one WAL record per touched shard under the `LatticeAtomicBatchContext` ambient; if it serialises (per-shard turn-token contention, or per-shard WAL-append serialisation), it caps the saga's per-step parallelism regardless of how parallel the prepare phase is. Worth measuring before optimising.

**Revert details.** The D2-A change is reverted (`git checkout -- src/lattice.replication/ReplicationApplier.Batch.cs`); no commits left on the branch from this attempt. The Azure tenancy / image were used for both A and B runs; the wasted compute is bounded at two `LocalBuild` deploys + two 105s rung runs (~6 minutes Azure ACI time at the c2-iii single-silo image). The csv `/.ladder-results-c2-xv-mode-set-many-atomic-A-baseline.csv` and the silo log `silo-20260527-211434Z.log` are kept for reproducibility but no production code or test change ships from this step.
...
#### U9p step 8c-c-iv-c2-xvii (2026-05-28) - Saga state checkpoints + reminder I/O attribution

**Goal.** Close the c2-xvi residual: phase-sum was ~1.4s, public-surface `lattice.op.duration_ms` was ~7.7s, leaving ~6.3s unattributed. Hypothesised candidates: (a) saga `state.WriteStateAsync` checkpoint persists, (b) Orleans reminder registry I/O at saga entry/exit.

**Instrumentation shipped.** Two new histograms on `LatticeMetrics`:

| Histogram | What it captures | Per-call phase tag values |
|-----------|------------------|-----------|
| `orleans.lattice.saga.checkpoint.duration` | Wall-clock ms inside a single `state.WriteStateAsync` on `AtomicWriteGrain` | `prepare`, `execute-batch-commit`, `execute-batch-retry`, `execute-to-compensate`, `complete`, `broadcast-touched-init`, `broadcast-touched-drift`, `broadcast-touched-registry`, `broadcast-touched-expand`, `broadcast-touched-late` (10 distinct call sites) |
| `orleans.lattice.saga.reminder.duration` | Wall-clock ms inside Orleans reminder registry RPCs on `AtomicWriteGrain` | `register`, `unregister-get`, `unregister-drop` |

All `state.WriteStateAsync()` calls are now routed through `WriteSagaStateAsync(phase)`, a helper that captures `Stopwatch.GetTimestamp()`, awaits the persist, and records on the histogram in a `try/finally` so the observation fires even on the failure path. The wrapping leaves every surrounding rollback `try/catch` untouched. Reminder calls are similarly wrapped inline. Both histograms share the cached `(tree, walPartitions)` tag pair from `_sagaMetricTags`; per-call allocation is bounded to the `KeyValuePair<string, object?>` struct passed for the per-call phase tag (stack-allocated; the string value is a literal, no boxing).

**Allocation-discipline correction.** The first version of `GetSagaMetricTags` cached the tree id from `state.State.TreeId`, which is `null` until `PrepareAsync` populates it. `RegisterKeepaliveAsync` runs before `PrepareAsync`, so its first observation cached a null tree tag for the activation's lifetime, poisoning every subsequent histogram observation. Fixed by sourcing the tree id from the grain key (`GrainContext.GrainId.Key.ToString()`), which is `{treeId}/{operationId}` and is always available at activation. The fix surfaced via the integration tests (7 saga-phase tests turned red immediately); pinned forward via the test-harness's tree-id-tagged matchers.

**Measured (Azure, single silo, c2-xi rung `500:10`, steady-state mid-run, 116 sagas).**

| Phase | Calls per saga | p50 (ms) | Per-saga contribution |
|-------|---:|----:|----:|
| `saga.reminder.duration[register]` | 1 | 0.12 | 0.12 |
| `saga.reminder.duration[unregister-get]` | 1 | 0.10 | 0.10 |
| `saga.reminder.duration[unregister-drop]` | 1 | 0.05 | 0.05 |
| `saga.checkpoint.duration[prepare]` | 1 | 17.58 | 17.58 |
| `saga.checkpoint.duration[execute-batch-commit]` | 1 | 15.74 | 15.74 |
| `saga.checkpoint.duration[complete]` | 1 | 18.36 | 18.36 |
| `saga.checkpoint.duration[execute-batch-retry]` | rare | 39.72 | ~0 (only on retry) |
| `saga.checkpoint.duration[execute-to-compensate]` | rare | 25.73 | ~0 (only on failure) |
| **Combined reminder + checkpoint** | | | **~52ms** |

**The unattributed-residual hypothesis is falsified.** Reminder I/O is sub-millisecond. Saga checkpoints sum to ~52ms p50 per happy-path saga (~3 persists × ~18ms each). Combined with the c2-xvi-measured phase sum (~1,424ms), the total per-saga cost is **~1,476ms p50** - far below the `lattice.op.duration_ms` p50 of ~8,400ms observed at this run.

**Root cause of the discrepancy.** `lattice.op.duration_ms` is a bench-only histogram defined in `benchmark/azure-throughput/Silo/BenchMetrics.cs`, recorded around `BenchWorkloadDispatcher.DispatchAsync`. Reading the dispatcher's `SetManyAtomic` branch reveals the misattribution: the dispatcher slices each producer batch (256 entries) into `AtomicBatchSize`-sized (64) sagas and **awaits each one sequentially** in a loop. `lattice.op.duration_ms` therefore measures **4 sequential saga calls** (256 / 64), not one saga. The c2-xi memo's *"per-saga p50 7,660ms"* attribution was wrong by a factor of 4; the actual per-saga p50 on the current branch is ~1.5-2.0s, fully attributable to the instrumented phases.

**Result of the instrumentation campaign.**

Final per-saga p50 attribution at the c2-iii operating point (single cluster, `shardCount=32`, `walPartitions=8`, `walMaxPendingBatches=8`, `phase2CoalescingMs=5`, rung `500:10`):

| Component | p50 (ms) | Share of single-saga cost |
|-----------|---------:|--------------------------:|
| Reminder I/O (register + unregister) | ~0.3 | <0.1% |
| Saga checkpoints (3 persists) | ~52 | ~3% |
| Phase: terminal_decision | ~13 | ~1% |
| Phase: prepare (parallel `lattice.SetManyAsync`) | ~531 | ~36% |
| Phase: broadcast (per-shard terminal fan-out) | ~880 | **~60%** |
| **Per-saga total** | **~1,476** | |

**Throughput overhead check.** A=220 keys/s (HEAD without instrumentation), B=226 (HEAD + saga.checkpoint), C=256 (HEAD + saga.checkpoint + saga.reminder). All three sit within the same +35%/-15% run-variance band the campaign has observed at this rung (229-309 keys/s). Instrumentation is not a regression; if anything the C run sits at the high end of variance, plausibly attributable to Azure-side noise across the three back-to-back deploys.

**Routing decision unchanged.** Broadcast remains the dominant cost at ~60% of per-saga p50. The next structural attack vector is the per-shard `AppendTxTerminalAsync` fan-out inside `BroadcastTerminalsAsync` (one Azure Tables WAL append per touched shard). Two candidate approaches identified in c2-xvi (eliminate per-shard terminal WAL append via registry-driven leaf drain; cross-saga WAL batching of terminal records) remain valid; both are larger than R1-scale changes and require a follow-up scoping memo. The instrumentation phase has now delivered the complete per-saga cost attribution so the next optimisation step can be evaluated against the measured contribution to per-saga p50.

**Test coverage.** 3 new tests in `LatticeMetricsIntegrationTests.SagaPhases.cs` (2 for checkpoint, 1 for reminder): each pins the new histogram emits at least one observation when a saga commits, tagged with the saga's tree id and the call-site phase. 17 metrics-integration tests pass (13 pre-instrumentation + 4 from c2-xvi + 3 from this step). 594 focused saga + metrics tests green at the full focused suite. 123 hygiene tests green.

**Provenance.**
- A (c2-xvii baseline, no checkpoint instrumentation): `silo-20260528-071411Z.log`, `.ladder-results-c2-xvii-mode-set-many-atomic-A-baseline.csv`.
- B (with checkpoint instrumentation only): `silo-20260528-072013Z.log`, `.ladder-results-c2-xvii-mode-set-many-atomic-B-instr2.csv`.
- C (with checkpoint + reminder instrumentation): `silo-20260528-073440Z.log`, `.ladder-results-c2-xviii-mode-set-many-atomic-full-instr.csv`.

**Last Updated**: 2026-05-28 (Phase D-instr complete: saga.checkpoint and saga.reminder histograms close the c2-xi attribution gap by revealing the c2-xi 7.7s p50 measured 4 sequential sagas not one; actual per-saga p50 is ~1.5s; broadcast 60% / prepare 36% / checkpoint 3% / decision 1% / reminder <0.1%; routing decision unchanged: broadcast is the next attack vector; structural optimisation requires a follow-up scoping memo).

#### U9p step 8c-c-iv-c2-xix (2026-05-28) - Phase D-broadcast-hlc-skip attempt: skip ComputeTerminalHlcAsync when no replication adapter (NULL/REGRESSION, reverted)

**Goal.** c2-xvii attributed ~60% of per-saga p50 (~880ms) to broadcast. Reading `ShardRootGrain.TxTerminal.cs::AppendTxTerminalAsync` showed `terminalHlc` is computed unconditionally via a parallel `GetClockAsync` fan-out across affected leaves, but is consumed ONLY by the WAL record built inside `if (writer is not null) { ... }`. When no replication adapter is registered (the single-cluster bench case), the writer resolves to `null` and the WAL record - and therefore the terminal HLC - is dead work. Hypothesis: move the HLC compute INSIDE the `if (writer is not null)` branch to skip the parallel fan-out in the single-cluster shape.

**Implementation shipped (then reverted).** Hoisted the `var writer = ResolveCommitLogWriter();` call before the HLC compute, gated the compute behind `if (writer is not null) { var terminalHlc = await ComputeTerminalHlcAsync(hlcLeaves); ... }`. ~10 LOC change in one method, single file. 322 focused atomic + shard-root + compensation tests green - correctness preserved.

**Measured (Azure, single silo, c2-xi operating point: rung `500:10`, `BENCH_BATCH_SIZE=256`, `BENCH_PHASEA_REPORT_SEC=30`, `DurationSec=180`).**

| Run | SteadyAvg | SteadyMax | FinalWritten | FinalFailed |
|-----|----------:|----------:|-------------:|------------:|
| A (HEAD, stashed) | 271/s | 1,279 | 27,956 | 1 |
| B (HEAD + hlc-skip) | **177/s** | 1,024 | 19,245 | 0 |

**-35% throughput regression.** Below the campaign's normal +35%/-15% run-variance band (229-309 keys/s at this rung). Per-phase histograms in B showed broadcast p50 at the steady-state window of ~930ms, essentially unchanged from the c2-xvi baseline (~880ms) - **the skip did not measurably reduce broadcast cost** despite eliminating the parallel fan-out.

**Root cause analysis.** The HLC fan-out via `ComputeTerminalHlcAsync` already targets `hlcLeaves`, which `AppendTxTerminalAsync` populates from `TryConsumeAffectedLeaves(transactionId)` - i.e. ONLY the leaves that recorded a prepare-phase write under this saga (the existing `TxAffectedLeaves` machinery shipped in earlier phases). Only when the per-saga set is empty (shard-root deactivated mid-saga, or routing bypass) does the code fall back to `CollectChainLeavesAsync` and walk the full leaf chain. In the bench's steady-state happy path the set is typically 1-2 leaves per shard, so the step-2 fan-out is a single parallel `Task.WhenAll` of ~16-32 leaf RPCs taking one Orleans turn-round-trip (~15-30ms). Removing it saves at most that much. The actual ~880ms broadcast cost lives in **step 4 (`BroadcastTerminalToLeavesAsync`)**, which performs the per-leaf terminal-apply (pending-tx drain into the visible projection + event publishes + idempotency mark + potential state persist) under the leaf's `_splitGate` semaphore. The c2-xv lesson repeats: the optimisation targeted dead code (in the bench) at one structural layer above the actual binding cost.

**Throughput regression source (unexplained).** Broadcast time per saga is unchanged in the per-phase histograms, but sustained throughput dropped 35%. The only plausible mechanism is reduced concurrent-saga depth (something upstream of the saga gate rate-limiting the producer-to-saga path). Not investigated further because the change is being reverted; the variance band at this rung (~±50% across observed runs) is wide enough that a small-effect change cannot be statistically distinguished without more reps.

**Routing decision.** The HLC-skip path is **dead** - both because the skip targets a small fraction of broadcast cost AND because the measurement showed no benefit. The next instrumentation step is to break down `BroadcastTerminalToLeavesAsync` itself (step 4 inside `AppendTxTerminalAsync`) with per-leaf duration / per-step (drain / publish / persist) histograms so the next optimisation candidate is chosen against measured per-leaf cost rather than inferred from comment archaeology. The repeating failure mode in this campaign is **scoping at one structural layer above the actual binding cost** (c2-xv: wrong cluster topology; c2-xix: dead code on a path that wasn't the slow one). The instrumentation discipline c2-xvi/c2-xvii applied to the saga grain needs the same application inside the shard root + leaf path.

**Provenance.**
- A (no hlc-skip): `silo-20260528-074840Z.log` (approx, the c2-xix A run), `.ladder-results-c2-xix-mode-set-many-atomic-A-baseline.csv`.
- B (with hlc-skip, reverted): `silo-20260528-080209Z.log`, `.ladder-results-c2-xix-mode-set-many-atomic-B-hlc-skip.csv`.

**Last Updated**: 2026-05-28 (Phase D-broadcast-hlc-skip null/regression reverted; the affected-leaves optimisation in HLC fan-out was already shipped via TxAffectedLeaves so the skip targeted negligible cost; broadcast cost lives in step 4 - per-leaf terminal apply - which now becomes the next instrument target).

#### U9p step 8c-c-iv-c2-xx (2026-05-28) - Per-shard and per-leaf broadcast attribution

**Goal.** The c2-xix attempt revealed that ad-hoc reasoning about which step inside `AppendTxTerminalAsync` was slow could not match measurement (we hypothesised step 2 HLC fan-out; shipped a change; observed no movement; reverted). The fix per the c2-xix routing memo is the same recipe c2-xvi/c2-xvii applied at the saga layer: break the broadcast cost down with per-shard and per-leaf histograms BEFORE attempting any structural change.

**Instrumentation shipped.** Two new histograms on `LatticeMetrics`:

| Histogram | What it captures | Tags |
|-----------|------------------|------|
| `orleans.lattice.saga.broadcast.shard.duration` | Wall-clock ms inside a single `ShardRootGrain.AppendTxTerminalAsync` call (step 1 + 2 + 3 + 4 from one shard's perspective) | `tree`, `shard` |
| `orleans.lattice.saga.broadcast.leaf.duration` | Wall-clock ms inside a single per-leaf `IBPlusLeafGrain.ApplyTxTerminalAsync` RPC dispatched by step 4 | `tree`, `shard` |

The shard histogram wraps the entire `AppendTxTerminalAsync` body after the `transactionId == Guid.Empty` short-circuit; the leaf histogram is dispatched via a new `TimedApplyTxTerminalAsync` helper that the two fan-out sites in `BroadcastTerminalToLeavesAsync` (the affected-leaves path and the fallback chain-walk path) call instead of the raw `leaf.ApplyTxTerminalAsync` invocation. Both follow the same `try/finally` allocation discipline used by the saga-grain histograms: stack-allocated tag pairs, no boxing beyond the existing wal-partitions int box that the saga-grain side already pays per activation.

**Measured (Azure, single silo, c2-iii operating point, rung `500:10`, steady-state mid-run window t=119.4s, sample shard 8).**

| Histogram | p50 (ms) | p90 (ms) | p99 (ms) |
|-----------|---------:|---------:|---------:|
| `saga.broadcast.duration` (saga-side, parallel WhenAll across ~32 shards) | 878 | 1,409 | 1,625 |
| `saga.broadcast.shard.duration` (sample shard 8) | **143** | 309 | 444 |
| `saga.broadcast.leaf.duration` (sample shard 8) | **0.04** | 0.13 | 0.35 |

**Findings.**

1. **Per-leaf RPC is essentially free** (~40us p50). The hypothesis from c2-xvii ("broadcast cost lives in step 4 - per-leaf terminal apply") is FALSIFIED. Step 4 is microseconds.

2. **Per-shard non-leaf cost is ~143ms p50.** With step 4 at ~0us and step 3 (WAL append) skipped in the bench (no replication adapter, `writer is null`), the remaining cost lives in **step 1 (affected-leaves resolution) and step 2 (HLC compute via `GetClockAsync` fan-out)**. Step 1 is a synchronous dictionary lookup so cannot account for 143ms; **step 2 is the binding cost**.

3. **The saga waits for the slowest shard.** Saga-side p50 (878ms) is ~6x the per-shard p50 (143ms). For a parallel `Task.WhenAll` of 32 long-tailed draws, the expected max-of-32 tail is roughly between the per-shard p90 and p99 - here ~310-440ms per shard, but observed 878ms saga-side, suggesting Orleans scheduler / co-located dispatcher congestion further amplifies the tail. The per-shard p99 of 444ms is the natural saga-side floor; the gap from 444ms to 878ms is the additional saga-side wait that this campaign has not yet attributed (a candidate for further instrumentation if needed).

4. **Step 2's cost is not the parallel `GetClockAsync` RPC itself - it is the LEAF TURN-TOKEN QUEUE.** `IBPlusLeafGrain.GetClockAsync` is declared on the public interface WITHOUT `[AlwaysInterleave]`. Inspecting the leaf implementation (`BPlusLeafGrain.cs:188`) confirms the body is `Task.FromResult(state.State.Clock)` - a pure in-memory field read with no awaits and no state mutation. The c2-iii ladder marked the leaf's hot-path mutators (`SetAsync`, `SetManyAsync`, `DeleteAsync`) `[AlwaysInterleave]` to break the turn-token queue, but the read-only `GetClockAsync` was left under the default single-turn policy. Under the bench's saga workload, every prepare-phase write the saga issues to the leaf takes the turn token (the per-key set goes through `[AlwaysInterleave] SetManyAsync`, but several non-saga foreground operations on the leaf still take the turn token), so the per-saga broadcast's `GetClockAsync` queues behind any in-flight non-interleave operation on each affected leaf. With ~32 shards x 1-2 affected leaves per shard in parallel, the saga waits for the slowest leaf's turn-token release - which is exactly the long-tailed shape the per-shard p50=143ms / p99=444ms reflects.

**Routing decision.** Mark `IBPlusLeafGrain.GetClockAsync` `[AlwaysInterleave]`. The body is provably side-effect-free (pure in-memory field read), so the safety argument is identical to R1's: a reader observing the field across an interleave window sees whatever value the writer most recently committed, and there is no consistency contract that would be violated by reading a stale-by-one-Tick HLC (the consumer is `ShardRootGrain.ComputeTerminalHlcAsync` which Ticks the max anyway, so a stale-by-one read still produces a monotonically-greater terminal HLC). The expected p50 drop is from 143ms to ~the shard-side fixed overhead (microseconds for step 1 + low-ms for the parallel-task dispatch overhead), which would propagate to the saga's broadcast p50 via the max-of-32 tail and to sustained throughput via the freed per-saga wall-clock budget.

**Provenance.** `silo-20260528-081916Z.log`, `.ladder-results-c2-xx-mode-set-many-atomic-broadcast-instr.csv` (SteadyAvg 261/s, within the campaign variance band - instrumentation is not a regression).

**Last Updated**: 2026-05-28 (per-shard + per-leaf broadcast attribution shipped; step 4 is microseconds, step 2's `GetClockAsync` fan-out under leaf turn-token queue is the binding cost; next attempt is `[AlwaysInterleave]` on `GetClockAsync` per the safety argument above).
...
- **DONE - U9p step 8c-c-iv-c2-vii (knob sweep at c2-iii baseline; leaf etag race fixed, commit `b872262`).** Five probes (P0/A/B/C/D) × 3 rungs (`10000:5`, `25000:5`, `50000:5`) against the c2-iii baseline. **Headline**: the c2-iii baseline does not reproduce on current HEAD until the leaf etag race is fixed (analogous to c2-vi-followup, this time on `BPlusLeafGrain` public state-writers); fix shipped via per-activation `_splitGate` on 6 methods (`SetNextSiblingAsync`, `SetPrevSiblingAsync`, `SetTreeIdAsync`, `SetShardIndexAsync`, `SetKeyRangeAsync`, `SetParentAsync`), 3 failing-first regression tests in `BPlusLeafGrainTests.EtagRace.cs`. Post-fix: 10000:5 = 12,708/s, 25000:5 = 12,044/s, 50000:5 = 6,109/s (all probe-0, `FinalFailed=0`). **Knob result**: every deviation from the c2-iii operating point (`WP=8`, `WMP=8`, `P2=5ms`) regressed on at least one rung; no Pareto win. Probe-A (`WP=16`) lost -61% at 25000:5; probe-B (`WMP=16`) lost -71% at 50000:5; probe-C (`WP=16+WMP=16`) lost -94% at 50000:5; probe-D (`P2=2`) lost -28% at 10000:5 and was cancelled after rung-1. **No knob-default flip needed.** See memo above for full table; library default flips (`WalPartitions`, `WalMaxPendingBatches`, `PhaseTwoCoalescingWindow`, `PipelinePhaseTwoCommits`) recommended for follow-up PRs gated on the chaos suite.
...
- **DONE - U9p step 8c-c-iv-c2-ix Phase D1 (parallel saga fan-out, commit `0017795`).** `AtomicWriteGrain.ExecutePhaseAsync` rewritten to fan out per-key writes under `Task.WhenAll` capped at `WalPartitions`, replacing the historically-sequential per-key loop. Per-key `state.WriteStateAsync` checkpoints replaced with a single post-batch persist. Class B revert preserved at all three new persist sites. 58/58 `AtomicWriteGrainTests` non-chaos pass. Measured at the c2-viii tuned operating point: `SteadyAvg 137 -> 203 keys/s (+48%)`, `FinalWritten 14,522 -> 21,065 (+45%)`, `FinalFailed=0` preserved. Per-saga p50 unchanged (~3,667 ms) - D1 lifts parallelism between sagas, not the per-saga cost. throughput.md updated with the new headline number (commit `74fbe3c`).
- **DONE - U9p step 8c-c-iv-c2-xi Phase D1c (offset-cursor changefeed + 4 prepared-batch HLC-gate bypasses).** The saga reverts to D1's single parallel `lattice.SetManyAsync(slice)` shape; the structural fix moves to the replication-stack gates that silently relied on per-WAL-partition HLC monotonicity (an invariant the producer-side WAL never actually held once `shardCount > 1`). Four coordinated changes: (1) `IChangeFeed.Subscribe` flips its cursor from `HybridLogicalClock` to per-partition WAL offset (new `ChangeFeedCursor` value type + `GetCurrentCursorAsync` resume primitive); the legacy HLC overload remains for source-compat but its `entry.Timestamp <= cursor` filter is now a no-op. (2) `ReplicationApplier.ApplyAsync` bypasses per-origin HWM dedup for `IsPrepared && AtomicBatchSize > 0` entries (single-entry + batched paths). (3) The same bypass on the causal-park gate (`HasCausalDependencies`+`ParkAsync`) closes a chicken-and-egg deadlock between sibling prepares' VC frontiers. (4) `ReplicationShipperGrain` bypasses its own `state.Cursor` HLC filter on the same class of entries. Measured at the c2-viii tuned operating point: **SteadyAvg 216 -> 268 keys/s (+24% over D1b, +96% over the pre-D1 baseline)**, FinalFailed=0, per-saga p50 9.3s -> 7.7s (-17% vs D1b). Regression coverage: 7 unit tests in `ReplicationApplierTests.PreparedNonMonotonicHlc.cs` pin the four bypass conditions. End-to-end correctness verified by 1667/1667 non-chaos replication tests + 6/6 reps of the previously-flaky `BootstrapAtomicVisibilityTests.Concurrent_producer_saga_during_bootstrap_*` + 5/5 reps of `CrossClusterAtomicVisibilityChaosTests.Concurrent_cross_cluster_sagas_*`. Two post-ship fixes followed the initial ship (see memo above): (a) a `ChaosDeliveryPump` mid-iteration partition truncation guard closing a ~30% chaos-test flake, and (b) a `ChangeFeedCursor` inclusive-vs-exclusive off-by-one (the original `next-1`-clamped-at-zero design made `cursor[p]=0` ambiguously mean "fresh" OR "consumed offset 0", silently re-yielding offset 0 across resumes until offset 1 was appended) - flipped to exclusive-lower-bound (cursor entry = next-offset-to-read), with a dedicated regression test `ChangeFeedTests.GetCurrentCursorAsync_resume_after_single_append_does_not_redeliver` pinning the contract. The remaining gap to the c2-ix structural-ceiling estimate (~8,500 keys/s) is the receiver-side per-key apply path - closing that becomes Phase D2 (teach `IReplicationApplyGrain` a batched-saga apply primitive). throughput.md updated with the new headline number; provenance `silo-20260527-143915Z.log` / `.ladder-results-c2-xi-mode-set-many-atomic-D1c.csv`.
- **NEW (c2-viii) - Phase B/C library default flips, re-ordered by measured Layer-2 impact.** Original ladder ordering was alphabetical (B2 -> B3 -> C1); c2-viii data ranks the impact as: (1) `PhaseTwoCoalescingWindow: 0 -> 5ms` (biggest single win; -57% measured at P2=0 vs P2=5 in c2-iv probe-D), (2) `PipelinePhaseTwoCommits: false -> true` (high-impact but never measured isolated; plausibly second-largest), (3) `WalPartitions: 1 -> 8` (removes WAL serialisation), (4) `WalMaxPendingBatches: 1 -> 8` (pairs with `WalPartitions` flip). Each remains its own gated PR per chaos suite; this is a planning-prioritisation change, not a scope change.
- **NEW (c2-viii) - Phase R: read-path scaling (new campaign axis).** c2-viii exposed reads sustaining ~5x writes on a single silo (`GetAsync` 45,750 keys/s, `GetManyAsync` 49,736 keys/s). Reads do not touch the WAL or `PhaseTwoWorker`; the entire Azure-Tables-account-ceiling story does not apply to them. Phase R's open questions: (a) where is the read-path ceiling? (b) what dominates per-call read latency - leaf cache hit, Orleans RTT, or both? (c) `GetManyAsync` batching gain ratio at small batch sizes? (d) cold-cache reads (would require a "deactivate-between-pre-seed-and-measurement" bench-harness lever). Not blocked by any prior phase.
- **NEW (c2-viii) - per-`ILattice`-call bench instrumentation.** Layer-2 per-call latency for the fan-out point modes (`SetAsync`, `GetAsync`) is derived (dispatcher latency / call count). A direct per-call recording inside `BenchWorkloadDispatcher.FanOutAsync` (new `lattice.call.duration_ms` instrument tagged by mode) would surface true per-call p50/p99 instead of averages. Estimated ~30 min of harness work; queue behind Phase D.
- **DEFERRED (revised) - prior B / C plan steps (Phase B1 / B4 / B5 / C2 / C3 / C4 / C5).** Phase A's outer-boundary instrumentation is done (c2-viii); Phase D is now active (above); Phase B/C library default flips are re-prioritised but still gated on chaos. The remaining work items (B1 chaos sub-scenario for `WalMaxPendingBatches > 1`; B4 turn-work audit; B5 buffer-pool review; C2 parallel batch transaction submission; C3 compression of large WAL payloads; C4 retry / backoff tuning; C5 account-partition spread guidance) become live once the library-flip ladder ships and per-stage attribution from Phase A's inner instrumentation is in place.

- **DEFERRED - U9p step 8c-c-iv-c2-x Phase D2 (receiver-side batched-saga apply).** Scoped to introduce a new `IReplicationApplyGrain.ApplyPreparedManyAsync` primitive collapsing the receiver applier's per-prepared-entry RPC fan-out (one `ApplyPreparedSetAsync`/`ApplyPreparedDeleteAsync` call per saga key) into a single batched RPC per saga, mirroring the existing `ApplyMergeManyAsync` shape. Two design constraints surfaced during scoping that motivated deferral: (a) preserving per-item ambient context (each prepared entry carries its own `SourceHlc`, `OriginClusterId`, `VectorClock`, `ExpiresAtTicks`) inside a single shard-root call requires either a per-key HLC-override map (significant new ambient-context machinery, correctness-critical) or per-item context re-establishment inside the shard-root loop (smaller scope, but still net-new code on the receiver hot path), (b) the alternative simpler optimisation - parallel-dispatch of contiguous prepared runs via `Task.WhenAll` inside `ApplyBatchAsync` - touches the applier's accounting state-machine (`runningHwm`, `highestApplied`, `cacheReservedForCurrent`, `pendingApplies`) which today only works because it is strictly serial, and partial-failure semantics under `AggregateException` need a new DLQ-routing design. Win bound is large (theoretical N-times serial-to-parallel speedup on the receiver) but unmeasured locally; the c2-xi memo explicitly defers measurement to operator-driven Azure runs, so shipping on inference is premature. Re-prioritise behind Phase R per the c2-xii memo above.
- **NEW (c2-xii) - Phase R kick-off: `PointGetMany_BatchSize` sweep instrument.** Shipped a parameterised `[Arguments(1, 2, 4, 8, 16, 32, 64)]` variant of `LatticeMicroBenchmarks.PointGetMany` to answer c2-viii open question (c) (`GetManyAsync` batching gain ratio at small batch sizes). Pre-built identity-stable batches in `GlobalSetup` so per-iteration allocation reflects only the async state machine and the result dictionary. Build green; em-dash hygiene green. Operator microbench run pending; follow-up c2-xiii memo will record the measured curve, the per-key inflection point, and route to either (i) `GetManyAsyncCore` fixed-overhead optimisation if the curve is dominated by the size=1 fixed cost, or (ii) a multi-shard `PointGetMany_BatchSize_MultiShard` instrument if the single-shard curve plateaus early.
...
- **DONE - U9p step 8c-c-iv-c2-xvii Phase D-instr-completion (saga checkpoint + reminder histograms; c2-xi misattribution discovered and corrected).** Wrapped all 10 `state.WriteStateAsync()` call sites in `AtomicWriteGrain` via a new `WriteSagaStateAsync(phase)` helper recording on `orleans.lattice.saga.checkpoint.duration`; instrumented `RegisterKeepaliveAsync` / `UnregisterKeepaliveAsync` recording on `orleans.lattice.saga.reminder.duration`. Per-call phase tag distinguishes the 10 checkpoint sites and the 3 reminder sites. Allocation-discipline fix: `GetSagaMetricTags` now sources the tree id from the grain key (`{treeId}/{operationId}`) so the cache is correctly populated even before `PrepareAsync` sets `state.State.TreeId` (originally cached null tree tag from `RegisterKeepaliveAsync` which runs first). Measured at the c2-iii operating point rung 500:10 (steady-state mid-run window, 116 sagas): **checkpoint sum ~52ms/saga (3 happy-path persists × ~18ms p50), reminder sum ~0.3ms/saga (sub-millisecond)** - falsifies the residual-cost hypothesis. Root cause of the c2-xvi unattributed-residual: `lattice.op.duration_ms` is a bench-only histogram that measures `BenchWorkloadDispatcher.DispatchAsync`, which slices each producer batch (256 entries) into 64-key sagas and **awaits them sequentially in a loop** - so the c2-xi-reported `7,660ms per-saga p50` was actually 4 sequential sagas, not one. **Actual per-saga p50 is ~1,476ms** (broadcast 880 + prepare 531 + checkpoint 52 + decision 13 + reminder 0.3). Routing decision unchanged: broadcast is the next attack vector. 17 metrics-integration tests pass (10 pre-instrumentation + 4 from c2-xvi + 3 from this step); 594 focused saga + metrics tests green; 123 hygiene tests green. See c2-xvii memo above for the full attribution table, the allocation-discipline correction, and the provenance `silo-20260528-073440Z.log`.

- **NULL/REGRESSION (reverted) - U9p step 8c-c-iv-c2-xix Phase D-broadcast-hlc-skip attempt.** Hoisted `ResolveCommitLogWriter()` before `ComputeTerminalHlcAsync` and gated the parallel `GetClockAsync` fan-out behind `if (writer is not null)` so the HLC fan-out is skipped when no replication adapter is registered (single-cluster bench shape). ~10 LOC change in one method; 322 focused atomic + shard-root + compensation tests green. **Azure A/B at the c2-xi operating point rung 500:10: A=271 keys/s, B=177 keys/s, -35% regression** below the campaign's normal variance band. Per-phase histograms in B showed broadcast p50 unchanged at ~930ms - the skip targeted a small fraction of broadcast cost (the affected-leaves optimisation was already shipped via `TxAffectedLeaves`, so `hlcLeaves` is already minimal in the happy path) AND did not measurably reduce broadcast time. The 880ms broadcast cost lives in step 4 (`BroadcastTerminalToLeavesAsync`: per-leaf pending-tx drain + event publishes + idempotency mark + state persist under `_splitGate`), not step 2. Throughput regression source (unexplained) not investigated further because the change is reverted; variance bracket at this rung is too wide to extract small-effect signal. Routing decision: instrument step 4 with per-leaf / per-stage histograms before any further structural attempt. The repeating failure mode in this campaign is scoping at one structural layer above the actual binding cost; the c2-xvi/c2-xvii instrumentation discipline needs to be applied inside the shard root + leaf path.

- **DONE - U9p step 8c-c-iv-c2-xx Phase D-broadcast-instr (per-shard + per-leaf broadcast attribution).** Two new histograms on `LatticeMetrics` (`saga.broadcast.shard.duration` wrapping `ShardRootGrain.AppendTxTerminalAsync`; `saga.broadcast.leaf.duration` wrapping each per-leaf `ApplyTxTerminalAsync` RPC dispatched by step 4). Same allocation discipline as the c2-xvi/c2-xvii saga histograms. Measured at the c2-iii operating point rung 500:10 (steady-state mid-run window, sample shard 8): **per-leaf RPC p50 = 0.04ms (microseconds), per-shard p50 = 143ms, saga-side p50 = 878ms**. Per-leaf is essentially free - the c2-xvii hypothesis that broadcast cost lives in step 4 is FALSIFIED. Per-shard cost dominates via step 2 (parallel `GetClockAsync` fan-out), which on inspection runs under the leaf's default single-turn policy (the c2-iii ladder added `[AlwaysInterleave]` to the hot-path mutators but left the read-only `GetClockAsync` under the turn token). Routing decision: ship `[AlwaysInterleave]` on `IBPlusLeafGrain.GetClockAsync` as the c2-xxi attempt. Safety argument mirrors R1's `[AlwaysInterleave]` on `GetDecisionsRevisionAsync`: the body is a pure in-memory field read with no side effects; the consumer (`ShardRootGrain.ComputeTerminalHlcAsync`) ticks the max anyway so a stale-by-one-Tick read still produces a monotonically-greater terminal HLC. Provenance: `silo-20260528-081916Z.log`, `.ladder-results-c2-xx-mode-set-many-atomic-broadcast-instr.csv` (SteadyAvg 261/s, within campaign variance band).

- **NULL (reverted) - U9p step 8c-c-iv-c2-xxi Phase D-broadcast-interleave attempt.** Added `[AlwaysInterleave]` to `IBPlusLeafGrain.GetClockAsync` so the saga's parallel `GetClockAsync` fan-out (step 2 of `BroadcastTerminalAsync`) bypasses the leaf turn token. Safety argument: body is `Task.FromResult(state.State.Clock)` - pure in-memory field read; consumer ticks the max so stale-by-one-Tick is harmless. 838 focused atomic + shard-root + leaf + compensation tests green. **Azure A/B at the c2-iii operating point rung 500:10: A=233 keys/s, B=259 keys/s (+11%)** - within the campaign variance band at this rung (229-309). B's per-shard `saga.broadcast.shard.duration` p50 ~190ms vs c2-xx baseline ~143ms (slightly UP, not down); saga-side `saga.broadcast.duration` p50 ~930ms vs c2-xx ~878ms (essentially unchanged). The hypothesis "step 2 cost is leaf turn-token queueing" is FALSIFIED - turn-token contention is not the dominant per-shard cost. The 140-190ms per-shard p50 must come from something else inside `ComputeTerminalHlcAsync` (the `Task.WhenAll` orchestration overhead at this fan-out factor, the `affectedLeafGrainIds` resolution, or shard-root self-scheduling between step 1 and step 2), or from step 4's per-leaf `_splitGate` + state persist that runs after the timer stops in `AppendTxTerminalAsync`. Routing decision: instrument step 2's internal sub-stages (resolve-affected-leaves vs WhenAll-fan-out vs Tick-max) and step 4's per-leaf `_splitGate` acquisition before the next structural attempt. The repeating failure mode this campaign - scoping at one structural layer above the actual binding cost - continues to require ever-finer instrumentation. Provenance: `silo-20260528-084539Z.log`, `.ladder-results-c2-xxi-mode-set-many-atomic-A-baseline.csv` and `-B-interleave.csv`.
- **DONE - U9p step 8c-c-iv-c2-xxii Phase D-broadcast-substage attribution.** One new histogram (`saga.broadcast.shard.stage.duration`) tagged with `tree`, `shard`, and a new `stage` slot (`resolve` | `hlc` | `wal` | `fanout`) wrapping the four sub-spans inside `ShardRootGrain.AppendTxTerminalAsync` (target resolution, `ComputeTerminalHlcAsync`, WAL `AppendAsync`, leaf fan-out + shadow-forward). PhaseA reporter folds the `stage` tag into its `phase` slot the same way it already folds `TagStep` so the silo log surfaces `phase=resolve|hlc|wal|fanout` rows alongside the existing per-shard envelope. Measured at the c2-iii operating point rung 500:10 (steady-state mid-run, all 64 shards). **Result is unambiguous: WAL append is ~100% of the per-shard cost.** Sample shard 8: resolve p50 = 0.02ms, hlc p50 = 0.02ms, **wal p50 = 191.47ms**, fanout p50 = 0.10ms; shard envelope p50 = 191.57ms. Distribution holds uniformly across all 64 shards (WAL p50 band 145-230ms, p99 band 400-1070ms). The saga-side `saga.broadcast.duration` p50 of ~959ms is the max-of-64-parallel-WAL-appends. The c2-xx attribution to "step 2 GetClockAsync turn-token queueing" was wrong by construction - HLC compute is 0.02ms not 143ms - and the c2-xxi `[AlwaysInterleave]` attempt could not have helped because the cost it targeted does not exist. The actual binding cost on the terminal-broadcast path is the durability hop: one Azure Tables `AppendAsync` per shard per saga, hitting the WAL partition. Routing decision: optimise the terminal-mark WAL write. Candidates: (a) coalesce per-shard terminal records inside one saga so the 64 shards write fewer than 64 partition-transactions (eligible because terminals are idempotent and dedup by transaction id on the receiver); (b) defer the terminal WAL append off the saga's critical path (it is currently the gate the saga waits on inside step 3 before step 4 fan-out, but receiver-side dedup means a foreground-RPC-first / WAL-second ordering is correctness-preserving for the local cluster); (c) batch terminal records across multiple in-flight sagas into one partition transaction. Option (b) is the cheapest to prototype and would re-attribute the broadcast wall-clock from ~960ms to the fanout-only p50 of ~0.1ms - immediately. Provenance: `silo-20260528-090858Z.log`, ladder SteadyAvg=212/s (within campaign variance band 122-309 - the c2-xxii instrumentation overhead is below the noise floor). 336 focused shard-root + atomic + metrics tests green; build clean.
- **DONE - U9p step 8c-c-iv-c2-xxiii Phase D-broadcast-batched-wal lift.** `IShardRootGrain.AppendTxTerminalAsync` now returns `Task<WalRecord?>` and takes a new `inlineWalAppend: bool = true` parameter (wire-compatible default; only the saga coordinator opts out). When `false`, the shard builds the terminal `WalRecord` through steps 1-4 (resolve, HLC compute, leaf fan-out, shadow-forward) but does not call `writer.AppendAsync` - the record is returned instead. `AtomicWriteGrain.BroadcastTerminalsAsync` collects every per-shard record from the parallel fan-out and dispatches them through `ICommitLogWriter.AppendManyAsync` once, which groups by WAL partition and fans out one batched `IWalShardGrain.AppendBatchAsync` per partition in parallel - the N serialised single-entry partition transactions collapse into one batched transaction per partition. Saga still awaits the batched write before returning so WAL durability is preserved (the c2-xxii false start that "fire-and-forget" was unsafe because leaves rely on the WAL for durability is honoured here - this lifts but does not defer). Direct callers (cross-cluster replay, shadow-forward, retroactive prepared-mutation sweep, unit tests) keep the historical inline-WAL shape via the default. Topology safety audited across online split / online resize / shadow-forward / single-shard / failure-mid-saga / coordinator reactivation - the lift preserves every existing invariant because (a) `MarkOneShardAsync` still retries `Stale*RoutingException` and only the *winning* attempt produces a record, (b) `ForwardShadowAsync` to a different physical tree uses the default inline shape, (c) `AppendManyAsync` already collapses single-entry to `AppendAsync` so degenerate sagas behave identically, (d) receiver-side `_recentlyTerminal` dedup makes any re-broadcast on coordinator reactivation idempotent.

  Measured at the c2-iii operating point rung 500:10, set-many-atomic, sample shard 8, B/A=`silo-20260528-100705Z.log`/`silo-20260528-085947Z.log` (the c2-xxii baseline):

  | Histogram                                       | A (c2-xxii)   | B (c2-xxiii)  | Delta     |
  |-------------------------------------------------|---------------|---------------|-----------|
  | `saga.broadcast.shard.stage.duration phase=wal` p50 | 191.47 ms     | **0.00 ms**   | -100%     |
  | `saga.broadcast.shard.duration` p50              | 191.57 ms     | **0.08 ms**   | -99.96%   |
  | **`saga.broadcast.duration` (saga-side) p50**    | ~960 ms       | **96.01 ms**  | **-90%**  |
  | `saga.broadcast.duration` p99                    | ~2700 ms      | **306.83 ms** | -89%      |
  | `wal.append.batch_entries` p50                   | 1             | 1 (p90=4, p99=7) | tail batches up |
  | `wal.shard.dispatch.duration` p50                | 138 ms        | 62 ms         | -55%      |
...
- **NULL/REGRESSION (env-only, no source change) - U9p step 8c-c-iv-c2-xxv Phase D-set-many-WMP-raise attempt.** The c2-xxiv attribution showed `wal.append.in_flight` saturating at p99=7 against the c2-iii cap of `WalMaxPendingBatches=8`; the cheapest test was to raise the cap and see if pipeline-depth relief shrank the per-leaf wait tail. Env-only change: `BENCH_WAL_MAX_PENDING_BATCHES=16` on the saga rung 500:10, no source diff. **Azure A/B: A=623 (c2-xxiv WMP=8), B=468 keys/s (WMP=16) = -25%** - a clear regression, not a null. Per-stage histograms confirm the mechanism: `wal.append.in_flight` p99 climbed 7 → 11 (cap now 14 not 7) but `wal.append.provider.duration` p50 38 ms vs 34 ms - Azure round-trip did NOT speed up to absorb the extra concurrency. `shard_root.set_many.local_apply` p50 grew 96 ms → 115 ms (+20%) and p99 662 ms → 746 ms (+13%); `leaf_rpc` p50 grew 78 ms → 94 ms (+21%). The hypothesis "raising the cap relieves leaves parked on the head of the in-flight chain" is FALSIFIED: at WMP=8 the cap is being hit precisely because Azure can serve at most ~8 concurrent Tables transactions per WAL partition usefully; raising the cap to 16 admits more concurrent flushes that *compete* for the same Azure budget, and each individual flush's tail grows (the leaf-side wait then queues behind a deeper slowest-in-flight). This re-confirms the c2-vii probe-0 verdict that `WMP=8` is the sweet spot for the c2-iii topology - the c2-xxiv re-attribution did not change that. Routing decision: the per-leaf WAL wait is **Azure-bound, not pipeline-depth bound**, so structural attempts to coalesce more entries into fewer Azure round-trips are the only viable next vector. Specifically: the c2-xxiv data showed `wal.append.batch_entries` p50=1 even under saturated traffic - because each leaf calls `writer.AppendManyAsync(walEntries)` independently and the writer's per-partition coalescing only sees entries from one leaf at a time. **Lifting cross-leaf WAL writes to the shard layer** (each leaf builds its records but returns them; the shard collects across all leaves of a saga slice and calls `writer.AppendManyAsync` once per shard) would let the writer's existing per-partition grouping see entries from all leaves on the same partition simultaneously, climbing `batch_entries` toward N (8-32) and reducing Azure round-trips by the same factor. The c2-xxiii safety analog applies but is more delicate because the leaf currently relies on inline WAL-before-Apply for its own durability; a shard-level lift requires either (a) the leaf still awaits the shard's batched dispatch before stepping 2/3/4 (two-RPC continuation contract, RPC overhead may exceed the WAL saving), or (b) restructuring the durability boundary so the shard's `state.WriteStateAsync` checkpoints the per-leaf in-memory deltas before reader visibility (substantial state-management change). Defer to a separate investigation phase. Provenance: `silo-20260528-111119Z.log`, `.ladder-results-c2-xxv-mode-set-many-atomic-WMP16.csv`. 893 focused tests green; no source changes.
- **CAMPAIGN PAUSE - U9p step 8c-c-iv-c2-xxvi Phase D-cross-leaf-WAL-lift evaluation.** Re-baselined the c2-xxv routing decision ("structural cross-leaf WAL lift is the only viable next vector") at the higher-signal `set-many` rung `10000:5` where the per-shard tail is dramatic. Result: **the cross-leaf lift hypothesis is partially falsified by the set-many workload's measured behaviour, and the set-many bottleneck does not match the saga bottleneck**.

  **set-many @ rung 10000:5 baseline (SteadyAvg 13,574 keys/s, WMP=8):**

  | Span                                       | p50         | p99         |
  |--------------------------------------------|-------------|-------------|
  | `set_many.fanout`                          | 2,012 ms    | 2,854 ms    |
  | `shard_root.set_many.local_apply`          | 1,638 ms    | 2,718 ms    |
  | `shard_root.set_many.leaf_rpc`             | 506 ms      | 1,328 ms    |
  | **`wal.append.batch_entries`** shard=0     | **8**       | **100**     |
  | `wal.append.provider.duration` shard=0     | 138 ms      | 318 ms      |
  | `wal.append.in_flight` shard=0 p50/p99     | 7 / 7       | (cap=8)     |
  | `wal.append.turn_wait` shard=0             | ~20 ms      | -           |

  **Key observation**: `wal.append.batch_entries` p50 at set-many traffic is already **8** (p90=100, the per-batch cap). The WAL grain's natural coalescing fires hard under saturated traffic - when one in-flight batch is mid-flush, subsequent arrivals park on the in-flight head and coalesce into the next batch. **The cross-leaf shard lift would not climb batch_entries above what the writer already achieves under saturation** because the writer was already seeing all leaves' entries on its per-partition grouping path. The set-many bottleneck is instead:

  1. **WAL provider serialised cutovers**: a single leaf's 4,096-entry batch fans out to ~8 partitions (~512 entries each); with `WalMaxBatchEntries=100`, each partition cuts into ~5-6 partition transactions which serialise through the in-flight cap. ~6 × 138 ms = ~828 ms accounts for the leaf_rpc p99=1,328 ms.
  2. **Max-of-N leaf tails inside one shard**: shard_root.local_apply p50=1,638 ms ≈ max-of-N draws from a distribution with leaf_rpc p50=506 ms / p99=1,328 ms (consistent with N≈8-16 leaves per shard fanout under saturation).
  3. **Max-of-N shard tails at the LatticeGrain**: set_many.fanout p50=2,012 ms ≈ max-of-N draws from local_apply p50=1,638 ms / p99=2,718 ms.

  None of (1)-(3) are addressed by cross-leaf WAL lift: (1) is per-partition provider concurrency-bound (raising it failed at c2-xxv); (2) is the max-of-N tail shape of parallel work and lifting WAL out of the leaf only shifts where the tail lives; (3) is the LatticeGrain shape itself. The saga rung (where `batch_entries p50=1` and there IS coalescing headroom) does show a cross-leaf opportunity in principle - but the rung's variance band at 500:10 is 228-623 keys/s, wide enough that any single A/B at this rung cannot reliably detect <50% effects.

  **Structural alternatives evaluated and rejected for this iteration:**

  - **Cross-leaf WAL lift at the shard layer (full c2-xxiii analog)**: requires breaking the leaf's WAL-before-Apply durability invariant or introducing a two-RPC continuation contract (`BuildWalForSetManyAsync` + `FinishSetManyAsync` with per-call state cached on the leaf). The second RPC cost (~10ms scheduling overhead) eats most of the WAL coalescing saving on the saga rung where the per-leaf entry counts are already small (1-2 entries/leaf at the 64-key saga slice spread across ~32 shards × 2 leaves), and is irrelevant on the set-many rung where batch_entries is already at its natural ceiling.

  - **WAL coalescing window inside `WalShardGrain.AppendAsync`** (deliberately wait briefly before kicking the first flush so siblings can coalesce): would help the saga rung's `batch_entries=1` profile but degrade the set-many rung's latency floor because the writer is already at p50=8 there. Mode-dependent tuning is a non-starter without per-tree options pollution.

  - **`WalMaxBatchEntries=200`/`500` to amortise more entries per partition transaction**: would reduce cutover serialisation on set-many but Azure Tables transaction size limits (~100 rows / 4 MB) bound how far this goes; the existing 100 entries / 4 MB cap is already at the platform ceiling.
...
- **DONE - U9p step 8c-c-iv-c2-xxviii Phase E-leaf-digest-coalesce.** Added `LatticeOptions.DigestCoalescingWindowMs` (default 0 - wire-compat synchronous publish; bench opts in at 5 ms via the new `BENCH_DIGEST_COALESCING_WINDOW_MS` env). When >0, `BPlusLeafGrain.PublishDigestUpwardAsync` defers the cross-grain `OnChildDigestPublishedAsync` hop behind a one-shot grain timer; mutations arriving within the window observe `_digestPublishTimer` non-null and skip rescheduling, so N per-call publishes collapse into one per window. The running `ProjectionHash` on persisted state still advances per-mutation (cold-reactivation replay invariant preserved); only the cross-grain hop to the parent internal node is deferred. Graceful `OnDeactivateAsync` drains any pending publish synchronously - but only when coalescing is active (running the drain under the synchronous path was discovered to re-publish a post-publish state that races with materialiser-driven projection rebuilds, breaking 2 `RebuildLeafProjectionAsync` regression tests; the gated drain matches the synchronous path's "nothing to drain" invariant). Allocation discipline: hot path returns `Task.CompletedTask` and allocates nothing once the timer is scheduled; timer registration alloc happens once per window, amortised across the N mutations it covers.

  Measured at the c2-iii operating point, set-point @ rung 10000:5, A baseline `silo-20260528-120517Z.log` (c2-xxvii instrumentation, window=0) → B `silo-20260528-122140Z.log` (window=5):

  | Span                                       | A (window=0) | B (window=5) | Delta     |
  |--------------------------------------------|--------------|--------------|-----------|
  | `set.duration` envelope p50                | 79 ms        | **58 ms**    | **-27%**  |
  | `set.duration` envelope p99                | 491 ms       | **300 ms**   | **-39%**  |
  | `set.stage phase=shard` p50                | 87 ms        | 60 ms        | -31%      |
  | **`leaf.commit phase=digest` p50**         | **13.13 ms** | **0.00 ms**  | **-100%** |
  | `leaf.commit phase=digest` p99             | 64 ms        | 0.09 ms      | -99.9%    |
  | `leaf.commit phase=wal` p50                | 48 ms        | 43 ms        | -10% (collateral)  |
  | `wal.shard.dispatch.duration` p50          | 48 ms        | 47 ms        | flat      |
  | `wal.append.provider.duration` p50         | 19 ms        | 14 ms        | -26% (collateral) |
  | `wal.append.batch_entries`                 | 1            | 1            | unchanged |

  **The digest publish disappeared from the per-call hot path entirely** (p50 0.00 ms = coalescing fires the inline `IsTimerNull` fast-path or rolls into the next window for ≥50% of calls). The 16% reduction the c2-xxvii routing decision predicted is matched and exceeded - the envelope dropped 27% p50 / 39% p99 because the digest-publish elision also un-blocks the leaf's turn-token, letting the next WAL append start sooner (the -10% collateral on `leaf.commit phase=wal` p50 and -26% on `wal.append.provider.duration` p50 reflect this knock-on). `wal.append.batch_entries` remained 1 (set-point still does not coalesce WAL entries across calls, by construction).

  **Throughput cell volatility.** Set-point at rung 10000:5 has a wide variance band: SteadyAvg samples this session were 237 (A1) / 146 (B1, c2-xxvii with instrumentation) / 202 (B2, c2-xxviii with coalescing), but SteadyMax climbed 8,187 / 3,327 / 16,381 keys/s respectively. The set-point dispatcher's behaviour at this rung is offered-load-bound by the producer batch + flush-window cadence, not by the silo's per-call ceiling - the throughput cell does not cleanly attribute to per-call latency changes. **The latency histograms (count=12,197 samples on the B run, all clean) are the definitive measurement.**

  **No effect expected on saga or set-many rows.** Saga prepare writes are `isPrepared=true` which routes through `AddPreparedMutation` and skips `PublishDigestUpwardAsync` entirely (per the `BPlusLeafGrain.CommitSetManyAsync` step-4 comment block). Set-many already coalesces digest publish once per batch - the per-batch hop is paid regardless of window. **The c2-xxviii win is set-point-specific.**

  **Routing decision: campaign pause for consolidation, again.** The campaign has now shipped two major structural wins (c2-xxiii batched-WAL saga terminal -90% saga broadcast p50; c2-xxviii leaf-digest coalescing -27% set-point envelope p50) and bottomed out the cheap-to-find structural overhead on every quoted Layer-2 row. Remaining cost is either Azure-provider-bound (per c2-xxvi) or framework-cost-bound (Orleans grain RPC ~46 ms total per point-write, per c2-xxvii). Both require either platform-level changes (more Azure throughput, more Orleans concurrency) or major durability-boundary refactors. The next sustainable improvement axis is **multi-silo horizontal scaling** (existing first-class roadmap item, see throughput.md "Single silo" caveat note about the 20,000 entities/s storage-account ceiling).

  Provenance: commit `e473b52`, `silo-20260528-122140Z.log`, `.ladder-results-c2-xxviii-mode-set-point-B-coalesce.csv`. 4 new unit tests (`BPlusLeafGrainTests.DigestCoalescing.cs`) + 1,200 focused regression tests green; build clean (the pre-existing CS9107 primary-ctor warning on `AtomicWriteGrain` is unrelated).
- **DONE - U9p step 8c-c-iv-c2-xxix Phase E-resolver-propagation-guard + c2-xxviii misattribution correction.** `LatticeOptionsResolver.ResolveAsync` builds a `ResolvedLatticeOptions` instance by copying `baseOptions` fields one-by-one. The `DigestCoalescingWindowMs` field was simply not copied; every leaf grain therefore observed the `ResolvedLatticeOptions` inherited default (`0` = synchronous publish) regardless of what the operator or bench configured. **The c2-xxviii leaf-side coalescing path never actually fired on Azure** - the apparent win in the c2-xxviii memo (`digest p50 13ms -> 0.00ms`) was misattribution. The resolver-drop bug was discovered when the c2-xxix `25000:5` set-point probe showed `digest p50 = 11ms`, contradicting c2-xxviii's `0.00ms` claim at `10000:5` and prompting a code audit.

  **Fix:** propagate `baseOptions.DigestCoalescingWindowMs` through to the returned `ResolvedLatticeOptions`. Single line in the resolver.

  **Regression gate (the more important deliverable):** new reflective propagation test (`LatticeOptionsResolverPropagationGuardTests`) enumerates every public instance property of `LatticeOptions`, sets a non-default sentinel on a fresh instance, resolves it, and asserts the property either passed through verbatim or matches the documented transformation expectation (`MaxLeafKeys` / `ShardCount` via `TreeRegistryEntry`, `MaintainProjectionDigest` via latch + per-tree override, compaction floors via clamps). On failure the test names the property AND lists call sites under `src/` that read the dropped field via a `receiver.PropName` grep so the operator can localise the wrong-value-observation site. The test was authored before the fix to confirm it fails for the exact reported bug:

      LatticeOptions.DigestCoalescingWindowMs not propagated by LatticeOptionsResolver.
          expected: 12289
          actual:   0
        consumer sites that may be observing the wrong value:
          src/lattice/BPlusTree/Grains/BPlusLeafGrain.cs:165: ? _options.DigestCoalescingWindowMs

  The `IntentionallyBypassedProperties` allowlist documents every field `LatticeOptions` exposes but the resolver does NOT need to forward, with the downstream consumer named (WAL options consumed via `IOptionsMonitor` inside `WalShardGrain`, registry-driven gates like `PublishEvents`, etc.). Adding a new `LatticeOptions` property without thinking about propagation now fails the guard with a localised error message.

  **Post-fix Azure verification at the c2-iii operating point, set-point mode, three rungs (silo logs `silo-20260528-144707Z.log`, `silo-20260528-145728Z.log`, plus the earlier `silo-20260528-122140Z.log` recalibrated post-fix):**

  | Span                                       | 1000:5    | 10000:5   | 25000:5   |
  |--------------------------------------------|-----------|-----------|-----------|
  | SteadyAvg                                  | 156 k/s   | 202 k/s   | 274 k/s   |
  | `set.duration` envelope p50                | 61 ms     | 58 ms     | 61 ms     |
  | `set.duration` envelope p99                | 441 ms    | 300 ms    | 416 ms    |
  | `leaf.commit phase=wal` p50                | 36 ms     | 43 ms     | 36 ms     |
  | **`leaf.commit phase=digest` p50**         | **0.00 ms** | **0.00 ms** | **0.00 ms** |
  | `leaf.commit phase=digest` p99             | 0.00 ms   | 0.10 ms   | 0.10 ms   |
  | `wal.append.provider.duration` p50         | 16 ms     | 14 ms     | 16 ms     |
  | `wal.append.batch_entries` p50/max         | 1 / 1     | 1 / 16    | 1 / 14    |
  | `wal.append.in_flight` max                 | 0         | n/a       | 3         |

  **The per-call cost is stable and offered-load-independent across all three rungs** (envelope p50 ~61 ms, leaf-WAL p50 ~36 ms, digest p50 = 0.00 ms). The set-point per-call cost is dominated by the leaf-side WAL append (`leaf.commit phase=wal` 36 ms = Azure provider 16 ms + Orleans grain RPC overhead ~20 ms) and is not addressable without either Azure-side parallelism (multiple storage accounts) or a major durability-boundary refactor (relocate WAL-before-Apply from leaf to shard). Throughput differences (156 / 202 / 274 keys/s) are purely offered-load-driven - more vehicles + same tick rate = more parallel hits = more total writes per second, but each individual call costs the same.

  **WAL coalescing not firing on set-point is by design, not a bug.** `wal.append.batch_entries p50 = 1` across all three rungs because the workload's per-partition arrival rate is below the coalescing threshold: with `FlushConcurrency=8` and `WalPartitions=8`, each partition sees roughly 1 in-flight call at a time, and the `WalShardGrain` first-arrival kick-flushes immediately (line 457: `kickFlush = _inFlight.Count < maxPending`). Coalescing requires a second arrival to land while the first is mid-flush, which requires either higher per-partition concurrency than this workload produces OR a larger producer-batch shape (which set-point breaks down into per-key calls anyway). Under saturated set-many (c2-iii baseline) the same `batch_entries` hits p50=8 / p90=100, confirming the mechanism is healthy.

  **Routing decision: throughput.md update only.** The campaign has shipped every viable structural optimisation on the SetAsync row; remaining cost is framework-cost (Orleans grain RPC) and Azure-provider-cost, both requiring out-of-scope changes. Update the `SetAsync` row in throughput.md to reflect the honest measured envelope (p50 ~58 ms / p99 ~300 ms on the c2-iii operating-point `10000:5` rung, replacing the legacy derived `~22 ms / ~37 ms` numbers that came from the `DispatchAsync(4096)/4096` heuristic in caveat ¹). Update caveat ¹⁰ to point at the c2-xxix log and call out the misattribution correction. Provenance: commits `43aacc3` (the resolver fix + propagation guard), `silo-20260528-144707Z.log` (25000:5), `silo-20260528-145728Z.log` (1000:5), `silo-20260528-122140Z.log` (10000:5; the original c2-xxviii log re-read post-fix). 1,195 focused regression tests green; build clean.