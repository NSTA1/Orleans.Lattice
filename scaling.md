...
- **NEW NEXT (c2-ix Phase D1b) - saga uses `SetManyAsync` instead of N x `SetAsync`.** D1's measured shortfall (+48% vs theoretical 4-8x) attributed to phase-A data showing `wal.append.in_flight=0` for the entire run - the WAL pipeline never saturates because each saga key dispatches as a size-1 batch. `BPlusLeafGrain.cs::SetManyAsync` has an existing batched fast path that the saga deliberately bails out of at line 833 when `isPrepared=true`. Two coordinated changes: (1) `CommitSetManyAsync` learns the `isPrepared` branch (calls `AddPreparedMutation` instead of `StoreEntry`); (2) `ExecutePhaseAsync` replaces `Task.WhenAll` of N `SetAsync` calls with one `lattice.SetManyAsync(entries)` call. Estimated effort ~1 hour. Structural ceiling estimate: ~8,500 keys/s, converging atomic with non-atomic write throughput. Gated on the chaos suite (atomic-visibility invariant).
