...
- **U9h-D - the next probe.** Full FC sweep on the U9h-B/C silo image at rung `1000:5` (1 000 vehicles * 5 Hz * 60 s) with `shardCount=16`, `batchSize=4096`, `walPartitions=4`, `walMaxPending=8`, FC in {8, 16, 24, 32}, single-knob change per probe. Hypothesis: U9h-A removed the etag race, U9h-B removed the dirty-leaf hot-path write, U9h-C removed the read-vs-write serial-turn collision; together they should let FC=24 and FC=32 drain without the per-shard turn queue collapsing at t=70 s (the U9h-A FC=24 failure mode). Falsifier: FC=24 and FC=32 must post zero `FinalFailed` and zero etag mismatches with steady avg >= 2 000/s; otherwise the next probe shifts to a rung sweep at FC=16 to engage more than 4 active shards.
- **U9j (deferred behind U9h-D) - rung ladder at the new safe FC** through `5000:5` and `10000:5` to break the "4 active shards out of 16" ceiling that bounds the `1000:5` rung at ~2 560/s (4 active shards * 1 000 ms / 25 ms phase-1 RT * ~16 entries-per-batch). Only revisit if U9h-D confirms FC>=24 is safe.
