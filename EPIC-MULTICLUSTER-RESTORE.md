...
| 6 | #1176 | Public user-defined saga participant SPI | #1175 | DONE (merged, replication 2631 green) |
...
- #1176 (public user-defined saga participant SPI): implemented in one worktree, merged (`0adfa29c`; merge `51f51909`, 703 insertions). Promotes `ISagaParticipant` and its `SagaParticipantPrepareResult` return type to public, and adds the `AddLatticeSagaParticipant<TParticipant>(name)` registration seam that appends a user participant to the coordinator's participant enumerable (via `TryAddEnumerable`, deduped on the `(ISagaParticipant, impl)` pair). The optional name wraps the impl in the closed-generic diagnostic decorator `NamedSagaParticipant<T>` (distinct closed type per T keeps the dedup working) and is logging/diagnostics only - it never touches the wire contract. `SagaParticipantPrepareResult` stays a plain in-process value type (never crosses a grain boundary) so it needs no `[GenerateSerializer]`/`[Alias]`/`[Id]`; no new aliases. Ships an `ExampleSagaParticipant` in the replication test project plus 13 new tests (67 regression, 5 hygiene green). Integration checkpoint: replication test project 2631 passed / 0 failed. Coordinator retains the public-SPI docs page and the samples/ wiring of a custom participant (folded into #1177).