...
/// request header into a <see cref="LatticeCredential"/> for the write-capable
/// data path. Proves scheme stripping, casing tolerance, the custom-header /
/// custom-scheme knobs, and the fail-closed null results for an absent, blank, or
/// scheme-only header - the anonymous cases the access gate denies on every
/// mutation and read.
/// </summary>
[TestFixture]
public sealed class HeaderLatticeDataApiCredentialBridgeTests
{
    private static HeaderLatticeDataApiCredentialBridge CreateBridge(
