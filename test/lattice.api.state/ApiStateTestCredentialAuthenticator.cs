...
/// Api.State authorization fixtures: it resolves the ambient credential's
...
/// groups. Selected only for credentials stamped with <see cref="Scheme"/>, so
/// it never shadows the anonymous fallback for an unstamped (system-origin) turn.
/// A per-assembly copy of the Auth-package test authenticator (internals are not
/// shared across test assemblies).
/// </summary>
public sealed class ApiStateTestCredentialAuthenticator : ILatticeCredentialAuthenticator
{
    ...
    public const string Issuer = "https://issuer.api-state.test/";
