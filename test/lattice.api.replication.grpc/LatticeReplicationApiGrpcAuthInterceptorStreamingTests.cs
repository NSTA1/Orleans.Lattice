...
/// <summary>
/// Behavioural regression coverage proving <see cref="LatticeReplicationApiGrpcAuthInterceptor"/>
...
/// </summary>
[TestFixture]
public sealed class LatticeReplicationApiGrpcAuthInterceptorStreamingTests
{
    private const string LatticeMethod =
        "/" + LatticeReplicationGrpcMethods.ServiceName + "/" + LatticeReplicationGrpcMethods.EnableReplicationMethodName;

    private const string ForeignMethod = "/some.other.service/Ping";

    private static LatticeReplicationApiGrpcAuthInterceptor Create(
        ...
        bool requireAuthorization = true)
    {
        ...
        options.CurrentValue.Returns(new LatticeReplicationApiGrpcOptions
        {
    ...
    private static ILatticeReplicationApiAuthorizer Authorizer(bool allow)
    {
        var authorizer = Substitute.For<ILatticeReplicationApiAuthorizer>();
        authorizer
            .IsAuthorizedAsync(Arg.Any<LatticeReplicationApiAuthorizationContext>(), Arg.Any<CancellationToken>())
            .Returns(allow);
        return authorizer;
    }
    ...
    public async Task A_foreign_server_streaming_method_is_passed_through_without_an_auth_check()
    {
        ...
        await authorizer.DidNotReceive().IsAuthorizedAsync(
            Arg.Any<LatticeReplicationApiAuthorizationContext>(),
