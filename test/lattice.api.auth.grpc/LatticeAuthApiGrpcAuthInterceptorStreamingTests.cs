...
/// <summary>
/// Behavioural regression coverage proving <see cref="LatticeAuthApiGrpcAuthInterceptor"/>
...
/// </summary>
[TestFixture]
public sealed class LatticeAuthApiGrpcAuthInterceptorStreamingTests
{
    private const string LatticeMethod =
        "/" + LatticeAuthApiGrpcMethods.ServiceName + "/" + LatticeAuthApiGrpcMethods.UpsertGroupMethodName;

    private const string ForeignMethod = "/some.other.service/Ping";

    private static LatticeAuthApiGrpcAuthInterceptor Create(
    ...
    private static ILatticeAuthApiAuthorizer Authorizer(bool allow)
    {
    ...
    public void A_denied_server_streaming_call_is_rejected_with_permission_denied()
    {
        ...
        var exception = Assert.ThrowsAsync<RpcException>(() => interceptor.ServerStreamingServerHandler<object, string>(
            ...
            new LoopbackServerCallContext(LatticeMethod),
            (_, _, _) =>
            {
                reached = true;
    ...
    public async Task A_foreign_server_streaming_method_is_passed_through_without_an_auth_check()
    {
        ...
        await interceptor.ServerStreamingServerHandler<object, string>(
            new object(),
            new DiscardingStreamWriter<string>(),
            new LoopbackServerCallContext(ForeignMethod),
            (_, _, _) =>
            {
                reached = true;
                return Task.CompletedTask;
        ...
        await authorizer.DidNotReceive().IsAuthorizedAsync(
            Arg.Any<LatticeAuthApiAuthorizationContext>(),
    ...
    {
        var interceptor = Create(Authorizer(allow: true));
        var context = new LoopbackServerCallContext(LatticeMethod);
