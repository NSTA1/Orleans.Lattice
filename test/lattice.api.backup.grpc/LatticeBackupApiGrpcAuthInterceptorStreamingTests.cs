...
/// <summary>
/// Behavioural regression coverage proving <see cref="LatticeBackupApiGrpcAuthInterceptor"/>
...
public sealed class LatticeBackupApiGrpcAuthInterceptorStreamingTests
{
    ...
    {
        var options = Substitute.For<IOptionsMonitor<LatticeBackupApiGrpcOptions>>();
        options.CurrentValue.Returns(new LatticeBackupApiGrpcOptions
        {
        ...
        return new LatticeBackupApiGrpcAuthInterceptor(
            authorizer,
            options,
            NullLogger<LatticeBackupApiGrpcAuthInterceptor>.Instance);
    }

    private static ILatticeBackupApiAuthorizer Authorizer(bool allow)
    {
        var authorizer = Substitute.For<ILatticeBackupApiAuthorizer>();
        authorizer
            .IsAuthorizedAsync(Arg.Any<LatticeBackupApiAuthorizationContext>(), Arg.Any<CancellationToken>())
            .Returns(allow);
        return authorizer;
    }
