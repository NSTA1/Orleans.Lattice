...
public sealed partial class LatticeWalGcSchedulerCadenceTests
{
    ...
    /// <summary>
    /// The re-arm fall-through relies on a transitive chain whose weakest link
    /// is an ordering between two constants, so assert the ordering directly
    /// (issue #2783).
    /// </summary>
    /// <remarks>
    /// <para>
    /// A restored budget must be spendable on the pass that restores it, or the
    /// first touch of every cycle serves a second wait on top of the backoff it
    /// has already served. Nothing clears the cooldown stamp to bring that
    /// about - the stamp lives on the observation and the re-arm only rebuilds
    /// the budget - so it holds only by transitivity:
    /// <c>now - lastAttempt &gt;= now - AbandonedAt &gt;= backoff &gt;= cooldown</c>.
    /// </para>
    /// <para>
    /// The first two links are structural: the abandonment is stamped on a pass
    /// no earlier than the attempt that spent the budget, and the re-arm will
    /// not fire before its backoff elapses. The third is not structural at all.
    /// It is an ordering between two independently-editable constants that
    /// currently hold it with <b>zero margin</b>, being equal, so reducing the
    /// backoff floor by one second breaks it. The failure is silent: every
    /// behavioural fixture still passes, because delaying the first touch of a
    /// cycle by the residual cooldown changes no outcome any of them assert.
    /// </para>
    /// <para>
    /// That combination - load-bearing, one edit away, and invisible to the
    /// behavioural suite - is what makes this worth a gate rather than the
    /// comment that used to be the only thing defending it. The assertion
    /// compares the two constants against <b>each other</b> rather than against
    /// a literal, so it is not the vacuous shape that passes for any value of
    /// the constant it names.
    /// </para>
    /// </remarks>
    [Test]
    public void Rearm_backoff_floor_is_never_shorter_than_the_retry_cooldown()
    {
        var floor = ReadSchedulerDuration("ReactivationRearmMinBackoff");
        var cooldown = ReadSchedulerDuration("ReactivationRetryCooldown");

        Assert.That(
            floor,
            Is.GreaterThanOrEqualTo(cooldown),
            "the shortest backoff a re-arm can serve must cover the retry cooldown, or the "
                + "restored budget is not spendable on the pass that restores it and the first "
                + "touch of every cycle is silently delayed.");
    }

    private static TimeSpan ReadSchedulerDuration(string fieldName)
    {
        var field = typeof(LatticeWalGcScheduler).GetField(
            fieldName,
        ...
        Assert.That(
            field,
            Is.Not.Null,
            $"{fieldName} must exist for the invariant to be checkable; renaming it without "
                + "updating this gate would leave the invariant undefended.");
