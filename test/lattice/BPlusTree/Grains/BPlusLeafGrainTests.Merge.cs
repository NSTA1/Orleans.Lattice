...
public partial class BPlusLeafGrainTests
{
    ...
    public async Task MergeEntries_preserves_original_timestamps()
    {
        ...
        Assert.That(grain.EntriesForTest["k1"].Timestamp, Is.EqualTo(clock));
    }
    ...
    public async Task MergeEntries_keeps_newer_local_value()
    {
        ...
        var localTimestamp = grain.EntriesForTest["k1"].Timestamp;
        ...
        Assert.That(Encoding.UTF8.GetString(grain.EntriesForTest["k1"].Value!), Is.EqualTo("local"));
        Assert.That(grain.EntriesForTest["k1"].Timestamp, Is.EqualTo(localTimestamp));
    }
    ...
    public async Task MergeEntries_with_mixed_live_and_tombstone_entries()
    {
        ...
        Assert.That(grain.EntriesForTest["live"].IsTombstone, Is.False);
        Assert.That(Encoding.UTF8.GetString(grain.EntriesForTest["live"].Value!), Is.EqualTo("value"));
    ...
    public async Task MergeEntries_with_empty_dictionary_is_noop()
    {
        ...
        var countBefore = grain.EntriesForTest.Count;

        await grain.MergeEntriesAsync(new Dictionary<string, LwwValue<byte[]>>());

        Assert.That(grain.EntriesForTest.Count, Is.EqualTo(countBefore));
    }
