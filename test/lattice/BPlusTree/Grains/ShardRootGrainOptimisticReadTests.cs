...
public sealed partial class ShardRootGrainOptimisticReadTests
{
    ...
    private static (ShardRootGrain Grain, FakePersistentState<ShardRootState> State, ILeafCacheGrain Cache) CreateGrain(
    ...
    [Test]
    public async Task TryGetOptimisticAsync_validates_an_absent_key_when_routing_is_stable()
    {
        var (grain, _, cache) = CreateGrain();
        cache.GetAsync("missing").Returns((byte[]?)null);
    ...
    public async Task TryGetOptimisticAsync_concurrent_reads_are_in_flight_together_against_a_gated_leaf()
    {
        ...
        cache.GetAsync("a").Returns(gateA.Task);
        cache.GetAsync("b").Returns(gateB.Task);
        ...
        await cache.Received(1).GetAsync("a");
        await cache.Received(1).GetAsync("b");
    ...
    public async Task TryGetOptimisticAsync_routing_mutation_between_snapshot_and_leaf_reply_forces_serial_retry()
    {
        ...
        cache.GetAsync("k1").Returns(gate.Task);

        var read = grain.TryGetOptimisticAsync("k1");

        // A simulated promotion / move-away publish runs to completion while the
        // read is parked on the leaf, and the leaf then answers "absent" (the U9h-C
        // shape: the key moved under the read).
    ...
    {
        var (grain, _, cache) = CreateGrain();
        cache.GetAsync("k1").Returns(Task.FromException<byte[]?>(new InvalidOperationException("boom")));
    ...
    {
        var (grain, _, cache) = CreateGrain(optimisticReads: false);
        cache.GetAsync("k1").Returns(Bytes("v1"));
    ...
    {
        var (grain, _, cache) = CreateGrain(seedRoot: false);
    ...
    {
        var (grain, state, cache) = CreateGrain();
    ...
    [Test]
    public void TryGetOptimisticAsync_moved_away_slot_throws_stale_routing()
    {
        var (grain, state, cache) = CreateGrain();
        ...
        Assert.ThrowsAsync<StaleShardRoutingException>(() => grain.TryGetOptimisticAsync("k1"));
        cache.DidNotReceive().GetAsync(Arg.Any<string>());
    }

    [Test]
    public void TryGetOptimisticAsync_deleted_tree_throws()
    {
