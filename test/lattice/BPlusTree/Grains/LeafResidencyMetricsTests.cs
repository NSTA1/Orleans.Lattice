...
public sealed class LeafResidencyMetricsTests
{
    ...
    [Test]
    public void Every_residency_gauge_carries_the_platform_tenant_dimension()
    {
        // The working set is per-silo and spans every tree, so it has no tenant
        // to derive. That is a positive claim and has to be stated as one: a
        // bare Measurement<long> leaves the series with no tenant dimension at
        // all, which an operator cannot tell apart from a site that forgot to
        // attribute one. TenantMetricDimensionHygieneTests enforces this across
        // src/, but that gate scans text; this pins the tag that actually
        // reaches a listener.
        LeafResidencyMetrics.EnsureRegistered();
        ...
        using var listener = MeterListening.StartForMeter(
            ...
            l => l.SetMeasurementEventCallback<long>(
                (instrument, _, tags, _) =>
                {
                    ...
                    {
                        if (string.Equals(tag.Key, LatticeTenantLabel.TagTenant, StringComparison.Ordinal))
                        {
                            tenant = tag.Value as string;
                        }
                    }

                    tagged[instrument.Name] = tenant;
                }));

        listener.RecordObservableInstruments();

        Assert.That(tagged.Keys, Is.EquivalentTo(GaugeNames),
            "every residency gauge must report, or the tag assertion below covers fewer instruments than it appears to");
        Assert.That(tagged.Values, Is.All.EqualTo(LatticeTenantLabel.Platform.Value),
            "a per-silo gauge must carry the platform tenant sentinel, not an absent tenant dimension");
    }
