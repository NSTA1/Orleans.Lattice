...
/// <summary>
/// Unit tests for the internal resilient scan wrappers
/// <see cref="SystemLatticeScanExtensions.ScanKeysAsync"/> and
...
public class SystemLatticeScanExtensionsTests
{
    ...
    public async Task ScanKeysAsync_yields_all_keys_when_no_abort()
    {
        ...
        tree.KeysAsync(
            ...
            .Returns(_ => ScriptedKeys(["a", "b", "c"], abortAfter: int.MaxValue));

        var keys = new List<string>();
        await foreach (var k in tree.ScanKeysAsync()) keys.Add(k);

        Assert.That(keys, Is.EqualTo(new[] { "a", "b", "c" }));
    }

    [Test]
    public async Task ScanKeysAsync_resumes_after_abort_with_successor_key()
    {
        var tree = Substitute.For<ISystemLattice>();
        ...
        tree.KeysAsync(
            ...
            .Returns(ci =>
            {
                ...
                var idx = callIndex++;
                return idx == 0
                    ? ScriptedKeys(["a", "b"], abortAfter: 2)
                    : ScriptedKeys(["c"], abortAfter: int.MaxValue);
            });

        var keys = new List<string>();
        await foreach (var k in tree.ScanKeysAsync()) keys.Add(k);

        Assert.That(keys, Is.EqualTo(new[] { "a", "b", "c" }), "no duplicates and no gaps across the reopen");
    ...
    public async Task ScanKeysAsync_reverse_resumes_with_last_key_as_upper_bound()
    {
        ...
        tree.KeysAsync(
            ...
            .Returns(ci =>
            {
                ...
                var idx = callIndex++;
                return idx == 0
                    ? ScriptedKeys(["d", "c"], abortAfter: 2)
                    : ScriptedKeys(["b", "a"], abortAfter: int.MaxValue);
            });

        var keys = new List<string>();
        await foreach (var k in tree.ScanKeysAsync(reverse: true)) keys.Add(k);
    ...
    public void ScanKeysAsync_rethrows_after_max_attempts_exhausted()
    {
        ...
        tree.KeysAsync(
            Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<bool>(), Arg.Any<bool?>(), Arg.Any<CancellationToken>())
            .Returns(_ => ScriptedKeys([], abortAfter: 0));

        Assert.ThrowsAsync<EnumerationAbortedException>(async () =>
        {
            await foreach (var _ in tree.ScanKeysAsync(maxAttempts: 2))
            {
            }
        });
    }
    ...
    public void ScanKeysAsync_propagates_non_abort_exceptions_immediately()
    {
        ...
        tree.KeysAsync(
            Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<bool>(), Arg.Any<bool?>(), Arg.Any<CancellationToken>())
            .Returns(_ => ThrowAsync<string>(new InvalidOperationException("boom")));
    ...
    public void ScanKeysAsync_honors_cancellation_token()
    {
        ...
        tree.KeysAsync(
            Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<bool>(), Arg.Any<bool?>(), Arg.Any<CancellationToken>())
            .Returns(_ => ScriptedKeys(["a", "b"], abortAfter: int.MaxValue));
    ...
    [Test]
    public void ScanKeysAsync_throws_for_null_tree()
    {
    ...
    public async Task ScanKeysAsync_forwards_prefetch_and_initial_bounds()
    {
        ...
        tree.KeysAsync(
            ...
            .Returns(ci =>
            {
                ...
                return ScriptedKeys(["m"], abortAfter: int.MaxValue);
            });

        var keys = new List<string>();
        await foreach (var k in tree.ScanKeysAsync("k", "z", prefetch: false)) keys.Add(k);
    ...
    public void ScanKeysAsync_negative_maxAttempts_is_clamped_to_zero()
    {
        ...
        tree.KeysAsync(
            ...
            {
                callIndex++;
                return ScriptedKeys([], abortAfter: 0);
            });

        Assert.ThrowsAsync<EnumerationAbortedException>(async () =>
        {
            await foreach (var _ in tree.ScanKeysAsync(maxAttempts: -5))
            {
            }
        });
