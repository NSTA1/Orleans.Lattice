...
/// implementation so Orleans's TestCluster grain-type discovery does not
/// pick up the test type as a competing implementation of the
/// system-lattice grain interface.
/// </summary>
internal static class FakeSystemLattice
{
    /// <summary>
    /// Creates a fresh in-memory store and the <see cref="ISystemLattice"/>
    /// substitute that operates on it. Returns both so tests can inspect the
    /// underlying data.
    /// </summary>
