...
/// <summary>
/// Value-equality regression tests for <see cref="RgaDeltaNode"/>. Its
/// <see cref="RgaDeltaNode.Value"/> byte array was compared by reference under
/// the compiler-generated record-struct equality, so two nodes built from
/// independently allocated but byte-identical values - including a node and its
/// post-serialization self - never compared equal.
/// </summary>
[TestFixture]
public sealed class RgaDeltaNodeEqualityTests
{
    private static RgaDeltaNode Sample(byte[]? value = null) => new()
    {
        ReplicaId = "replica-a",
        Counter = 7L,
        ParentDot = new OrSetDot { ReplicaId = "replica-b", Counter = 3L },
        Value = value ?? [1, 2, 3],
    };
    ...
    public void Not_equal_when_a_scalar_field_differs()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(a.Equals(a with { ReplicaId = "replica-z" }), Is.False);
            Assert.That(a.Equals(a with { Counter = 8L }), Is.False);
            Assert.That(
                a.Equals(a with { ParentDot = new OrSetDot { ReplicaId = "replica-b", Counter = 4L } }),
                Is.False);
        });
    }
    ...
    public void Serialization_round_trip_preserves_value_equality()
    {
        ...
        var serializer = services.GetRequiredService<Serializer<RgaDeltaNode>>();
