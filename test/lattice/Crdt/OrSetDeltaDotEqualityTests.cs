...
/// <summary>
/// Value-equality regression tests for <see cref="OrSetDeltaDot"/>. Its
/// <see cref="OrSetDeltaDot.Element"/> byte array was compared by reference
/// under the compiler-generated record-struct equality, so two dots built from
/// independently allocated but byte-identical elements - including a dot and
/// its post-serialization self - never compared equal.
/// </summary>
[TestFixture]
public sealed class OrSetDeltaDotEqualityTests
{
    private static OrSetDeltaDot Sample(byte[]? element = null) => new()
    {
    ...
    {
        var a = Sample() with { Element = null! };
        var b = Sample() with { Element = null! };
    ...
    public void Serialization_round_trip_preserves_value_equality()
    {
        ...
        var serializer = services.GetRequiredService<Serializer<OrSetDeltaDot>>();
