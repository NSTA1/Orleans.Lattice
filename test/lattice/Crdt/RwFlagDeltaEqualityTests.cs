...
/// <summary>
/// Value-equality regression tests for <see cref="RwFlagDelta"/>. Its
/// <see cref="RwFlagDelta.Enables"/>, <see cref="RwFlagDelta.Disables"/>, and
/// <see cref="RwFlagDelta.Tombstones"/> lists were compared by reference under
/// the compiler-generated record-struct equality - even though the contained
/// <see cref="OrSetDot"/> already compares by value - so two deltas built from
...
public sealed class RwFlagDeltaEqualityTests
{
    ...
    {
        var a = Sample();
        var b = a with { Tombstones = [Dot("r2", 6)] };
    ...
    {
        var a = RwFlagDelta.Empty;
        var b = new RwFlagDelta { Enables = [], Disables = [], Tombstones = [] };
    ...
    {
        var a = default(RwFlagDelta);
        var b = default(RwFlagDelta);
    ...
    public void Serialization_round_trip_preserves_value_equality()
    {
        ...
        var serializer = services.GetRequiredService<Serializer<RwFlagDelta>>();
