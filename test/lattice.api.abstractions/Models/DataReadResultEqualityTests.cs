...
/// <summary>
/// Value-equality regression tests for <see cref="DataReadResult"/>, the point-read
/// result surfaced by <c>ILatticeDataApi.GetAsync</c>. Its
/// <see cref="DataReadResult.Value"/> byte array was compared by reference under the
/// compiler-generated record equality, so two structurally identical results -
/// including a result and its post-serialization self - never compared equal.
/// </summary>
[TestFixture]
public sealed class DataReadResultEqualityTests
{
    private static DataReadResult Sample(byte[]? value = null) => new()
    {
        TreeId = "tree",
        Key = "k",
        Found = true,
    ...
    public void Serialization_round_trip_preserves_value_equality()
    {
        ...
        var serializer = services.GetRequiredService<Serializer<DataReadResult>>();
