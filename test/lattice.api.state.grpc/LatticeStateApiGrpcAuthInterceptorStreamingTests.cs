...
/// <summary>
/// Behavioural regression coverage proving <see cref="LatticeStateApiGrpcAuthInterceptor"/>
...
/// </summary>
[TestFixture]
public sealed class LatticeStateApiGrpcAuthInterceptorStreamingTests
{
    private const string LatticeMethod =
        "/" + LatticeStateGrpcMethods.ServiceName + "/" + LatticeStateGrpcMethods.ListTreesMethodName;
    ...
    {
        var options = Substitute.For<IOptionsMonitor<LatticeStateApiGrpcOptions>>();
        options.CurrentValue.Returns(new LatticeStateApiGrpcOptions
        {
        ...
        return new LatticeStateApiGrpcAuthInterceptor(
            authorizer,
            options,
            NullLogger<LatticeStateApiGrpcAuthInterceptor>.Instance);
    }

    private static ILatticeStateApiAuthorizer Authorizer(bool allow)
    {
        var authorizer = Substitute.For<ILatticeStateApiAuthorizer>();
        authorizer
            .IsAuthorizedAsync(Arg.Any<LatticeStateApiAuthorizationContext>(), Arg.Any<CancellationToken>())
            .Returns(allow);
        return authorizer;
    }
    ...
    public void A_denied_server_streaming_call_is_rejected_with_permission_denied()
    {
        ...
        var exception = Assert.ThrowsAsync<RpcException>(() => interceptor.ServerStreamingServerHandler<object, string>(
            ...
            new StateGrpcCallContext(LatticeMethod),
            (_, _, _) =>
            {
                reached = true;
    ...
    public async Task A_foreign_server_streaming_method_is_passed_through_without_an_auth_check()
    {
        ...
        await interceptor.ServerStreamingServerHandler<object, string>(
            new object(),
            new DiscardingStreamWriter<string>(),
            new StateGrpcCallContext(ForeignMethod),
            (_, _, _) =>
            {
                reached = true;
                return Task.CompletedTask;
        ...
        await authorizer.DidNotReceive().IsAuthorizedAsync(
            Arg.Any<LatticeStateApiAuthorizationContext>(),
    ...
    {
        var interceptor = Create(Authorizer(allow: true));
        var context = new StateGrpcCallContext(LatticeMethod);
