...
/// request header into a <see cref="LatticeCredential"/> for the state-API read
/// path (issue #981). Proves scheme stripping, casing tolerance, the
/// custom-header / custom-scheme knobs, and the fail-closed null results for an
/// absent, blank, or scheme-only header - the anonymous cases the auth-backed
/// visibility filter denies.
/// </summary>
[TestFixture]
public sealed class HeaderLatticeStateApiCredentialBridgeTests
{
    private static HeaderLatticeStateApiCredentialBridge CreateBridge(
        LatticeStateApiGrpcOptions? options = null) =>
        new(Options.Create(options ?? new LatticeStateApiGrpcOptions()));
