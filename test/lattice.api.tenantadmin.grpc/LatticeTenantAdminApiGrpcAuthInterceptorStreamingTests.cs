...
/// <summary>
/// Behavioural regression coverage proving <see cref="LatticeTenantAdminApiGrpcAuthInterceptor"/>
...
/// </summary>
[TestFixture]
public sealed class LatticeTenantAdminApiGrpcAuthInterceptorStreamingTests
{
    private const string LatticeMethod =
        "/" + LatticeTenantAdminGrpcMethods.ServiceName + "/" + LatticeTenantAdminGrpcMethods.CreateTenantMethodName;

    private const string ForeignMethod = "/some.other.service/Ping";

    private static LatticeTenantAdminApiGrpcAuthInterceptor Create(
        ILatticeTenantAdminApiAuthorizer authorizer,
        bool requireAuthorization = true)
    {
        var options = Substitute.For<IOptionsMonitor<LatticeTenantAdminApiGrpcOptions>>();
        options.CurrentValue.Returns(new LatticeTenantAdminApiGrpcOptions
        {
    ...
    private static ILatticeTenantAdminApiAuthorizer Authorizer(bool allow)
    {
    ...
    public async Task A_foreign_server_streaming_method_is_passed_through_without_an_auth_check()
    {
        ...
        await authorizer.DidNotReceive().IsAuthorizedAsync(
            Arg.Any<LatticeTenantAdminApiAuthorizationContext>(),
