...
/// <summary>
/// Guards the single-namespace contract for the replication package: every public
/// type in <c>Orleans.Lattice.Replication</c> must live in that exact root namespace
/// so the whole public surface sits behind a single <c>using Orleans.Lattice.Replication;</c>.
/// </summary>
[TestFixture]
public class PublicApiNamespaceTests
{
    ...
    {
        const string root = "Orleans.Lattice.Replication";
        var assembly = typeof(LatticeReplicationOptions).Assembly;
