...
public sealed class CompiledReplicationConfigSnapshotMaintainerTests
{
    ...
    private static Task<bool> WaitForEpochAtLeast(
        CompiledReplicationConfigSnapshotMaintainer maintainer, long target, int timeoutMs = 5000) =>
        TestPoll.TryUntilAsync(
            () => maintainer.CurrentEpoch >= target, TimeSpan.FromMilliseconds(timeoutMs));
    ...
    /// </summary>
    private static async Task<long> WaitForEpochToSettle(
    ...
    public async Task OnMutationAsync_on_an_unrelated_tree_does_not_rebuild()
    {
        ...
        Assert.That(await WaitForEpochAtLeast(maintainer, controlEpoch + 1), Is.True,
            "positive control: a config-tree mutation must reach the rebuild pump");
