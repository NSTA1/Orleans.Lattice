...
public sealed class RepoTreeWalkerTests
{
    ...
    [Test]
    public void Walk_keeps_build_output_pruned_under_a_re_included_directory()
    {
        // The repository's own .gitignore has exactly this shape: a broad rule, the
        // conventional build-output rules, and a negation re-including one real
        // source directory. The build output beneath the re-included directory must
        // stay pruned, because it is regenerated by every build and so can never
        // converge - it would pin the whole repository's gap-scan cadence and inflate
        // the walked-file denominator every other figure is normalised against.
        Write(".gitignore", "[Bb]in/\n[Oo]bj/\nBackup*/\n!/src/app/Backups/\n");
        Write("src/app/Backups/Plugin.cs", "code");
        Write("src/app/Backups/bin/Debug/net10.0/Plugin.deps.json", "{}");
        Write("src/app/Backups/obj/project.assets.json", "{}");
        Write("src/app/Other/bin/Debug/net10.0/Other.deps.json", "{}");
        Write("src/app/Other/Other.cs", "code");

        var entries = RepoTreeWalker.Walk(_root, null, null, respectGitignore: true);

        Assert.That(
            entries.Select(e => e.RelativePath),
            Is.EqualTo(new[] { ".gitignore", "src/app/Backups/Plugin.cs", "src/app/Other/Other.cs" }));
    }
