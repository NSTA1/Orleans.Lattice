...
/// <summary>
/// Behavioural regression coverage proving <see cref="LatticeSchemaApiGrpcAuthInterceptor"/>
...
/// </summary>
[TestFixture]
public sealed class LatticeSchemaApiGrpcAuthInterceptorStreamingTests
{
    private const string LatticeMethod =
        "/" + LatticeSchemaGrpcMethods.ServiceName + "/" + LatticeSchemaGrpcMethods.SetPolicyMethodName;

    private const string ForeignMethod = "/some.other.service/Ping";

    private static LatticeSchemaApiGrpcAuthInterceptor Create(
        ILatticeSchemaApiAuthorizer authorizer,
        bool requireAuthorization = true)
    {
        var options = Substitute.For<IOptionsMonitor<LatticeSchemaApiGrpcOptions>>();
        options.CurrentValue.Returns(new LatticeSchemaApiGrpcOptions
        {
            RequireAuthorization = requireAuthorization,
        });
        return new LatticeSchemaApiGrpcAuthInterceptor(
            authorizer,
            options,
            NullLogger<LatticeSchemaApiGrpcAuthInterceptor>.Instance);
    }

    private static ILatticeSchemaApiAuthorizer Authorizer(bool allow)
    {
        var authorizer = Substitute.For<ILatticeSchemaApiAuthorizer>();
        authorizer
            .IsAuthorizedAsync(Arg.Any<LatticeSchemaApiAuthorizationContext>(), Arg.Any<CancellationToken>())
            .Returns(allow);
        return authorizer;
    }
    ...
    public async Task A_foreign_server_streaming_method_is_passed_through_without_an_auth_check()
    {
        ...
        await authorizer.DidNotReceive().IsAuthorizedAsync(
            Arg.Any<LatticeSchemaApiAuthorizationContext>(),
