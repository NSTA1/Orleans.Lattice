...
### FX-006 — `CancellationToken` on `ILattice` *(audit #19 — API / breaking — deferred)*
Still outstanding. Public `ILattice` methods don't accept `CancellationToken`; long operations (scans, merges, range deletes) can't be cooperatively cancelled. Fix: add `CancellationToken = default` optional parameters to the public surface. Marked **breaking** for binary compatibility even though source-compatible; staged behind a major-version bump.
...
`LatticeGrain.CountAsyncCore` and `CountPerShardAsyncCore` now participate in the F-032 per-slot reconciliation path (bounded by `LatticeOptions.MaxScanRetries`), so they reduce counts from a consistent shard-map snapshot rather than double-counting keys in migrating virtual slots during the split settle window. Regression tests: `CountAsyncSplitRegressionTests.CountAsync_matches_true_live_count_after_split` and `CountPerShardAsync_matches_true_live_count_after_split` (400 keys / 4 shards / mid-scan split commit).
