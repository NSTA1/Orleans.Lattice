...
- **F-019c — Registry-authoritative structural sizing**. Removes the `MaxLeafKeys`, `MaxInternalChildren`, and `ShardCount` knobs from `LatticeOptions` entirely so they can never silently diverge from the layout a tree was built with. Canonical defaults move to `LatticeConstants` (`DefaultMaxLeafKeys = 128`, `DefaultMaxInternalChildren = 128`, `DefaultShardCount = 64`) and are seeded into the `TreeRegistryEntry` on first tree creation. After seeding, the registry is the only source of structural truth: `ShardRootGrain`, `BPlusLeafGrain`, and `BPlusInternalGrain` all read through `LatticeOptionsResolver`, which asserts the pin is present and throws if the tree was not seeded. `ResizeAsync` and `ReshardAsync` remain the only mutation paths for the pinned fields, and both gain an empty-tree fast-path that atomically updates the registry pin without activating the coordinator machinery; `ReshardAsync`'s grow-only restriction is relaxed when the tree is empty so callers who want non-default sizing (including test fixtures) simply call `ResizeAsync` and/or `ReshardAsync` on the freshly-created tree. System trees (e.g. `_lattice_trees`) are seeded from the same constants rather than special-cased, closing the system-tree corruption surface that FX-013's monitor-fallback design left open. Pre-production breaking change: callers that previously set `LatticeOptions.MaxLeafKeys` / `MaxInternalChildren` / `ShardCount` must migrate to calling `ResizeAsync` / `ReshardAsync` on the new tree. FX-013's registry-first resolver, drift-throw, and "pin wins over monitor" rules become moot once the monitor no longer exposes the fields.
