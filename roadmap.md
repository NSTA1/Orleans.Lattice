...
### 1 · F-032 — Scan ordering preservation under topology change
**Correctness / highest priority**

`KeysAsync` and `EntriesAsync` guarantee lexicographic ordering during stable topology. When a shard split commits mid-scan, pass-2 reconciliation recovers the complete key set (using a `HashSet<string>` deduplicator) but appends reconciled entries to the output stream in unspecified order rather than merging them back into the sorted sequence. For callers who depend on sorted output — the primary behavioural contract of a B+ tree scan — this is a correctness gap. `CountAsync` is not affected (it reduces rather than streams).

**Proposed ordering fix:** merge pass-2 results back into the k-way merge channel before yielding resumes, either by (a) buffering the reconciled set in memory and inserting it into the priority queue (single-pass, bounded extra memory), or (b) re-running pass 1 from scratch using a stable `ShardMap.Version` snapshot once reconciliation is needed (double-latency on splits, no extra buffer). Neither option removes the `MaxScanRetries` liveness bound — exhaustion still throws on sustained split storms. The complete fix for liveness requires G-003 (stateful cursor grain).

The default of 3 is well-calibrated for the default split configuration: `MaxConcurrentAutoSplits` (default 2) caps how many topology changes can be in flight simultaneously **per tree** — `HotShardMonitorGrain` is keyed per-tree so the counter covers only that tree's physical shards — and `HotShardSplitCooldown` (default 2 minutes) prevents the same shard from re-splitting rapidly. Crucially, each retry re-fetches only the virtual slots that actually moved during the scan — cost is O(moved slots), not O(all shards) — so retries are cheap. The in-memory cooldown dictionary on `HotShardMonitorGrain` is the main realistic exposure: it is lost on grain deactivation, so after a silo restart a newly-split target shard (which was never tracked in the dictionary) could immediately qualify for another split. Even then, `MaxConcurrentAutoSplits` is the hard backstop per tree.
...
### 6 · F-031 — Atomic multi-key writes (saga)
**Feature / high impact**
...
All management operations (force split, run diagnostics, inspect shard map, trigger snapshot) require in-process code or test harnesses. A `dotnet tool` wrapping the `ILatticeAdmin` / `ITreeShardSplitGrain` surfaces would lower the operational bar significantly — particularly for diagnosing concurrent split activity or stale cache behaviour described in F-022.
...
| F-032 (Scan ordering under topology change) | F-011 (Adaptive shard splitting) | 🔲 Ordering gap is introduced by F-011's pass-2 reconciliation path |
| F-032 (Scan ordering under topology change) | G-003 (Stateful cursor grain) | 🔲 G-003 is the complete liveness fix; F-032 addresses ordering independently |
