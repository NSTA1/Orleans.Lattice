...
**Status: largely delivered.** Both halves of F-019 are live:
...
- **Online `ResizeAsync`** (shipped, phases 1–3 complete) — `ILattice.ResizeAsync(int newMaxLeafKeys, int newMaxInternalChildren)` now runs online. A new `ShardRootGrain` shadow-forwarding primitive (`BeginShadowForwardAsync` / `MarkDrainedAsync` / `EnterRejectingAsync` / `ClearShadowForwardAsync`, LWW-commutative by construction) lets `TreeSnapshotGrain` run a strictly-consistent `SnapshotMode.Online` drain while live traffic is mirrored to the destination physical tree. `TreeResizeGrain` adds a `ResizePhase.Reject` transition, a dual-window `UndoResizeAsync` (before-swap vs after-swap), and threads a `logicalTreeId` through `SnapshotWithOperationIdAsync` so the transient per-shard `StaleTreeRoutingException` at swap carries the caller's logical name. `HotShardMonitorGrain` polls `ITreeResizeGrain.IsCompleteAsync`; `ReshardAsync` rejects while a resize is running. See [Online Resize](docs/online-resize.md).

**Outstanding.** Phase 4 — a dedicated chaos test modelled on the reshard chaos suite (3 writers + 2 readers + 1 scanner + 1 counter racing against a resize that halves `MaxLeafKeys`/`MaxInternalChildren`, universe pinned, verifying zero envelope violations and final `Count == UniverseSize`). The online-resize design doc has been promoted to "phases 1–3 ✅"; no API-level work remains for `ResizeAsync` itself. A future `MergeAsync`-style shrink primitive and cross-cluster replication over the shadow-forward primitive are separate features (see §11 of the design doc).
