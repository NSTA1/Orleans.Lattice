...
**Status: partially delivered.** `ILattice.ReshardAsync(int newShardCount, CancellationToken)` grows a tree's physical shard count online — a new `ITreeReshardGrain` coordinator (keyed per tree, `[EditorBrowsable(Never)]`, guarded by `InternalGrainGuardFilter`) inspects the current `ShardMap`, picks the largest-slot-owning eligible physical shards (those owning ≥ 2 virtual slots and not already splitting), and dispatches up to `LatticeOptions.MaxConcurrentMigrations` (default 4) concurrent `ITreeShardSplitGrain.SplitAsync` calls per tick. Each underlying split atomically grows the map by one distinct physical shard via its own shadow-write + swap + reject phases (F-011), so the tree continues to serve reads and writes throughout with no global cutover lock. Grow-only — target must be strictly greater than the current distinct-shard count and ≤ `LatticeOptions.VirtualShardCount`; shrink is not supported. Idempotent for repeated calls with the same target while in progress; `InvalidOperationException` when a different target is already running. Crash-safe: reminder-anchored (`reshard-keepalive`, 1 min) with a 2 s grain timer driving the phase machine (`Planning` → `Migrating` → `Complete`). Progress observable via `ILattice.IsReshardCompleteAsync(CancellationToken)` and interlocked against the autonomic split monitor (`HotShardMonitorGrain` suppresses its own passes while a reshard is in progress). Shrink and the existing offline `ResizeAsync` tree-sizing path remain outstanding — an online tree-sizing variant (the second half of F-019) is the candidate follow-up.
