...
2. **Three ways in**, Build, Evaluate, and Operate, each a chain of pages in
   reading order. These names are fixed, and the video series uses them too.
3. **One programming model, Local to Global**: the three deployment stages on a
   single chain, closed by the invariant `ILattice`.
4. **A core plus seams**: one node per section of `PACKAGES.md`, generated by
   `stage.ps1`, so a new package appears without an edit here.
5. A short close pointing to the overview and the capability catalogue.
