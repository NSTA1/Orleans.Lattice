...
| **State model** | WAL is canonical; the leaf state row holds topology, checkpoints, and outstanding replay work, never per-key entries; CRDT keys use delta-only producer-side mutation. | [State Model](docs/lattice/state-model.md) | n/a |
| **Write-ahead log storage** | The write-ahead log is the durability boundary for every committed write, behind the pluggable `IWalStorageProvider` seam: an in-memory default in the core for tests and samples, a durable local-disk log per shard for single-node and container deployments, durable Azure Table Storage for production clusters, replicated or not, or a backend of your own. | [WAL Storage Providers](docs/lattice/wal-storage-providers.md) - [File WAL](docs/lattice.storage.file/README.md) - [Azure Table WAL](docs/lattice.storage.azuretable/README.md) | n/a |
| **WAL saturation signal** | A per-tree, three-state (healthy, throttled, saturated) back-pressure signal through the public `IWalSaturationSignal` and `IWalSaturationObserver`, so callers can throttle offered load before it queues silently at the WAL writer's admission gate. | [WAL Saturation Signal](docs/lattice/wal-saturation-signal.md) - [WAL Tuning](docs/lattice/wal-tuning.md) | n/a |
...
| **Per-entry TTL** | Per-entry time-to-live with absolute server-side expiry, on last-writer-wins `SetAsync` writes and on typed CRDT writes, preserved verbatim across splits, snapshots, resize, and replication. | [TTL](docs/lattice/ttl.md) | [sample](samples/Ttl/README.md) |
...
| **Compression** | Replication's wire framing and the Azure Table WAL backend's larger payloads are compressed with dictionary-less Zstandard by default, and a host can plug in its own algorithm through the public `ILatticeCompressor` seam, registered with `AddLatticeCompressor` under a host-reserved one-byte tag. | [Compression](docs/lattice/compression.md) | n/a |
...
| Feature | What it gives you | Docs | Sample |
|---|---|---|---|
| **Schema enforcement & versioning** | Opt-in companion package that validates every local write against a per-tree policy (JSON, UTF-8, size, regex, or a structured predicate), rejecting a non-compliant one, and stamps values with a self-describing schema version, upcasting stale values to the current version on read. With strict ingest enabled, a non-compliant replicated or restored item is dead-lettered rather than applied; existing data migrates via a crash-safe shadow-build. Zero overhead when unused. | [Schema](docs/lattice.schema/README.md) | [sample](samples/SchemaEnforcement/README.md) |
| **Multi-tenancy** | Opt-in tenant isolation across a single-cluster or multi-cluster deployment: a keyspace-partitioned tenant per `t/{tenant}/` prefix, a tenant registry with a create / suspend / resume / delete lifecycle, per-tenant quotas / metering / rate limiting, optional per-tenant region residency, and a fail-closed operator control-plane facade reachable in-process, over gRPC, or as MCP tools. Turned on by adding the companion packages, with no change to the core tree. | [Multi-tenancy](docs/lattice.tenancy/README.md) | [sample](samples/MultiTenancy/README.md) |
| **Tenant administration** | Two console surfaces over the tenancy control API, split by privilege: platform operators manage tenant lifecycle, quota, region authorization, admin subjects and cross-tenant grants; tenant administrators manage their own membership, cross-tenant grants and region residency, and see usage against a quota they cannot change. | [Tenant admin API](docs/lattice.api.tenantadmin/README.md) | [sample](samples/MultiTenancy/README.md) |
...
Running a cluster: backup, autoscaling, the external and administration APIs, the observability surfaces, and the operator console.
...
| **External state and data APIs** | Transport-agnostic facades with code-first gRPC bindings for clients outside the cluster: a read-only state API to query, observe, and subscribe to trees, structure, entries, change feeds, and metrics, and a write-capable data API for point sets and deletes, point and bounded-range reads, and single- and cross-tree atomic batches from non-.NET clients, both default-deny over gRPC. | [State API](docs/lattice.api.state/README.md) - [Data API](docs/lattice.api.data/README.md) | [sample](samples/StateExplorer/README.md) |
| **Tree administration** | One fail-closed control facade for whole-tree administration, reachable in-process, over gRPC, or as MCP tools: create, inspect, and reconfigure trees; aliases; delete, recover, and purge; bulk load; restore and revert; reshard, resize, and snapshot; WAL placement and movement; materialised-view and tag-index management; shard compaction; history retention; and orphaned-leaf audit and repair - each authorized fail-closed on the server, with an advisory per-operation capability probe. | [Tree admin API](docs/lattice.api.treeadmin/README.md) | n/a |
...
| **Metrics** | `System.Diagnostics.Metrics` instruments published on the `orleans.lattice` meter, ready for OpenTelemetry subscription, with bundled Grafana dashboards for the Lattice meters in a companion package. | [Metrics](docs/lattice/metrics.md) - [Dashboards](docs/lattice.dashboards/README.md) | [sample](samples/Metrics/README.md) |
...
| **Repo-context MCP** | An opt-in MCP tools package that gives an AI agent durable, conflict-free context and memory about a codebase: bootstrap onboarding of a repository through resumable background indexing that reports its progress, structural and symbol recall, free-form memories with optional TTL, semantic search with explainable ranking (approximate nearest-neighbour by default, or an exact scan when configured), structural graph navigation, and a budgeted context bundle with reuse economics and usage accounting - all stored in the CRDT B+ tree and served fail-closed to the authorized agent, packaged for local use as a dedicated Docker container. | [Repo-context MCP](docs/lattice.api.mcp.repocontext/README.md) | [sample](samples/RepoContextContainer/README.md) |
...
| **Change history** | Per-key revision timeline for both tree shapes: successive values (with diffs) for last-writer-wins keys and decoded element-level member changes for CRDT keys. Read it from the core `ScanEntryHistoryAsync`, the read-only State API, or the History view behind the Explorer Data tab's History button, with live-follow. Served from a durable, retention-bounded history view when enabled, or a best-effort retained-WAL-window fallback otherwise. | [Change History](docs/lattice/change-history.md) | [sample](samples/ChangeHistory/README.md) |
...
| **Vector search** | Approximate nearest-neighbour search over vectors held in a tree, with query cost sub-linear in the corpus instead of proportional to it. The index is persisted on a Lattice tree and loaded back in bounded chunks, so a restart reloads it rather than rebuilding it, and a lazily opened index warms as it serves. An interrupted build resumes from its last durable checkpoint, and a corrupt or version-incompatible index is rebuilt rather than served. Publishes a measured recall target and reports per query whether an approximate or an exact path answered. | [Vector](docs/lattice.vector/README.md) | [sample](samples/VectorSearch/README.md) |
