...
- **Credential isolation by construction.** The backend client's only collaborators are an `HttpClient`, the telemetry options, the optional backend-token provider the `DynamicBearer` mode draws on, and a server-side logger; none of them is a Lattice credential source, so the caller's identity cannot leak to the backend.
- **Fail-clean.** A backend timeout, HTTP failure, non-success status, malformed payload, guardrail rejection, or metric-access denial is returned as a structured result with `Success = false` and a human-readable `Error`, never as an unhandled exception. A backend transport or payload fault reports a fixed message rather than the caught exception's text, so the backend credential cannot ride out on an error string; the detail is logged server-side. The one deliberate exception is the metadata tool: a `404` from the backend metadata endpoint is read as "no metadata surface", so it degrades to `Success = true` with an empty `Metrics` list rather than a failure. A genuine caller cancellation still propagates.
