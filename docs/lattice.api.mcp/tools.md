...
This module is served in-silo only. Its backing `ILatticeSchemaControl` facade has no gRPC client referenced by `Orleans.Lattice.Api.Mcp`, so the remote (out-of-silo) topology cannot wire it and every `lattice_treeadmin_schema_*` tool is deferred there (omitted from a remote session's tool set rather than listed-then-erroring). When a schema gRPC client is added and referenced, the remote host can wire the facade and the tools become discoverable remotely with no other change.
