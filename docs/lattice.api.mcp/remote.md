...
| `TelemetryAvailable` | When `true`, the head-local telemetry group is advertised for the **current** region in `lattice_list_regions`, keeping that report consistent with `lattice_capabilities`. Telemetry is a head-local PromQL proxy with no per-region gRPC endpoint, so it is never advertised for peer regions. Set it when the host wires the telemetry tools (`AddTelemetryTools`). Defaults to `false`. |
