...
Because the read path and the reported shard tail are both derived from commit metadata, this option introduces a bounded visibility lag: the trailing batch on a shard is durable when the append returns but is not readable until its completion lands, so a read can omit it and the reported highest offset can lag. Callers that need read-after-write should await the provider's phase-two flush barrier rather than sleeping or polling; see [Architecture](architecture.md#visibility-lag-under-pipelining). Set the option to `false` if you would rather pay the latency on every append than take a barrier where you need one.
