...
| Invariant | Meaning |
|---|---|
| **All-or-nothing append** | `AppendBatchAsync` is atomic per call. Either every entry in the supplied list is durably persisted before the returned task completes, or none of them are. A backend that cannot satisfy this for a particular batch (e.g. a multi-partition write on a backend without cross-partition transactions) must reject the batch at validation time rather than silently fragmenting it. |
...
Durable Azure Table Storage implementation shipped in the optional `Orleans.Lattice.Storage.AzureTable` NuGet package. Translates each `AppendBatchAsync` call into a single `TableClient.SubmitTransactionAsync` within the target partition.
...
- **Per-call allocations**: the partition key is built per call (UTF-8 encode + percent-encode + concatenate). The hot path uses `ArrayBufferWriter<byte>` for the per-entry payload and a single `List<TableTransactionAction>` sized to `entries.Count + 1` for the batch.
