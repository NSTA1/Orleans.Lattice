...
**Observability.** An abandoned drive increments `orleans.lattice.wal.replay.starvation_drive_abandonments` (tagged `tree`), zero-primed per tree beside the sweep's `attempted` arm so an absent series proves the build is not deployed rather than that no drive has run, and emits a warning log carrying the leaf, the tree, the budget and how long the drive actually ran. The warning takes one of two forms, split at the moment the drive acquired its replay permit, because the two have opposite remedies (issue #3479). A drive that **never acquired a permit** spent its whole budget queued on the per-silo replay gate: the warning reports the wait, the gate's permit ceiling and the waiters still queued, and points at `orleans.lattice.wal.replay.permits_queued` and `orleans.lattice.wal.replay.permit_queue_wait`, since this is permit contention and neither a larger budget nor the storage provider will help. A drive that **was replaying** reports the time it waited for the permit, the time it spent replaying, and how far the replay advanced the leaf's checkpoint (in memory and persisted) before it was abandoned; only this form points at the storage provider and the leaf's replay gap. It also records the `drove_timed_out` verdict on `orleans.lattice.wal.gc.blocked_leaf_reactivations` - but that arm is near-silent in practice, because the caller has usually already timed out, so read the counter and not the verdict. See [Metrics](metrics.md).
