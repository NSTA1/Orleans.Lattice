...
- **No commit-path change.** VC capture happens at the existing commit-time observer site. The atomic-transaction boundary is a metadata signal on `LatticeMutation`, not a new commit phase.
...
- **Per-shard apply.** The shadow-forward receiver-side dedupe cache and the multi-shard producer-side VC cache are both per-shard; no cross-shard locks.
