...
| `AtomicApplyEntry` | `ol.aae` | internal | `readonly record struct` carrying the per-entry source metadata (Key, Value, Timestamp, ExpiresAtTicks, VectorClock, IsTombstone) for the cross-cluster atomic-batch apply seam on `IReplicationApplyGrain.ApplyManyAtomicAsync`. |
