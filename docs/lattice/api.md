...
| `TxRegistryShardCount` | `int` | 1 | Saga decision registry shards new sagas are minted across, per tree (1 to 256). Each shard has its own row and admission budget, so the per-tree saga ceiling scales with it. Opt-in: raise only once every silo and replication peer runs a sharding-aware version. Safe to change live in either direction. Global (unnamed options) only. See [Configuration](configuration.md#txregistryshardcount). |
