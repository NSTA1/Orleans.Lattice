...
| `TxRegistryAdmissionBudgetBytes` | `long?` | 768 KiB | Row-size budget above which the per-tree transaction registry refuses new atomic-write sagas with a retryable `LatticeSaturatedException` (`TxRegistryCapacity`). `null` disables the bound. See [Configuration](configuration.md#txregistryadmissionbudgetbytes). |
