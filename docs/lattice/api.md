...
| `StorageUsagePollInterval` | `TimeSpan` | 15 s | Cadence at which every silo's background poller calls `ILatticeAdmin.GetTotalStorageUsageAsync` so the byte-accurate storage gauges populate without any caller invoking the public API. Global knob read from the default (unnamed) options; per-tree overrides do not apply. `TimeSpan.Zero` or a negative value disables the poller (the gauges then populate only when the API is called). |
