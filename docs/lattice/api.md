...
| `LatticeSaturatedException` | `ol.lsa` | public | Typed `InvalidOperationException` subclass raised by four seams - the WAL writer admission gate, the atomic-write saga coordinator, the snapshot-cursor open path, and the per-silo replay-permit admission gate - when an operation is refused because the tree's storage layer is back-pressured. Carries the originating `TreeId` and a `SaturationSource` naming the refusing seam. See [Saturation back-pressure](#saturation-back-pressure---latticesaturatedexception) and [WAL Saturation Signal](wal-saturation-signal.md). |
