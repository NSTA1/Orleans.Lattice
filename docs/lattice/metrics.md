...
| `orleans.lattice.leaf.projection_accept_loss_rebuilds` | `Counter<long>` | `{rebuild}` | Activation-time accept-loss projection rebuilds: a leaf partition on a tree configured with `ProjectionRebuildPolicy.RebuildFromWalAcceptLoss` fell off the shared WAL and rebuilt from the surviving suffix, discarding the trimmed prefix, instead of throwing `LeafProjectionStaleException`. Each increment is an expected data-loss event on a derived, re-derivable tree; any occurrence on a store-of-record tree is a misconfiguration alarm. Tagged `tree`, `shard`. See [Projection Rebuild](projection-rebuild.md). |
