...
| `orleans.lattice.compaction.leaves.visited` | `Counter<long>` | `{leaf}` | Per-leaf compaction outcome. Tagged `tree`, `outcome=reaped` (at least one tombstone physically removed) or `outcome=noop` (leaf had pending tombstones still inside the grace window), and - when a policy-trigger pass is in flight - `trigger=reminder|ratio|size|operator`. |
