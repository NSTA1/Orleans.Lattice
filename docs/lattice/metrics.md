...
| `orleans.lattice.leaf.activation.failures` | `Counter<long>` | `{activation}` | Leaf activations that ended by **throwing** out of `OnActivateAsync` rather than coming online (issue #2280). Tagged `tree`, `activation_temperature` and `reason`: `canceled` when a replay already in progress was cancelled, `canceled_awaiting_permit` when the cancellation arrived while the activation was still queued for the replay concurrency permit and no replay had begun, and `faulted` for any other failure. The two cancellation arms are kept apart deliberately - one lost work in flight, the other never started - so a rise in queueing cannot masquerade as a rise in abandoned replays. This counter exists because `orleans.lattice.leaf.deactivation.checkpoint_delta` is structurally blind to this population: a failed activation never runs the deactivation hook, so without it a cancelled cold replay would read as zero at every rate of occurrence including the highest, an absence indistinguishable from health. **Also a lower bound**, though a tight one: the permit acquisition sits inside the guarded region so the queue wait is covered, but a failure raised before that region is entered - during the snapshot rehydrate or the coherence reset - is not counted, and an outright process kill reaches no observation site anywhere. |
| `orleans.lattice.leaf.snapshot.load_failures` | `Counter<long>` | `{load}` | Activation-time leaf-snapshot loads that **failed** and were swallowed as "no snapshot" by `BPlusLeafGrain.TryRehydrateFromSnapshotAsync` (issue #2364). Tagged `tree` and `reason`: `resource_exhausted` when an `OutOfMemoryException` appears anywhere in the thrown exception's chain, `faulted` for anything else. It exists because the decline is **indistinguishable from a leaf that genuinely has no snapshot**: both return the same declined rehydrate, the activation then takes the `-1` replay-start override and replays its whole readable WAL window, and the accompanying cold-replay log line reads "no snapshot rehydrate" - true, and read by every operator as "there was no snapshot". Without this counter that failure population reads as zero at every rate of occurrence. The `resource_exhausted` arm is kept separate because that fault **arrives wearing a storage fault's clothes**: under a container memory limit the .NET GC heap hard limit is sized from the cgroup, so the process is never OOM-killed (no restart, no exit code, no resource event) and instead throws inside the provider's deserialise of the blob, surfacing only as the provider's own `Error reading grain state`, which names no memory anywhere. It also **compounds** - the forced cold replay allocates more than the load that just failed, so the same few leaves go cold repeatedly. A sustained non-zero `resource_exhausted` rate means the host is provisioned below the deployment's working set and calls for a higher memory limit, **not** a storage-provider investigation; a sustained `faulted` rate is the opposite. Observation only: nothing here changes the decline, so availability is unaffected. |
...
| `orleans.lattice.materialiser.pin.reports_shed` | `Counter<long>` | `{report}` | Steady-state per-checkpoint pin reports `LeafCursorReporter` declined to enqueue because the target shard's last durable write showed the store is not keeping up. Only that path is shed; the birth block-pin seed and the deactivation flush are last-chance writes and are always issued. Always safe - a shed report leaves the durable pin staler, which retains more WAL. A sustained non-zero rate means the pin store is the bottleneck; pair it with `durable_write_latency` and consider raising `WalMaterialiserPinBuckets`. Tagged `tree`. |
...
| `orleans.lattice.wal.gc.backlog_bytes` | `Histogram<long>` | `By` | Retained WAL bytes remaining after a garbage-collection pass, sampled from the pass's own report so it costs no extra I/O. Emitted only when the byte-pressure policy is enabled (`LatticeOptions.WalMaxRetainedBytes`) and the WAL provider supports byte accounting; because `wal.gc.passes` is emitted unconditionally, a tree reporting passes but no backlog samples is knowably "not measured" rather than "no backlog". Tagged `tree`. |
...
| `Replication` | Orleans.Lattice - Replication | Cross-cluster ship/apply/lag, WAL append vs trim, dead-letter churn, FIFO violations, fall-off-log events, per-peer cursor lag. Sources the `orleans.lattice.replication` meter. |
