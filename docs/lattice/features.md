...
- [FX-064](https://github.com/NSTA1/Orleans.Lattice/issues/1015) - Replication shipper flooded WAL storage with no-op idle tail-poll reads (one Azure Table query per partition per pump tick on an idle link); the WAL shard shipping read now short-circuits from its in-memory tail cursor when the caller is already at or beyond the WAL head, removing the storage round-trip while leaving real backlog reads and write-propagation latency unchanged
