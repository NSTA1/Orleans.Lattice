...
- [FX-061](https://github.com/NSTA1/Orleans.Lattice/issues/947) - WAL GC can trim past a data-holding leaf's durable materialiser frontier before that leaf has seeded a blocking pin - an adaptive-split-created leaf that has not yet registered a pin, or a leaf still inside the pre-first-checkpoint window - so a forward trim driver (the replication shipper, a materialised-view cursor, or the WAL-retention TTL ceiling) removes committed-but-un-materialised data from the WAL in the first place; seed a blocking materialiser pin atomically as the leaf becomes routable so the trim is prevented, not merely surfaced after the fact by the cold-replay fall-off guard
