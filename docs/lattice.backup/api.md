...
- `Task CommitShadowAsync(LatticeRestoreResult shadow, CancellationToken cancellationToken = default)` - commits a built shadow by atomically swapping the registry alias, then refreshing routing and converging any covering tag index. The caller engages the write fence around this call. Idempotent. The physical tree ids carried on `shadow` are re-validated against registry provenance, so a result whose `ShadowPhysicalTreeId` or `PreviousPhysicalTreeId` names a tree that is neither the target, nor a shadow the engine built for that target, nor the target's current physical tree is refused. Throws `ArgumentNullException` (`shadow` null), `ArgumentException` (not a shadow-cutover build result), and `LatticeRestoreValidationException` (a named physical tree does not belong to the target).
