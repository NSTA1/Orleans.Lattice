...
| `LatticeTenantAdminApiOperation` | enum | The per-operation discriminator: `CreateTenant`, `SuspendTenant`, `ResumeTenant`, `DeleteTenant`, `SetTenantQuotas`, `Unknown`, `AuthorizeAllowedRegions`, `SetTenantResidency`, `GetTenantRegionStatus`. An unrecognised method maps to `Unknown`, never to a permissive default. |
