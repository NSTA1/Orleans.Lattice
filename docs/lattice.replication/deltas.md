...
- `byte[]` fields (`LwwRegisterDelta.Value`, `OrSetDot.Element`) compare by **reference**, not content.
- Collection-typed fields (`OrSetDelta.Adds`/`Removes`, `PnCounterDelta.Increments`/`Decrements`, `VersionVectorDelta.Entries`) compare by **reference** as well.
