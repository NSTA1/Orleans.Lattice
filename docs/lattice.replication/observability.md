...
- **Throughput counters** - `wal.entries_appended`, `wal.entries_shipped`. Used to compute growth-rate vs. ship-rate ratios. The companion `wal.entries_trimmed` counter belongs to the core library and is published on the `orleans.lattice` meter (`LatticeMetrics.WalEntriesTrimmed`); subscribe to both meters when correlating ship-rate against trim-rate. The opt-in `ship.redundant_payloads` / `ship.redundant_payload_bytes` counters (see below) ride on the same meter when content-hash dedup measurement is enabled; the opt-in `coalesce.entries_elided` / `coalesce.bytes_elided` / `coalesce.deltas_merged` counters ride on it when pre-ship coalescing is enabled.
