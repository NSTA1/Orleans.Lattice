...
The repair travels the **same** TX-aware, causal-stable apply path as ordinary replication. Re-shipped entries carry their source clock verbatim, so re-sending is idempotent: the receiver suppresses a recently applied `(originClusterId, hlc, key, op)` identity and otherwise re-applies the entry to the same state under per-key last-writer-wins, and a typed CRDT delta merge is itself idempotent, so a re-send never double-applies a counter or re-adds a set element.
...
- **Bounded read window.** Each pass reads from the oldest retained entry of every WAL partition, in partition order, up to a budget of `ReplogPartitions x ShipPartitionPageSize` entries shared across all partitions (so one long partition can use the whole budget). Within that window the pass makes partial progress per cadence as the peer's cursor advances. The window stays anchored at the oldest retained entries rather than following the cursor, so entries beyond it are not reached until WAL garbage collection trims the head; once the peer's cursor has passed every candidate in the window, the selection comes back empty and the pass reports `range_empty` (see the next bullet).
