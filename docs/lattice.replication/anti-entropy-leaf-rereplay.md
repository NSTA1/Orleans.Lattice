...
- **Peer cursor seam defaults to zero.** Reading the peer's applied watermark needs a read-only RPC the default transport does not implement, so `IReplicationDigestProbeTransport.GetPeerHighWaterMarkAsync` returns `HybridLogicalClock.Zero` until a host overrides it. With the default, the pass re-ships every in-range retained entry and relies on the receiver's per-origin idempotent dedup to discard duplicates - correct, but more work than necessary. Override the method to tighten the bound.
