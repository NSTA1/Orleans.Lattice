...
|---|---|---|
| `orleans.lattice.replication.dead_letter.enqueued` | `tree`, `reason in { schema, hlc_skew, oversized, atomic-apply-failure, orphan-transaction, unknown }` | Replog entry parked. The `DeadLetterTrackingReplicationApplier` decorator classifies the terminal failure exception: `ArgumentException` and `InvalidOperationException` are tagged `schema` (malformed entry, missing field, unrecognised `LatticeMergeMode`, CAS-budget exhaustion); every other exception type lands on `unknown`. The `atomic-apply-failure` reason flags an atomic-batch saga that threw or compensated on the receiver — every entry in the batch is parked under one transaction id. The `orphan-transaction` reason flags a partially-staged atomic batch evicted by the maintenance grain's orphan-sweep cadence after exceeding the configured `TxBufferOrphanTimeout` — every staged sibling of the orphan is parked under one transaction id. The `hlc_skew` and `oversized` reason values are reserved for future receiver decorators that surface size / clock-skew violations as classified exceptions. |
