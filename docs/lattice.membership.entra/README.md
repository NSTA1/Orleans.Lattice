...
- **Groups-overage handling.** When `GroupResolutionMode` is `ResolveOnOverage` and a token's group membership overflows - Entra emits the overage markers in place of the `groups` claim - full membership is resolved through a pluggable resolver abstraction (`IEntraGroupResolver`). In the default `TokenOnly` mode, or with no resolver registered, the authenticator applies a documented, dependency-free token-only fallback and never throws.
