...
| `repocontext` | `LATTICE_WAL_MAX_RETAINED_BYTES` | `1073741824` | Base opt-in. An advisory per-tree ceiling that lets WAL garbage collection lower its trim frontier toward 80% of the ceiling, within the frontier that was already safe. Unset, every tree on this box reported `wal_gc_backlog_bytes_unavailable{reason="policy_disabled"}` and `wal_compaction_reclaimed_bytes_total` was zero on all but one tree, so a 3048 MB WAL never gave a byte back. 1 GiB was chosen from measured occupancy: every tree except the two vector accelerators sits under 200 MB, so the ceiling arms exactly the two that grew (`vector-index` 1710 MB, `vector-payload` 1097 MB) and leaves the other nine paying only a probe. `0` is a safe rollback to the library default. |
