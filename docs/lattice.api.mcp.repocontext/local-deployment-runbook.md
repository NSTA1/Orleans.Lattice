...
| `repocontext` | `LATTICE_WAL_MAX_RETAINED_BYTES` | `8589934592` | Base opt-in. An advisory per-tree ceiling that lets WAL garbage collection lower its trim frontier toward 80% of the ceiling, within the frontier that was already safe. Unset, every tree on this box reported `wal_gc_backlog_bytes_unavailable{reason="policy_disabled"}` and `wal_compaction_reclaimed_bytes_total` was zero on all but one tree, so a 3048 MB WAL never gave a byte back. Sized at 8 GiB by issue #3242, after the 4 GiB of issue #3234 was outgrown within hours. The ceiling is compared against PHYSICAL bytes summed over a tree's partitions, and WAL compaction only reclaims once a shard is 50% dead, so steady-state physical settles at about 2x logical; a ceiling below 2x a tree's logical working set is therefore unsatisfiable by construction, and at a 0.8 reclaim target it can never disarm. The 1 GiB original was calibrated during the issue #3229 incident, when repair was starved and occupancy was inflated by the very defect being measured. The 4 GiB replacement was calibrated against a measured 1077 MB logical `vector-index`; that tree then grew to about 2226 MB logical, putting the 2x floor at 4451 MB and overrunning the 4096 MB ceiling by 355 MB, so the ceiling became unsatisfiable a second time for an entirely different reason. 8 GiB clears 2x the measured 2226 MB with headroom on a 1 TB volume holding 6.4 GB of data. The durable lesson: a ceiling calibrated against a measured working set acquires an expiry date the moment that workload can grow, so re-check it against the tree rather than trusting a past calibration - issue #3242 adds an in-band guard so the next overrun reports itself instead of being found by hand. `0` is a safe rollback to the library default. |
...
| `LATTICE_WAL_COMPACTION_MAX_DEAD_BYTES: 268435456` on `repocontext`, supplied by an untracked `.deploy/docker-compose.compaction.yml` | The absolute dead-byte compaction ceiling added in #3107 and wired to this host in #3109. 256 MiB was measured against **this** corpus (about 344 MB dead per shard across 8 evenly sized shards), so it is a figure for this host's data rather than a repository default. The provider default of `0` leaves the ceiling disabled on purpose: compaction rewrites every live byte to reclaim the dead ones, so enabling it by default would be a silent I/O regression on every existing deployment. Putting it in `docker-compose.tuning.yml` would also have to answer that overlay's `:?` no-default convention, which exists for host-derived grants (#2779) and not for a policy knob. | A corpus-independent default is agreed and tracked, or the ratio trigger is replaced so no absolute ceiling is needed. Until then, **a redeploy that omits this override silently restores unbounded WAL growth** - the ratio trigger alone does not fire until the log reaches about 2x live size. |
