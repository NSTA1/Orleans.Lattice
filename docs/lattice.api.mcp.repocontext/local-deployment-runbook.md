...
| `repocontext` | `LATTICE_WAL_MAX_RETAINED_BYTES` | `4294967296` | Base opt-in. An advisory per-tree ceiling that lets WAL garbage collection lower its trim frontier toward 80% of the ceiling, within the frontier that was already safe. Unset, every tree on this box reported `wal_gc_backlog_bytes_unavailable{reason="policy_disabled"}` and `wal_compaction_reclaimed_bytes_total` was zero on all but one tree, so a 3048 MB WAL never gave a byte back. Sized at 4 GiB by issue #3234. The ceiling is compared against PHYSICAL bytes summed over a tree's partitions, and WAL compaction only reclaims once a shard is 50% dead, so steady-state physical settles at about 2x logical; a ceiling below 2x a tree's logical working set is therefore unsatisfiable by construction, and at a 0.8 reclaim target it can never disarm. The earlier 1 GiB was calibrated during the issue #3229 incident, when repair was starved and occupancy was inflated by the very defect being measured. `vector-index` alone holds about 1077 MB logical, so 1 GiB sat below its logical size and the 819 MB disarm point was unreachable. 4 GiB clears 2x the largest tree with headroom on a 1 TB volume holding 6.4 GB of data. `0` is a safe rollback to the library default. |
