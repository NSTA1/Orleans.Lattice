...
| `repo-context-vector-index` | The persisted approximate nearest-neighbour index, one exclusive prefix per repository and embedding space. Wholly derived and local: never replicated | Higher (every flush rewrites cell chunks; a retrain or rebuild range-deletes a superseded generation) |
| `repo-context-vector-coverage` | The per-page vector-coverage digest mirroring vector membership, so gap detection reads a fixed 257 rows. Wholly derived and local: never replicated | Moderate (every covered source rewrites its page) |
...
A host tunes a tree by name through `IOptionsMonitor<LatticeOptions>.Get(treeName)`. The higher-churn trees (memory, vector membership, vector metadata, the vector-coverage digest, the approximate index, the session bookkeeping, and the structural, symbol, content, and cross-reference trees, which the reconcile prunes and re-writes) should carry a finite tombstone grace period and a compaction trigger so re-write and forget tombstones are reaped; the local container applies exactly that to all ten in every durability profile, leaving out only the write-once vector-payload tree.

### Symbol nodes

The symbol tree holds one record per declared symbol at `repo/{repoId}/symbol/{fqName}`, extracted from source during onboarding and kept converged by the same incremental reconcile that maintains file nodes. Extraction is language-dispatched behind a per-language seam; C# is extracted with the Roslyn syntax parser today, and a file whose language has no registered extractor simply declares no symbols. A symbol's declaring files are an add-wins set, so a symbol split across several files - a C# partial type is the canonical case - survives as long as any file still declares it and is pruned only once the last one stops. The package itself writes every tree, the symbol tree included, without a per-record schema-version stamp. The container host goes one step further: at startup it opts the symbol tree - and only the symbol tree - in to self-describing [schema-version envelopes](../lattice.schema/README.md) (schema family 1, target version 1), so on that host every symbol value carries a version stamp and a future change to the symbol record shape can ship as a new target version with an upcaster rather than a reinterpretation of stored bytes. Every other tree stays unversioned and byte-identical.
...
The leading segment after the repository id (`pkg`, `file`, `symbol`, `content`, `xref`, `mem`, `session`, `vec`, `vpay`, `vmem`, `vcov`, `vidx`) is the family discriminator, so a key both addresses a record and routes it to the correct tree. The one key outside this grammar is the memory archive's store-local restore-state marker, which deliberately sits outside every repository's range so it is never exported or counted as memory (see [Memory durability](memory-durability.md)). Because keys are ordered, a scope like "every file under a repo" or "every memory entry under a topic" is a single bounded range walk.
...
On an evaluating read - `repocontext_recall` of a memory entry, and each neighbor of a `repocontext_neighbors` walk - the store walks the **live link set** and checks every structural target against its present state. A link is fresh only when a digest was captured for it, the target still has a live record, and the two digests are ordinal-equal; every other outcome is reported through `stale` and `staleLinks`. Three outcomes are therefore stale: the target **drifted**, the target has **no live record** (deleted, or never present in the corpus at all), or **no digest was ever captured** for the edge so drift was never measurable. The walk is driven by the live links rather than by the captured-digest map, so an unlinked-but-still-recorded digest is never evaluated and removing an edge cannot leave a phantom flag.

The subset of `staleLinks` whose target has no live record is also named in **`danglingLinks`** - always a subset of `staleLinks`, never a partition of it, so a caller reading only `staleLinks` keeps full coverage. The distinction is worth a field because the two states have **opposite remedies**: a drifted target asks the caller to re-read the file now, whereas a dangling one - most often a note written about code that has not yet reached the indexed branch - asks it to do nothing and retry once the target is indexed. Deciding the answer from the target's present state rather than from whether a digest happened to be captured is what makes the guarantee reachable: before this, a target deleted *after* the edge was written was flagged while one that never reached the corpus was not, so the answer turned on link-time history no caller can observe.

Bulk reads (`repocontext_scan`, semantic-search hydration) do **not** evaluate staleness and leave `stale`/`staleLinks`/`danglingLinks` null ("not evaluated"), exactly as they leave the expiry fields null - `recall` a key for the authoritative answer. The captured digest is a change-detection fingerprint only, never a security boundary, and the map merges last-writer-wins so concurrent captures converge.
