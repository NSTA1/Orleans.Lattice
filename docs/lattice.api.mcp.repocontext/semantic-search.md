...
Vectors are produced by the indexing path. `AddRepoContextTools` always wires the embed-and-store ingestor in place of the deferred no-op - the registration does not depend on `enableWrites` - so an indexing pass embeds the files it added or updated and lands their vectors on the reserved vector trees. A later search then finds them by meaning. If no embedder is bound at onboarding time, no vectors are written and search stays on the keyword path until vectors exist.
...
Each reconcile pass runs the three embedding arms in a fixed order: **memory first, then files, then symbols**. Memory is the smallest arm and the most valuable per vector, so on a fresh onboarding or after `repocontext_reset_index` captured decisions and gotchas become semantically searchable within the first pass rather than waiting behind the whole file and symbol back-fill. Files precede symbols because symbols outnumber files and file recall is what `repocontext_context` resolves to. Each arm is fault-isolated: a failure in one does not skip the others, and a memory- or symbol-arm failure does not withdraw the coverage verdict the file arm measured (it is still counted as `arm_failure` on `repocontext.bootstrap.coverage_verdict`). Any arm failure still fails the pass, so the next reconcile re-drives it.
...
The heal is a **cure, not a mask**. Every write and coverage probe against the vector-metadata and vector-membership trees runs through a guard at the single seam where the target tree is a known local constant. When that guard observes the terminal fall-off, it always logs the originating exception with its full stack trace and increments a dedicated `repocontext.vectorplane.rederive` counter (tagged with the tree and the outcome) **before** any remediation, then triggers a bounded, single-flight, idempotent re-derivation of that one tree: the terminal state is reset so the tree activates clean, and the always-on gap scanner and back-fill re-embed every uncovered source from the store-of-record structural, symbol, and memory trees plus the working files. A re-derivation already in flight for a tree is joined rather than restarted, and the originating fault still propagates so the current pass fails loudly and the next always-on pass converges once the reset has landed.

The reset itself issues a privileged tree-lifecycle operation, so it runs inside a **system-origin scope**: it is infrastructure-authored maintenance with no user identity behind it, and evaluating it against whatever caller subject happened to be ambient when the fall-off surfaced is the wrong question. The scope is not a general relaxation of the access gate, and the gate's own default is never widened. It is bounded by the fail-closed allow-list that precedes it (a store-of-record tree can never reach it), by the target tree name being a local layout constant rather than any wire- or exception-derived value, by containing nothing but the delete and purge of the rebuildable projections and the coverage digest that mirrors them, and by being lexically scoped and disposed so it never leaks into a caller's turn.

A reset that does not succeed is **not** re-attempted on the observation cadence. Each tree carries a per-tree backoff that doubles up to a cap and is cleared by a completed reset; an observation inside that window is metered `suppressed` and logged at debug. A reset the access gate refuses is partitioned separately, as `denied`, and gets a much longer schedule, because unlike a transient fault it is a deterministic decision that cannot clear on retry - retrying it at observation cadence only produces identical refusals that bury the underlying fall-off signal.

Re-derivation is **fail-closed**. It applies only to the two rebuildable vector projections (`repo-context-vector-metadata` and `repo-context-vector-membership`). The write-once, content-addressed payload tree is excluded (it has no in-place deletes and cannot be re-derived by a drop-and-re-embed), and every store-of-record tree - structural, symbol, agent memory - is refused outright: resetting one of those would be real data loss. A refused fall-off is still surfaced (logged and metered) but never auto-reset. Because only the vector-metadata and vector-membership paths run through the guard, no other tree reaches it in the current wiring: the refusal is a defence-in-depth check, and a fall-off on any other tree surfaces as an ordinary fault on its own path rather than on this counter.
...
- It **stands aside for the indexing pacer**: while a search or context call is in flight on the silo, or the embedding drain is waiting on a saturated vector tree or backing off a congested one, a tick takes no step - for at most 15 consecutive ticks, about 30 seconds at the two-second phase period, after which it steps regardless, so foreground load slows the build but never stops it. The first step of an activation, which opens the in-memory index queries are served from, is never deferred, and `LATTICE_REPOCONTEXT_PACING=false` switches the deferral off with the rest of the pacer.
...
**Multi-silo note.** A coordinator is a single cluster-wide activation, and the in-memory index a query is served from is per silo. A query never opens an index itself: on a silo where no coordinator step has run, the plane reports `bootstrapping` and the exact scan answers in its place. On a multi-silo host the coordinator's silo is therefore warmed with no query, and another silo serves approximately only once a later arming - the periodic sweep, or an indexing pass - activates the coordinator there; that open is a **reload** of the already-built index rather than a rebuild. The expensive half - streaming the corpus - is paid once, off the request path, whichever silo hosts the coordinator.
...
Faults are announced once per episode rather than once per attempt. The first fault of a run is a warning carrying the exception; its repetitions go to the counter, because the retry backoff tops out at thirty seconds and an unconditional line would write roughly 2,880 of them a day for as long as the fault lasted. A sweep that completes after one or more faults logs a closing line reporting how many consecutive attempts had failed, so an episode that has ended is distinguishable from one still in progress.
...
Refusing to converge means the coordinator stays alive, so it must not spin. Retries back off from the two-second phase period out to a five-minute ceiling, and a backed-off tick takes no build step and no probe at all. After five consecutive uninterpretable reads the coordinator emits `repocontext.ann.build.denial_terminal` once and one `Error` line, then keeps retrying as the backoff climbs to that ceiling (reached from the eighth consecutive refusal) - so a grant that seeds slightly after the first phase tick is still picked up without a restart, while a permanently refused deployment is loudly parked rather than quietly busy. Any non-zero value on that counter warrants an operator: an index that should exist does not, and it will not appear on its own.
...
- A **repository-scoped breaker** observes the fault directly. A gather that does stall is caught rather than propagated, and no further gather is started for that repository until either the approximate plane answers for itself - at which point the fallback is restored with no cooldown to wait out - or a time-based half-open probe runs one gather and it completes. The probe is what keeps the exit reachable when the plane never answers at all: the first is allowed 60 seconds after a stall and the delay doubles per further stall to a 15 minute ceiling, so a permanently wedged repository settles at one stall ceiling per 15 minutes rather than one per query. This is what makes failing open on an unknown corpus safe: the cost of a wrong prediction is one query, once per repository per build.

A gather the corpus bound declines is reported as `keyword.vector_plane_unavailable` - a plane that is still building - and never as `keyword.index_degraded`, which would claim a capability loss that is not present. A gather the **breaker** is withholding reports `keyword.exact_fallback_suppressed` instead, because the two have opposite remedies: the first needs the build to finish, whereas the second needs the contention that stalled the gather to clear and will retry on its own whether or not the build ever completes. The corpus bound keeps the weaker value deliberately, because it is re-evaluated from a live count on every query and so cannot outlive its cause, which is exactly the property the breaker lacked. No result is ever wrong on any of these paths: the same keyword recall is returned.

Below the corpus bound nothing changes and the exact gather still answers with complete recall, which is the regime a small repository sits in permanently. Because the derived threshold lands above the training minimum of 1,024 vectors, a corpus too small to train a partitioning is answered exhaustively by the plane itself and never reaches the fallback at all.
