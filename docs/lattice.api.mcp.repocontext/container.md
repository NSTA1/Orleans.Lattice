...
| `LATTICE_WAL_COMPACTION_MAX_DEAD_BYTES` | `0` (disabled) | An absolute ceiling, in bytes, on the dead (trimmed but not yet reclaimed) bytes a single file-WAL shard may hold before a compaction is forced. Compaction is otherwise triggered only by a **ratio** - dead bytes reaching half the file - which bounds waste relative to live data rather than absolutely, so a growing log roughly doubles in size before any space returns to the filesystem. Left at `0` that ratio is the only trigger. Set it when the log's absolute footprint matters more than write amplification: compaction rewrites every live byte to reclaim the dead ones, so a ceiling far below a shard's live size makes each reclaim progressively more expensive. `orleans_lattice_wal_compactions_total{trigger="ceiling"}` reports when this trigger fires.
