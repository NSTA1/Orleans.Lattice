...
| `Replication` | `orleans.lattice.replication`, plus `orleans.lattice` for its WAL, WAL-compaction and WAL GC panels | Cross-cluster operator view: ship / apply / lag durations, WAL append vs trim throughput, dead-letter churn, apply FIFO and causal violations, fall-off-log events, and per-peer cursor lag. Its replication panels need the replication package registered on the silo; its WAL, WAL-compaction and WAL GC panels read the core meter. |
