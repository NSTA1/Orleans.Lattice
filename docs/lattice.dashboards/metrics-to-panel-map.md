...
| `orleans.lattice.materialiser.lagging_consumers` | histogram (`{consumer}`) | `tree` | *(not charted)* | Consumers individually past the drain-lag threshold on a tree already over it (issue #2444). Decomposes the `materialiser.drain_lag` minimum above, which reads identically for one dormant consumer and for many falling behind. Emitted only for over-threshold trees, so a healthy estate reports nothing and an absent series is the expected reading. Triageable, not diagnosable: it does not name the contributor (issue #2505) |
