...
| `repocontext.ann.index.load` | counter (`{attempt}`) | `outcome` = `fresh`, `resumed`, `faulted` | (none) | **not charted** - a two-sided discriminator read as a pair, not as a rate: `faulted` rising with `resumed` flat is a durable-index load that is restarting rather than resuming (#2953), and no faults at all is health. All three arms are pre-minted at zero, so an absent arm means the build did not ship. |
