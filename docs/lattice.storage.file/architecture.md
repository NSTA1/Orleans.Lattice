...
Caller-assigned offsets are stored verbatim. The provider never assumes contiguity with the current tail: it rejects an append that overlaps any persisted offset, and it accepts a gap, so out-of-order concurrent appends (`LatticeOptions.WalMaxPendingBatches` greater than 1) are supported and a failed flush surfaces as an honest gap rather than a silently renumbered tail. `GetHighestOffsetAsync` returns the shard's high-water mark: the highest committed offset, or the durable trim watermark when that is higher (issue #3366). It therefore never regresses when a trim empties the shard - otherwise the WAL grain would restart offset allocation beneath a watermark that the next recovery honours, and that recovery would discard acknowledged writes as already trimmed. `GetLowestOffsetAsync` returns the lowest still-retained offset, or `-1` when no entry is retained, so a caller can bound the live range without scanning the log.

## Filtered replay reads

A leaf replaying a WAL partition owns only some of its records, and passes that ownership down to the provider as a `WalKeyFilter` (issue #3565). `ReadFilteredAsync` reads the first kilobyte of each record in the window into a stack buffer and classifies the record from it. The routing fields - the tree id, the mutation kind, and the key - are the first fields of an encoded record, so the prefix holds them unless the tree id and key are unusually long. A record the filter excludes is then skipped without its payload being read in full or decoded, so it costs one short read from the page cache and no allocation. The one exception is the last record of the page, as the core contract requires: when it is excluded it is still returned routing-only (its offset, mutation kind, and key), so a reader that resumes from the last offset it received moves past everything that was dropped. An owned record is read and decoded as `ReadAsync` would read it. A record the prefix cannot classify - a key that runs past it, say - is decoded in full and judged from the result, which gives the same answer. The page is bounded exactly as an unfiltered one is: by entry count, by examined payload bytes narrowed under memory pressure, and by the quarter-and-retry on an allocation failure.

The classification needs the routing reader that `AddFileWalStorage` builds from the silo's serializer session pool. A provider constructed through its public constructor has none, and decodes every record it examines: it returns the same records, at the cost the filtered read exists to avoid.

## Trimming and compaction

`TrimAsync` marks every entry at or below the supplied offset as dead and advances the retained head; it is idempotent, and trimming through an offset that does not yet exist reserves the trim point for a future append. Dead payload bytes are not reclaimed in place. Instead, once a shard's dead bytes cross both `CompactionThreshold` (a fraction of the on-disk payload) and `CompactionMinimumDeadBytes` (an absolute floor that prevents churn on small trims), the segment file is rewritten to a fresh copy holding only the live entries, and the old file is replaced atomically. Setting `CompactionThreshold` to `1.0` or greater disables threshold-triggered compaction; space is then reclaimed only by the absolute ceiling below, when `CompactionMaximumDeadBytes` is enabled, or by the next reconciliation (at activation, or in the WAL grain's resync after a failed flush).
...
`ReconcileAsync` runs at grain activation, before the WAL grain reads the highest offset, and again when the WAL grain resyncs after a failed flush - by then the log is already loaded, so that second call only reclaims trimmed space. On first load it scans the segment file, rolls every committed batch forward, discards any torn or uncommitted trailing batch, and reclaims previously-trimmed space. After reconciliation the retained shard entries are sorted by offset and durable, preserving any honest gaps from failed or out-of-order appends, so normal reads and writes rely on a consistent view. This is the local-disk analogue of the Azure Table provider's phase-1/phase-2 orphan repair.

## Durable WAL garbage collection

`AddFileWalStorage` also registers the same durable-WAL GC wiring the Azure Table provider installs - the WAL cursor registry, the leaf reporter, and the WAL GC. The cursor registry itself stays the process-local `InMemoryWalCursorRegistry` that `AddLattice` installs unless the host registers its own; what the wiring adds is a leaf reporter that also mirrors each leaf's applied frontier into cluster-wide pins persisted through the configured grain storage, which the GC consults, so a restart that wipes the registry cannot let it trim entries a not-yet-reactivated leaf still needs. All three are registered idempotently, so a host that already supplied its own keeps it.
