...
|-- start-history.ps1                # Bring up the long-lived history stack only (and open its
|                                    # Grafana); stop it with `./benchmark.ps1 -CloseHistory`.
...
6. Writes the result to **`benchmark/.fleet-size.config`**, which is gitignored
   ...
   # Calibrated:        2026-05-15T08:30:12.0000000Z
   # Git sha:           abc1234
   # Scenario:          current-state-no-replication
   # Ladder probed:     500, 1000, 2000, 4000, 8000, 16000
...
| `-WarmupSeconds`     | `15`                                   | Per-rung warm-up, used only in the time estimate; the actual warm-up comes from the scenario `.env`. |
| `-DurationSeconds`   | `30`                                   | Per-rung measurement window, used only in the time estimate; the actual window comes from the scenario `.env`. |
...
8. **Captures an auto-discovered panel of summary scalars** while the stack is
   ...
   collisions, and `$ScalarAliases` copies a few auto-discovered keys to short
   KPI names.
...
Each run produces a `.run/<scenario>/<run_id>/results.json` like the one below.
`run_id` is the UTC time (`yyyy-MM-ddTHH-mm-ssZ`) at which `benchmark.ps1` starts the
run, taken before the stack comes up; `started` is stamped once `docker compose up`
returns, so it never precedes `run_id`:
...
```json
{
  ...
  "git_sha":  "abc1234",
  "started":  "2026-04-30T14:10:07.4213381Z",
  "ended":    "2026-04-30T14:15:37.9036412Z",
  "duration_s": 330,
...
Five configuration blocks at the top of `benchmark.ps1` drive it:
...
| `$ScalarAliases`            | Short, stable KPI names (e.g. `replication_ship_p95_ms`) copied in memory from auto-discovered keys after capture - no extra PromQL. An alias whose source key was not emitted is `null`. |
...
- you need a before/after measurement for a WAL hot-path optimisation against real Azure
  Tables latency (the silo accepts `BENCH_*` env overrides for every WAL-side knob so
  the same harness can drive both arms of an A/B);
...
VictoriaMetrics stack - results land in `benchmark/.run/azure-throughput/` as a silo
