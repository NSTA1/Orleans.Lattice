...
| Cohort runner `Verdict` | `WEDGE` on 3/3 | mixed: `DEGRADED` / `WEDGE` / `WEDGE` on the three `set-many` cohorts pre-FX-029 (DEGRADED from cross-cohort residual-grain exceptions; WEDGE from drain-tail trips) | **Partial.** FX-029 closes the drain-tail leg (the two WEDGE cohorts' root cause); the DEGRADED tag remains until FX-031 / #615 filters cross-cohort residual-grain exceptions out of the runner's verdict tally. Next live cohort run will confirm 3/3 cleanly. |
...
The producer-active window is the surface F-086 was scoped to address (per the parent issue's AC #2: "Pauses TCP ingest reading on `Saturated`"). That surface is now clean: the kernel TCP window naturally back-pressures the producer once the silo crosses the saturation threshold, and the producer-active window holds `failed=0` consistently. FX-029 has since closed the drain-tail leg (the residual ingest-channel batch at producer-stop is now abandoned when the silo has recently been `Saturated`, surfacing on FINAL as `discarded=N` rather than queueing into the in-flight pipeline where it would trip `WalAppendDispatchTimeout` and surface as `failed=N`). Two remaining gaps (classifier flapping FX-030, cohort-runner verdict noise FX-031) are tracked as discrete follow-ups; together they collapse the remaining "Partial" rows above into "Yes".

### 33.4 Observed remaining gaps (post-F-086)

Three sibling FX issues were opened from the F-086 closeout run to track the path from "producer-phase clean" to "end-to-end clean". FX-029 has since shipped; the remaining two are tracked as discrete follow-ups.
...
The two remaining open issues are independent: FX-030 makes the advisory `Throttled` regime observable for the producer-active path, FX-031 makes the runner verdict an honest reflection of the current cohort. They can ship in any order; together they collapse the remaining "Partial" rows in §33.3 above into "Yes".
...
- **End-to-end `failed=0` on FINAL.** F-086 closed the producer-active window; FX-029 closed the drain-tail residual-batch leak. The cohort runner verdict can still report DEGRADED until FX-031 filters cross-cohort residual-grain exceptions out of its tally.
