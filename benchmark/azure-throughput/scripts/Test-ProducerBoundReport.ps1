...
try {
    $cases = @(
        @{ Name = 'legacy slip'; Line = '[producer] t= 30.0s sent=1 rate=460,000 msg/s slipMaxMs= 12000.0'; Bound = $true },
        @{ Name = 'final slip'; Line = '[producer] DONE total=1 elapsed=1s avg=400,000 msg/s genBlockedFrac=0.010 slipMaxMs=1001.0'; Bound = $true },
        @{ Name = 'backpressure below offered'; Line = '[producer] t=1s genBlockedFrac=0.200 slipMaxMs=20.0'; Bound = $true },
        @{ Name = 'at thresholds healthy'; Line = '[producer] t=1s genBlockedFrac=0.199 slipMaxMs=1000.0'; Bound = $false },
    ...
    [IO.File]::WriteAllText($log, '[producer] t=1s genBlockedFrac=0.5 slipMaxMs=20.0')
    ...
    Assert-Case 'periodic maxima survive final summary' ($cohort.producerBound -and $cohort.producerSlipMaxMs -eq 12000 -and $cohort.producerGenBlockedFracMax -eq 0.8)
